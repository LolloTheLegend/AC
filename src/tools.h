...
template<class T>
	struct databuf
	{
		...
		databuf () :
				buf(NULL), len(0), maxlen(0), flags(0)
		{
		}

		template<class U>
			databuf ( T *buf, U maxlen ) :
					buf(buf), len(0), maxlen((int) maxlen), flags(0)
			{
			}
...
struct packetbuf : ucharbuf
{
	...
	packetbuf ( ENetPacket *packet ) :
			ucharbuf(packet->data, packet->dataLength), packet(packet),
			        growth(0)
	{
	}
...
template<class T>
	struct bitbuf
	{
		...
		{
			return ( 8 - blen ) % 8;
		}   // 0..7 bits remaining in current byte
	};
...
template<class T>
	struct vector
	{
		...
		template<class ST>
			void sort ( int (__cdecl *cf) ( ST *, ST * ),
			            int i = 0,
			            int n = -1 )
			{
				qsort(&buf[i], n < 0 ? ulen : n, sizeof(T),
...
template<class K, class T>
	struct hashtable
	{
		...
		{
			loopi(CHUNKSIZE-1)
				emptychunk->chains[i].next = &emptychunk->chains[i + 1];
...
#undef HTFIND
		...
		{
			uint h = hthash(key) & ( size - 1 );
			for ( chain **p = &table[h], *c = table[h]; c;
			        p = &c->next, c = c->next ) {
				if ( htcmp(key, c->key) ) {
					*p = c->next;
					c->data.~T();
...
static inline char *newstring ( size_t l )
{
	return new char[l + 1];
}
static inline char *newstring ( const char *s, size_t l )
{
	return copystring(newstring(l), s, l + 1);
}
static inline char *newstring ( const char *s )
{
	return newstring(s, strlen(s));
}
static inline char *newstringbuf ()
{
	return newstring(MAXSTRLEN - 1);
}
static inline char *newstringbuf ( const char *s )
{
	return newstring(s, MAXSTRLEN - 1);
}
