...
void remip ( const block &b, int level )
{
	...
	for ( int y = s.y; y < s.y + s.ys; y += 2 ) {
		...
		for ( int x = s.x; x < s.x + s.xs; x += 2 ) {
			*r = *o[0];
			r->type =
			        o[0]->type == o[1]->type && o[0]->type == o[2]->type
			        && o[0]->type == o[3]->type ? o[0]->type : SEMISOLID;
			if ( !SOLID(r) ) {
			...
			if ( SOLID(r) ) {
				...
				loopi(3)
				{
					...
					if ( * ( (uint32_t *) q ) != * ( (uint32_t *) o[0] )   // type, floor, ceil, wtex
					     ...
					     || q->ftex != o[0]->ftex || q->ctex != o[0]->ctex
				...
				if ( r->type == CHF || r->type == FHF )   // can make a perfect mip out of a hf if slopes lie on one line
				        {
					if ( o[0]->vdelta - o[1]->vdelta != o[1]->vdelta
					        - SWS(w,x+2,y,wfactor)->vdelta
					     || o[0]->vdelta - o[2]->vdelta != o[2]->vdelta
					             - SWS(w,x+2,y+2,wfactor)->vdelta
					     || o[0]->vdelta - o[3]->vdelta != o[3]->vdelta
					             - SWS(w,x,y+2,wfactor)->vdelta
					     || o[3]->vdelta - o[2]->vdelta != o[2]->vdelta
					             - SWS(w,x+2,y+1,wfactor)->vdelta
					     || o[1]->vdelta - o[2]->vdelta != o[2]->vdelta
					             - SWS(w,x+1,y+2,wfactor)->vdelta ) goto c;
				}
			}
			mip: r->defer = 0;
			c: ;
...
static void nextclosestent ( void )
{
	clentsel++;
}
...
int closestent ()        // used for delent and edit mode ent display
{
	...
	loopj(3)
	{
		...
		loopv(ents)
		{
			...
			bool ice = false;
			loopk(eh_ents.length())
				if ( eh_ents[k] == e.type ) ice = true;
			if ( ice ) continue;
			vec v(e.x, e.y, e.z);
			float dist = v.dist(camera1->o);
			if ( j ) {
				if ( ents[best].x == e.x && ents[best].y == e.y
				     && ents[best].z == e.z ) {
...
static void entproperty ( int prop, int amount )
{
	...
	switch ( e.type )
	{
		...
		case SOUND:
			...
			location *loc = audiomgr.locations.find(e.attr1 - amount, &entref,
			                                        mapsounds);
			if ( loc ) loc->drop();
	}
...
{
	loopi(MAXENTTYPES)
		if ( strcmp(what, entnames[i]) == 0 ) return i;
	conoutf("unknown entity type \"%s\"", what);
	return NOTUSED;
}

entity *newentity ( int index,
                    int x,
                    int y,
                    int z,
                    char *what,
                    ...
                    int v4 )
{
	...
	switch ( type )
	{
		...
		case CTF_FLAG:
			...
			if ( type != PLAYERSTART )
			    e.attr1 = ( e.attr1 + 7 - ( e.attr1 + 7 ) % 15 ) * 4;
			break;
	}
	syncentchanges(true);
	addmsg(SV_EDITENT, "ri9", index < 0 ? ents.length() : index, type, e.x, e.y,
	       e.z, e.attr1, e.attr2, e.attr3, e.attr4);
	e.spawned = true;
	int oldtype = type;
	if ( index < 0 )
		ents.add(e);
...
int findentity ( int type, int index, uchar attr2 )
{
	...
	loopj(index)
		if ( ents[j].type == type && ents[j].attr2 == attr2 ) return j;
	return -1;
}
	...
	loop(x, ssize)
		loop(y, ssize)
		{
			if ( x >= x1 && x < ssize - x2 && y >= y1 && y < ssize - y2 ) {
				if ( S(x,y)->type != SOLID && ( S(x,y)->ceil > 126 - z1
				        || S(x,y)->floor < -127 + z2 ) ) return false;
			}
			else {
...
void calcmapdims ()
{
	...
	mapdims.minfloor = 127;
	mapdims.maxceil = -128;   // flow initialised those two with "0", but i'd guess, that was by mistake
	sqr *s = world;
	for ( int y = 0; y < ssize; y++ )
		for ( int x = 0; x < ssize; x++ ) {
			if ( !SOLID(s) ) {
				if ( OUTBORD(x, y) )
				    conoutf("non-solid cube in world border found: x %d, y %d",
				            x, y);   // will also go fatal() below
	...
	if ( mapdims.x2 < mapdims.x1 || mapdims.y2 < mapdims.y1 ) {   // map is completely solid -> default to empty map values
		mapdims.x1 = mapdims.y1 = 2;
		mapdims.x2 = mapdims.y2 = ssize - 3;
		mapdims.minfloor = 0;
		mapdims.maxceil = 16;
	}
	if ( mapdims.x1 < MINBORD || mapdims.y1 < MINBORD
	     || mapdims.x2 >= ssize - MINBORD || mapdims.y2 >= ssize - MINBORD )
	    fatal("calcmapdims(): world border violation (x:%d..%d, y:%d..%d)",
	          mapdims.x1, mapdims.x2, mapdims.y1, mapdims.y2);
...
bool empty_world ( int factor, bool force )   // main empty world creation routine, if passed factor -1 will enlarge old world by 1, factor -2 will shrink old world by 1
{
	...
	if ( factor == -2 && !worldbordercheck(ssize / 4 + MINBORD,
	                                       ssize / 4 + MINBORD,
	                                       ssize / 4 + MINBORD,
	                                       ssize / 4 + MINBORD, 0, 0) ) {
...
COMMANDF(
        calcmipstats,
        "",
        () { int st[SMALLEST_FACTOR + 1]; countperfectmips(SMALLEST_FACTOR, 0, 0, 0, st); conoutf("current mips: %d / %d / %d / %d / %d / %d / %d", st[0], st[1], st[2], st[3], st[4], st[5], st[6]); });
...
{
	int sta[SMALLEST_FACTOR + 1], stb[SMALLEST_FACTOR + 1], stc[SMALLEST_FACTOR
	        + 1];
	sqr *s, *r, *o[4];
		...
		for ( int y = 1; y < bs - 1; y++ )
			for ( int x = 1; x < bs - 1; x++ ) {
				...
				if ( r->type == CORNER )
				    ...
				    {
#define cc(p) s->p = r->p
					s = SWS(wmip[mip - 1], x * 2 + ( i & 1 ), y * 2 + i / 2,
					        mfactor + 1);
					cc(floor);
					cc(ceil);
					cc(wtex);
...
#undef cc
	...
	sqr *worldbackup = (sqr *) memcpy(new sqr[ssize * ssize], world,
	                                  ssize * ssize * sizeof(sqr));
	r = world;
	...
	r = world;
	loopirev(ssize * ssize)
		( r++ )->reserved[0] = INVISUTEX | INVISWTEX;
			...
			loopi(4)
				if ( !SOLID(o[i]) ) {
					if ( SOLID(s) || s->floor > o[i]->floor
					     || ( o[i]->type == FHF && ( s->type != FHF
					             || s->floor != o[i]->floor ) ) ) rs &=
					        ~INVISWTEX;   // wall texture visible
					if ( !SOLID(s) && ( s->ceil < o[i]->ceil
					        || ( o[i]->type == CHF && ( s->type != CHF
					                || s->ceil != o[i]->ceil ) ) ) ) rs &=
					        ~INVISUTEX;   // upper wall texture visible
				}
			if ( SOLID(
			        s) && SOLID(o[0]) && SOLID(o[1]) && SOLID(o[2]) && SOLID(o[3]) )
			    rs |= INVISIBLE;
		}
	...
	for ( int mip = SMALLEST_FACTOR; mip > 0; mip-- ) {
		...
		for ( int y = 1; y < bs - 1; y++ )
			for ( int x = 1; x < bs - 1; x++ ) {
				...
				if ( s->type == CORNER && ( mip == SMALLEST_FACTOR
				        || SWS(wmip[mip + 1], x / 2, y / 2, mfactor - 1)->type != CORNER ) ) {   // highest corner mip (this one is rendered) - mark helper cubes
	...
	for ( int mip = SMALLEST_FACTOR; mip > 0; mip-- ) {
		...
		for ( int y = 1; y < bs - 1; y++ )
			for ( int x = 1; x < bs - 1; x++ ) {
				s = SWS(wmip[mip], x, y, mfactor);
				if ( s->type == CORNER && ( mip == SMALLEST_FACTOR
				        || SWS(wmip[mip + 1], x / 2, y / 2, mfactor - 1)->type != CORNER ) ) {   // highest mip - mark the important cube like the highest mip
	...
	for ( int mip = SMALLEST_FACTOR; mip > 0; mip-- ) {
		...
		for ( int y = 0; y < bs; y++ )
			for ( int x = 0; x < bs; x++ ) {
				...
				domip0(if(!(rs & INVISWTEX) && s->wtex != wtex) { wtex = s->wtex; wdiff++; } if(!(rs & INVISUTEX) && s->utex != utex) { utex = s->utex; udiff++; });
