...
static const char *levelname[] = { "DEBUG", "VERBOSE", "INFO", "WARNING",
                                   "ERROR", "DISABLED" };
static FILE *fp = NULL;
...
bool initlogging ( const char *identity,
                   int facility_,
                   int consolethres,
                   int filethres,
                   int syslogthres,
                   bool logtimestamp )
{
	...
	if ( consolethres >= 0 )
	    consolethreshold = min(consolethres, (int) ACLOG_NUM);
...
bool logline ( int level, const char *msg, ... )
{
	...
	const char *ts = timestamp ? timestring(true, "%b %d %H:%M:%S ") : "", *ld =
	        levelprefix[level];
	char *p, *l = sf;
