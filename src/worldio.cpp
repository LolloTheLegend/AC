...
static string cgzname, ocgzname, bakname, cbakname, pcfname, mcfname, omcfname,
        mapname;
...
const char *setnames ( const char *name )
{
	...
	formatstring(cbakname)(
	        "packages/%s/%s.cfg_%s.BAK", pakname, mapname, numtime());
...
#define c(f) (s->f==t->f)
	while ( len-- > 0 ) {
		...
		if ( SOLID(s) && !preservesolids ) {
			...
			if ( t && c(
			        type) && c(floor) && c(ceil) && c(ctex) && c(ftex) && c(utex) && c(wtex) && c(vdelta) && c(tag) ) {
				sc++;
			}
			else {
				spurge;
...
struct headerextra
{
	...
	uchar *data;
	headerextra () :
			len(0), flags(0), data(NULL)
	{
	}
	headerextra ( int l, int f, uchar *d ) :
			len(l), flags(f), data(NULL)
	{
		if ( d ) {
			data = new uchar[len];
			memcpy(data, d, len);
	...
	headerextra *duplicate ()
	{
		return new headerextra(len, flags, data);
	}
};
...
static const char *hx_names[] = { "unused", "mapinfo", "modeinfo", "artist",
                                  "editundo", "config", "vantage point",
                                  "unknown" };
...
{
	loopvrev(headerextras)
		delete headerextras.remove(i);
}

static void deleteheaderextra ( int n )
{
...
COMMANDF(
        listheaderextras,
        "",
        () { loopv(headerextras) conoutf("extra header record %d: %s, %d bytes", i, hx_name(headerextras[i]->flags), headerextras[i]->len); if(!headerextras.length()) conoutf("no extra header records found"); });

static int findheaderextra ( int type )
{
	loopv(headerextras)
		if ( ( headerextras[i]->flags & HX_TYPEMASK ) == type ) return i;
	return -1;
}
...
static void unpackheaderextra ( uchar *buf, int len )   // break the extra data from the mapheader into its pieces
{
	...
	while ( 1 ) {
		...
		clientlogf(" found headerextra \"%s\", %d bytes%s", hx_name(type), len,
		           flags & HX_FLAG_PERSIST ? "persistent" : "");   // debug info
		headerextras.add(new headerextra(len, flags, p.subbuf(len).buf));
	}
}

static void parseheaderextra ( bool clearnonpersist = true,
                               int ignoretypes = 0 )   // parse all headerextra packets, delete the nonpersistent ones (like editundos)
{
	...
	loopv(headerextras)
	{
		...
		if ( deletethis || ( clearnonpersist
		        && ! ( headerextras[i]->flags & HX_FLAG_PERSIST ) ) )
		    delete headerextras.remove(i--);
	}
}
...
static ucharbuf packheaderextras ( int ignoretypes = 0 )   // serialise all extra data packets to save with the map header
{
	...
	loopv(headerextras)
		if ( ! ( ignoretypes & ( 1 << ( headerextras[i]->flags & HX_TYPEMASK ) ) ) ) {   // copy existing persistent hx packets
...
static bool embedconfigfile ()
{
	...
	if ( buf ) {
		for ( int n; ( n = findheaderextra(HX_CONFIG) ) >= 0; )
			deleteheaderextra(n);
		headerextras.add(
		        new headerextra(len + 1, HX_CONFIG | HX_FLAG_PERSIST, NULL))
		    ->data = buf;
...
COMMANDF(
        clearvantagepoint,
        "",
        () { if(!noteditmode("clearvantagepoint") && !multiplayer() && clearvantagepoint()) conoutf("cleared vantage point"); });
...
static void setvantagepoint ()
{
	...
	loopi(5)
		putint(buf, p[i]);
	headerextras.add(
	        new headerextra(buf.length(), HX_VANTAGEPOINT | HX_FLAG_PERSIST,
	                        buf.getbuf()));
...
void save_world ( char *mname, bool skipoptimise, bool addcomfort )
{
	...
	if ( !validmapname(mname) ) {
		conoutf("\f3Invalid map name. It must only contain letters, digits, '-', '_' and be less than %d characters long",
		        MAXMAPNAMELEN);
		return;
	}
	if ( !skipoptimise ) {
	...
	hdr.numents = 0;
	loopv(ents)
		if ( ents[i].type != NOTUSED ) hdr.numents++;
	...
	loopv(ents)
	{
		if ( ents[i].type != NOTUSED ) {
			...
			if ( oldmapmodelscaling && tmp.type == MAPMODEL )
			    tmp.attr1 = tmp.attr1 / 4;
			lilswap((short *) &tmp, 4);
	...
	delete f;
	unsavededits = 0;
	conoutf("wrote map file %s %s%s", cgzname,
...
COMMANDF(
        savemap,
        "s",
        (char *name) { save_world(name, preserveundosonsave && editmode, preserveundosonsave && editmode); });
COMMANDF(savemapoptimised, "s",
         (char *name) { save_world(name, false, false); });

static void showmapdims ()
{
	conoutf("  min X|Y|Z: %3d : %3d : %3d", mapdims.x1, mapdims.y1,
	        mapdims.minfloor);
	conoutf("  max X|Y|Z: %3d : %3d : %3d", mapdims.x2, mapdims.y2,
	        mapdims.maxceil);
	conoutf("delta X|Y|Z: %3d : %3d : %3d", mapdims.xspan, mapdims.yspan,
	        mapdims.maxceil - mapdims.minfloor);
}
COMMAND(showmapdims, "");
...
{
	const int sizeof_header = sizeof(header), sizeof_baseheader = sizeof_header
	        - sizeof(int) * 16;
	stopwatch watch;
	watch.start();
	...
	if ( f->read(&tmp, sizeof_baseheader) != sizeof_baseheader || ( strncmp(
	        tmp.head, "CUBE", 4)
	                                                                != 0
	                                                                && strncmp(
	                                                                        tmp
	...
	if ( tmp.headersize > sizeof_header ) {
		...
		if ( extrasize ) {   // map file actually has extra header data that we want too preserve
	...
	loopi(hdr.numents)
	{
		...
		if ( e.type == PLAYERSTART && ( e.attr2 == 0 || e.attr2 == 1
		                                || e.attr2 == 100 ) ) {
			if ( e.attr2 == 100 )
				numspawn[2]++;
			else
	...
	if ( !mapinfo.numelems || ( mapinfo.access(mname)
	        && !cmpf(cgzname, mapinfo[mname]) ) ) world = (sqr *) ents.getbuf();
	c2skeepalive();
	...
	watch.start();
	loopi(256)
		if ( texuse[i] ) lookupworldtexture(i, autodownload ? true : false);
	...
	if ( downloaded > 0 )
	    clientlogf("downloaded content (%d KB in %d seconds)",
	               downloaded / 1024, watch.stop() / 1000);
	...
	watch.start();
	loopi(256)
		if ( texuse[i] ) lookupworldtexture(i, false);
...
COMMANDF(
        hexbinchunk,
        "v",
        (char **args, int numargs)   // read up to 24 bytes from the command's arguments to the hexbin buffer
        { if(!hexbinenabled) { conoutf("hexbinchunk out of context"); return; } loopi(numargs) hexbin.add(strtol(args[i], NULL, 16)); });
...
struct xmap
{
	...
	xmap () :
			world(NULL), ssize(0), cubicsize(0), numundo(0)
	{
		*nick = *name = *mcfname = '\0';
	}
	...
	xmap ( const char *nnick )   // take the current map and store it in an xmap
	{
		...
		cubicsize = ::cubicsize;
		loopv(::headerextras)
			headerextras.add(::headerextras[i]->duplicate());
		world = new sqr[cubicsize];
		memcpy(world, ::world, cubicsize * sizeof(sqr));
		loopv(::ents)
			if ( ::ents[i].type != NOTUSED ) ents.add() = ::ents[i];
		...
		headerextras.add(
		        new headerextra(tmp.length(), HX_EDITUNDO, tmp.getbuf()));
	}
	...
	void write ( stream *f )   // write xmap as cubescript to a file
	{
		f->printf("restorexmap version %d %d %d\n", XMAPVERSION,
		          isbigendian() ? 1 : 0, sizeof ( world ));   // it is a non-portable binary file format, so we have to check...
		...
		uLongf worldsize = cubicsize * sizeof(sqr), gzbufsize =
		        ( worldsize * 11 ) / 10;   // gzip the world, so the file will only be big instead of huge
		uchar *gzbuf = new uchar[gzbufsize];
		...
		loopv(ents)   // entities are stored as plain text - you may edit them
		{
			...
			f->printf(
			        "restorexmap ent %d  %d %d %d  %d %d %d %d // %s\n",
			        e.type,
			        e.x,
			        e.y,
			        e.z,
...
static bool validxmapname ( const char *nick )
{
...
void xmapbackup ( const char *nickprefix, const char *nick )   // throw away existing backup and make a new one
{
	...
	bak = new xmap(text);
	if ( unsavededits )
	    conoutf("\f3stored backup of unsaved edits on map '%s'; to restore, type \"/xmap_restore\"",
	            bak->name);
}
...
static const char *xmapdescstring ( xmap *xm, bool shortform = false )
{
	...
	formatstring(s[toggle])(
	        "\"%s\": %s rev %d, %d ents%c size %d, hdrs %d, %d undo steps", xm->nick, xm->name, xm->hdr.maprevision, xm->ents.length(),
	        shortform ? '\0' : ',', xm->hdr.sfactor, xm->headerextras.length() - 1, xm->numundo);
	return s[toggle];
}

static xmap *getxmapbynick ( const char *nick, int *index = NULL, bool errmsg =
        true )
{
...
COMMANDF(
        ...
        { loopv(xmaps) conoutf("xmap %d %s", i, xmapdescstring(xmaps[i])); if(xmaps.length() == 0) conoutf("no xmaps in memory"); if(bak) conoutf("backup stored before %s", xmapdescstring(bak)); });
...
COMMANDF(
        ...
        if(!*nick || !validxmapname(nick)) return; int i; if(getxmapbynick(nick, &i, false)) { xmap *xm = xmaps.remove(i); xmapdelete(xm); }   // overwrite existing same nick
        xmaps.add(new xmap(nick)); unsavededits = 0; conoutf("stored xmap %s", xmapdescstring(xmaps.last())); });

COMMANDF(
        xmap_delete,
        "s",
        (const char *nick)     // delete xmap buffer
        { if(!*nick) return; int i; if(getxmapbynick(nick, &i)) { xmap *xm = xmaps.remove(i); xmapdelete(xm); conoutf("deleted xmap \"%s\"", nick); } });

COMMANDF(xmap_delete_backup, "",
         () { if(bak) { xmapdelete(bak); conoutf("deleted backup xmap"); }});

COMMANDF(
        xmap_keep_backup,
        "s",
        (const char *nick)     // move backup xmap to more permanent position
        { if(!bak) { conoutf("no backup xmap available"); return ; } defformatstring(bakdesc)("%s", bak->nick); if(*nick) { if(validxmapname(nick)) copystring(bak->nick, nick); else return; } loopi(42) if(getxmapbynick(bak->nick, NULL, false)) concatstring(bak->nick, "_");   // evade existing nicknames
        xmaps.add(bak); bak = NULL; conoutf("stored backup xmap \"%s\" as xmap %s", bakdesc, xmapdescstring(xmaps.last())); });

COMMANDF(
        xmap_rename,
        "ss",
        (const char *oldnick, const char *newnick)     // rename xmap buffer
        { if(!*oldnick || !*newnick) return; xmap *xm = getxmapbynick(oldnick); if(!xm || !validxmapname(newnick) || getxmapbynick(newnick, NULL, false)) return; copystring(xm->nick, newnick); });

COMMANDF(
        xmap_restore,
        "s",
        (const char *nick)     // use xmap as current map
        { SPEDIT; if(*nick) { xmap *xm = getxmapbynick(nick); if(!xm) return; xmapbackup("restore_xmap_", nick);   // keep backup, so we can restore from the restore :)
        xm->restore(); unsavededits = 0; conoutf("restored xmap %s", xmapdescstring(xm)); } else if(bak) { bak->restore(); unsavededits = 0; conoutf("restored backup created before %s", xmapdescstring(bak)); } });

static void restorexmap ( char **args, int numargs )   // read an xmap from a cubescript file
{
	const char *cmdnames[] = { "version", "names", "sizes", "header", "world",
	                           "headerextra", "ent", "position" };
	const char cmdnumarg[] = { 3, 2, 3, 0, 0, 1, 8, 5 };

	if ( !xmjigsaw || numargs < 1 ) return;   // { conoutf("restorexmap out of context"); return; }
	bool abort = false;
	int cmd = -1;
	loopi(sizeof(cmdnames)/sizeof(cmdnames[0]))
		if ( !strcmp(cmdnames[i], args[0]) ) {
			cmd = i;
			break;
		}
	...
	{
		case 0:     // version
			if ( ATOI(args[1]) != XMAPVERSION || ATOI(args[2])
			        != ( isbigendian() ? 1 : 0 )
			     || ATOI(args[3]) != sizeof ( world ) ) {
		...
		case 4:     // world
		{
			...
			if ( uncompress((uchar *) xmjigsaw->world, &rawsize,
			                hexbin.getbuf(), hexbin.length())
			     != Z_OK
			     || rawsize - rawworldsize != 0 ) abort = true;
			break;
		}
		case 5:     // headerextra
			xmjigsaw->headerextras.add(
			        new headerextra(hexbin.length(), ATOI(args[1]),
			                        hexbin.getbuf()));
			break;
		case 6:     // ent
		{
		...
		{
			loopi(5)
				xmjigsaw->position[i] = ATOI(args[i + 1]);
			int i;
...
static char *xmapfilename ( const char *nick, const char *prefix = "" )
{
	static defformatstring(fn)
	("%s/%s%s.xmap", xmapspath, prefix, nick);
	return path(fn);
}
...
int loadallxmaps ()     // at game start, load all xmaps from mapediting/xmaps
{
	...
	loopv(xmaps)
		if ( !strncmp(xmaps[i]->nick, bakprefix, strlen(bakprefix)) ) {
			xmapdelete(bak);
			bak = xmaps.remove(i);
