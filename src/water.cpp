...
VARF(waterlevel, -128, -128, 127, if ( !noteditmode("waterlevel") ) {
...
void setwatercolor ( const char *r,
                     const char *g,
                     const char *b,
                     const char *a )
{
...
VERTWC(vertwc,
       { varray::defattrib(varray::ATTRIB_COLOR, 4, GL_UNSIGNED_BYTE); },
       {
	       varray::attrib<uchar>(
	               ...
	               hdr.watercolor[1],
	               hdr.watercolor[2],
	               (uchar ) ( hdr.watercolor[3]
	                       + ( max(v, 0.0f) - 0.5f ) * 51.0f ))
	       ;
       })
VERTWT(vertwt, { varray::defattrib(varray::ATTRIB_TEXCOORD0, 3, GL_FLOAT); }, {
	varray::attrib<float>(v1 + duv, v2 + duv, v3 + h)
	;
})
VERTWT(vertwtc,
       { varray::defattrib(varray::ATTRIB_COLOR, 4, GL_UNSIGNED_BYTE); varray::defattrib(varray::ATTRIB_TEXCOORD0, 3, GL_FLOAT); },
       {
...
VERTWT(vertwmtc,
       { varray::defattrib(varray::ATTRIB_COLOR, 4, GL_UNSIGNED_BYTE); varray::defattrib(varray::ATTRIB_TEXCOORD0, 3, GL_FLOAT); varray::defattrib(varray::ATTRIB_TEXCOORD1, 3, GL_FLOAT); },
       {
	       varray::attrib<uchar>(255, 255, 255, uchar(38 + max(v, 0.0f) * 38))
	       ;
	       varray::attrib<float>(v1 - duv, v2 + duv, v3 + h)
...
static void setupmultitexrefract ( GLuint reflecttex, GLuint refracttex )
{
	...
	colortmu(0, hdr.watercolor[0] / 255.0f, hdr.watercolor[1] / 255.0f,
	         hdr.watercolor[2] / 255.0f, hdr.watercolor[3] / 255.0f);
