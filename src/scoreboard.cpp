...
struct teamscore
{
	...
	teamscore ( int t ) :
			team(t), frags(0), deaths(0), flagscore(0), points(0)
	{
	}
...
void teamflagscores ( int &team1, int &team2 )
{
	...
	loopv(players)
		if ( players[i] && players[i]->team != TEAM_SPECT ) {
			teamscores[team_base(players[i]->team)].addplayer(players[i]);
		}
	loopv(discscores)
		if ( discscores[i].team != TEAM_SPECT ) {
...
{
	loopv(discscores)
		if ( team == team_group(discscores[i].team) ) {
...
static void renderscore ( playerent *d )
{
	...
	const char *ign =
	        d->ignored ? " (ignored)" : ( d->muted ? " (muted)" : "" );
	sline &line = scorelines.add();
	if ( team_isspect(d->team) ) line.textcolor = '4';
...
static int renderteamscore ( teamscore *t )
{
	...
	static color teamcolors[2] = { color(1.0f, 0, 0, 0.2f), color(0, 0, 1.0f,
	                                                              0.2f) };
...
void renderscores ( void *menu, bool init )
{
	...
	int spectators = 0;
	loopv(scores)
		if ( scores[i]->team == TEAM_SPECT ) spectators++;
	loopv(discscores)
		if ( discscores[i].team == TEAM_SPECT ) spectators++;
	...
	if ( m_teammode ) {
		...
		loopv(scores)
			if ( scores[i]->team != TEAM_SPECT )
			    teamscores[team_base(scores[i]->team)].addplayer(scores[i]);
		loopv(discscores)
			if ( discscores[i].team != TEAM_SPECT )
			    teamscores[team_base(discscores[i].team)].addscore(
			            discscores[i]);

		int sort =
		        teamscorecmp(&teamscores[TEAM_CLA], &teamscores[TEAM_RVSF]) < 0 ?
		                0 : 1;
		loopi(2)
		{
		...
		winner =
		        m_flags ?
		                ( teamscores[sort].flagscore > teamscores[team_opposite(
		                        sort)].flagscore ? sort : -1 ) :
		                ( teamscores[sort].frags > teamscores[team_opposite(
		                        sort)].frags ? sort : -1 );

	}
	else {   // ffa mode
		loopv(scores)
			if ( scores[i]->team != TEAM_SPECT ) renderscore(scores[i]);
		loopi(2)
			renderdiscscores(i);
		if ( scores.length() > 0 ) {
			winner = scores[0]->clientnum;
			if ( scores.length() > 1 && ( ( m_flags
			        && scores[0]->flagscore == scores[1]->flagscore )
			                              || ( !m_flags && scores[0]->frags
			                                      == scores[1]->frags ) ) )
			    winner = -1;
		}
	}
	...
	if ( getclientmap()[0] ) {
		...
		formatstring(modeline)(
		        "\"%s\" on map %s", modestr(gamemode, modeacronyms > 0), fldrprefix ? getclientmap()+strlen("maps/") : getclientmap());
	}
	...
	if ( ( gamemode > 1 || ( gamemode == 0
	        && ( multiplayer(false) || watchingdemo ) ) )
	     && minutesremaining >= 0 ) {
		if ( !minutesremaining ) {
			...
			else   // Add the winning player
			{
				if ( winner < 0 )
					...
					concatformatstring(modeline, ", \f1%s wins!",
					                   scores[0]->name);
			}
		}
		else {
			...
			if ( !clockcount )
				...
				concatformatstring(modeline, " / %d:00",
				                   gametimemaximum / 60000);
		}
	}
	...
	if ( multiplayer(false) ) {
		...
		if ( s ) {
			if ( servstate.mastermode > MM_OPEN )
			    concatformatstring(
			            serverline,
			            servstate.mastermode == MM_MATCH ? "M%d " : "P ",
			            servstate.matchteamsize);
	...
	menureset(menu);
	loopv(scorelines)
		menumanual(menu, scorelines[i].getcols(), NULL, scorelines[i].bgcolor);
...
static inline void addstr ( char *dest, const char *src )
{
	if ( strlen(dest) + strlen(src) < MAXJPGCOM ) strcat(dest, src);
}
...
const char *asciiscores ( bool destjpg )
{
	...
	if ( !watchingdemo ) scores.add(player1);
	loopv(players)
		if ( players[i] ) scores.add(players[i]);
	scores.sort(scorecmp);
...
//         const char *sr = scoreratio(d->frags, d->deaths);
		formatstring(team)(
		        destjpg ? ", %s" : " %-4s", team_string(d->team, true));
	...
	if ( m_teammode ) {
		...
		loopv(scores)
		{
			if ( !i || !scorecmp(&scores[i], &scores[i - 1]) )
				concatformatstring(winners, "%s%d", i ? " " : "",
				                   scores[i]->clientnum);
			else
				break;
		}
