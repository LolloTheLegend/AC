...
static VARP(nosway, 0, 0, 1);
static VARP(swayspeeddiv, 1, 105, 1000);
static VARP(swaymovediv, 1, 200, 1000);
static VARP(swayupspeeddiv, 1, 105, 1000);
static VARP(swayupmovediv, 1, 200, 1000);
...
struct weaponmove
{
	...
	weaponmove () :
			k_rot(0), kick(0), anim(0), basetime(0)
	{
	...
	void calcmove ( vec aimdir, int lastaction, playerent *p )
	{
		...
		if ( p->onfloor || p->onladder || p->inwater )
		    swaymillis += lastmillis - lastsway;
		lastsway = lastmillis;
		...
		if ( p->weaponchanging ) {
			...
			float progress =
			        clamp( ( lastmillis - p->weaponchanging ) / (float )weapon::weaponchangetime,
			              0.0f, 1.0f);
			...
			float reloadtime = (float) p->weaponsel->info.reloadtime,
			        progress =
			                clamp( ( lastmillis - p->weaponsel->reloading ) / reloadtime,
			                      0.0f,
			                      clamp(1.0f - (p->lastaction + p->weaponsel->gunwait - lastmillis)/reloadtime, 0.5f, 1.0f));
			k_rot = -90 * sin(progress * M_PI);
		}
		else {
			anim = ANIM_GUN_IDLE;
			...
			int timediff = lastmillis - lastaction, animtime = min(
			        p->weaponsel->gunwait,
			        (int) p->weaponsel->info.attackdelay);
			vec sway = aimdir;
			float progress = 0.0f;
			float k_back = 0.0f;
			...
			if ( p->weaponsel->info.mdl_kick_rot || p->weaponsel->info
			         .mdl_kick_back ) {
			...
			if ( nosway )
				...
				float swayspeed = sin((float) swaymillis / swayspeeddiv)
				        / ( swaymovediv / 10.0f );
				float swayupspeed = cos((float) swaymillis / swayupspeeddiv)
				        / ( swayupmovediv / 10.0f );

				float plspeed = min(
				        1.0, sqrt(p->vel.x * p->vel.x + p->vel.y * p->vel.y));
...
static void updatelastaction ( playerent *d, int millis = lastmillis )
{
	loopi(NUMGUNS)
		d->weapons[i]->updatetimers(millis);
	d->lastaction = millis;
}

static void checkweaponswitch ()
{
...
static void selectweapon ( weapon *w )
{
...
static void requestweapon ( int *w )
{
...
static void quicknadethrow ( bool on )
{
...
inline static void currentprimary ()
{
	intret(player1->primweap->type);
}
inline static void prevweapon ()
{
	intret(player1->prevweaponsel->type);
}
inline static void curweapon ()
{
	intret(player1->weaponsel->type);
}

inline static void magcontent ( int *w )
{
	if ( *w >= 0 && *w < NUMGUNS )
...
inline static void magreserve ( int *w )
{
	if ( *w >= 0 && *w < NUMGUNS )
...
void tryreload ( playerent *p )
{
	if ( !p || p->state != CS_ALIVE || p->weaponsel->reloading
	     || p->weaponchanging ) return;
	p->weaponsel->reload(false);
}

static inline void selfreload ()
{
	tryreload(player1);
}
COMMANDN(reload, selfreload, "");

static int lastsgs_hits = 0;
static int lastsgs_dmgt = 0;
...
void createrays ( vec &from, vec &to )   // create random spread of rays for the shotgun
{
	...
	loopk(3)
	{
		...
		loopi(SGRAYS)
		{
			...
			vec rray = vec(to);
			float nvl = veclen
			        / ( ( SGCObase / 100.0f ) + ( SGCOrange / 100.0f ) );
			vec pbv = vec(nvl, 0, 0);
...
static inline bool intersectsphere ( const vec &from,
                                     const vec &to,
                                     vec center,
                                     float radius,
                                     float &dist )
...
static inline bool intersectcylinder ( const vec &from,
                                       const vec &to,
                                       const vec &start,
                                       const vec &end,
                                       float radius,
                                       float &dist )
{
	...
	float nn = n.squaredlen(), mn = m.dot(n), a = dd * nn - nd * nd, k =
	        m.squaredlen() - radius * radius, c = dd * k - md * md;
	if ( fabs(a) < 0.005f ) {
		if ( c > 0 ) return false;
		if ( md < 0 )
			dist = -mn / nn;
...
static playerent *intersectclosest ( const vec &from,
                                     const vec &to,
                                     playerent *at,
                                     float &bestdist,
                                     int &hitzone,
                                     bool aiming = true )
{
	...
	loopv(players)
	{
		...
		if ( !o || o == at
		     || ( o->state != CS_ALIVE && ( aiming
		             || ( o->state != CS_EDITING && o->state != CS_LAGGED ) ) ) )
		    continue;
...
{
	if ( camera1->type == ENT_PLAYER || ( camera1->type == ENT_CAMERA
...
static hitweap accuracym[NUMGUNS];

static inline void attackevent ( playerent *owner, int weapon )
{
...
static void hitpush ( int damage,
                      playerent *d,
                      playerent *at,
                      vec &from,
                      vec &to,
...
static float expdist ( playerent *o, vec &dir, const vec &v )
{
	vec middle = o->o;
...
static void radialeffect ( playerent *o,
                           vec &v,
                           int qdam,
                           playerent *at,
                           int gun )
...
void movebounceents ( void )
{
	loopv(bounceents)
		if ( bounceents[i] ) {
			...
			if ( ( p->bouncetype == BT_NADE || p->bouncetype == BT_GIB ) && p
			         ->applyphysics() ) movebounceent(p, 1, false);
...
void renderbounceents ( void )
{
	loopv(bounceents)
	{
		...
		switch ( p->bouncetype )
		{
			...
			case BT_GIB:
			default: {
				uint n = ( ( ( 4 * (uint) (size_t) p ) + (uint) p->timetolive )
				        % 3 )
				         + 1;
		...
		path(model);
		rendermodel(model, anim | ANIM_LOOP | ANIM_DYNALLOC, 0, 1.1f, o,
		            p->yaw + 90, p->pitch, 0, basetime);
	}
}

static VARP(gib, 0, 1, 1);
static VARP(gibnum, 0, 6, 1000);
static VARP(gibttl, 0, 7000, 60000);
static VARP(gibspeed, 1, 30, 100);
...
static void shorten ( vec &from, vec &target, float dist )
{
	target.sub(from).mul(min(1.0f, dist)).add(from);
}

static void raydamage ( vec &from, vec &to, playerent *d )
{
	...
	if ( d->weaponsel->type == GUN_SHOTGUN ) {
		...
		loopk(3)
			loopi(SGRAYS)
			{
				rayscount++;
				int h = k * SGRAYS + i;
				if ( ( hits[h] = intersectclosest(from, sgr[h].rv, d, dist,
				                                  hitzone) ) )
		...
		loopk(3)
			loopi(SGRAYS)
			{
				...
				if ( hits[h] ) {
					...
					dmg4r +=
					        /*(int)*/( ( SGCOdmg / 10.0f * SGDMGTOTAL / 100.0f )
					                * numhits_o
					                   / 21.0f );
					dmg4r +=
					        /*(int)*/( ( SGCMdmg / 10.0f * SGDMGTOTAL / 100.0f )
					                * numhits_m
					                   / 21.0f );
					dmg4r +=
					        /*(int)*/( ( SGCCdmg / 10.0f * SGDMGTOTAL / 100.0f )
					                * numhits_c
					                   / 21.0f );
					dmgreal = (int) ceil(dmg4r);
					lastsgs_hits = numhits;
					lastsgs_dmgt = dmgreal;
					int info = ( withBONUS ? SGDMGBONUS : 0 )
					        | ( numhits_c << 8 ) | ( numhits_m << 16 )
					        | ( numhits_o << 24 );
					if ( numhits )
					    hitpush(dmgreal, o, d, from, to, d->weaponsel->type,
					            lastsgs_hits == SGRAYS * 3,
					            isbigendian() ? endianswap(info) : info);
...
static inline const char *weapstr ( unsigned int i )
{
	switch ( i )
	{
		case GUN_AKIMBO:
...
void r_accuracy ( int h )
{
	...
	vector<char*> lines;
	int rows = 0, cols = 0;
	float spacing = curfont->defaultw * 2, x_offset = curfont->defaultw,
	        y_offset = float(2 * h) - 2 * spacing;

	loopi(NUMGUNS)
		if ( i != GUN_CPISTOL && accuracym[i].shots ) {
			float acc = 100.0f * accuracym[i].hits / (float) accuracym[i].shots;
			string line;
			rows++;
			if ( i == GUN_GRENADE || i == GUN_SHOTGUN ) {
				formatstring(line)(
				        "\f5%5.1f%s (%.1f/%d) :\f0%s", acc, "%", accuracym[i].hits, (int)accuracym[i].shots, weapstr(i));
			}
			else
			{
	...
	if ( rows < 1 ) return;
	cols++;
	blendbox(x_offset, spacing + y_offset,
	         spacing + x_offset + curfont->defaultw * cols,
	         y_offset - curfont->defaulth * rows, true, -1);
	int x = 0;
	loopv(lines)
	{
		char *line = lines[i];
		draw_textf(line, spacing * 0.5 + x_offset,
		           y_offset - x * curfont->defaulth - 0.5 * spacing);
		x++;
	}
}

static void accuracyreset ()
{
	loopi(NUMGUNS)
	{
...
weapon::weapon ( class playerent *owner, int type ) :
		type(type), owner(owner), info(guns[type]), ammo(owner->ammo[type]),
		        mag(owner->mag[type]), gunwait(owner->gunwait[type]),
		        reloading(0)
{
}
...
inline int weapon::flashtime () const
{
	return max((int) info.attackdelay, 120) / 4;
}
...
void weapon::sendshoot ( vec &from, vec &to, int millis )
{
	...
	addmsg(SV_SHOOT, "ri2i3iv", millis, owner->weaponsel->type,
	       ...
	       hits.length(), hits.length() * sizeof(hitmsg) / sizeof(int),
	       hits.getbuf());
...
static VARP(oldfashionedgunstats, 0, 0, 1);
...
#undef RNDD
	...
	if ( recoiltest ) {
		owner->vel.add(
		        vec(unitv).mul(recoil / dist).mul(
		                owner->crouching ? 0.75 : 1.0f));
		owner->pitchvel = min(
		        pow(shots / recoilincrease, 2.0) + recoilbase / 10.0,
		        maxrecoil / 10.0);
	}
	else {
		owner->vel.add(
		        vec(unitv).mul(recoil / dist).mul(
		                owner->crouching ? 0.75 : 1.0f));
		owner->pitchvel = min(
		        pow(shots / g.recoilincrease, 2.0) + g.recoilbase / 10.0,
		        g.maxrecoil / 10.0);
	}
}

static VARP(righthanded, 0, 1, 1);   // flowtron 20090727
...
void weapon::renderhudmodel ( int lastaction, int index )
{
	...
	defformatstring(path)("weapons/%s",
	                      identexists(widn) ? getalias(widn) : info.modelname);
	bool emit = ( wm.anim & ANIM_INDEX ) == ANIM_GUN_SHOOT
	        && ( lastmillis - lastaction ) < flashtime();
	rendermodel(
	        path,
	        wm.anim | ANIM_DYNALLOC | ( righthanded == index ? ANIM_MIRROR : 0 )
	        | ( emit ? ANIM_PARTICLE : 0 ),
	        0, -1, wm.pos, p->yaw + 90, p->pitch + wm.k_rot, 40.0f, wm.basetime,
	        NULL, NULL, 1.28f);
}
...
inline void weapon::renderhudmodel ()
{
	renderhudmodel(owner->lastaction);
}
inline void weapon::renderaimhelp ( bool teamwarning )
{
	drawcrosshair(
	        owner,
	        teamwarning ? CROSSHAIR_TEAMMATE : owner->weaponsel->type + 3);
}
inline int weapon::dynspread ()
{
	return info.spread;
}
inline float weapon::dynrecoil ()
{
	return info.recoil;
}
inline bool weapon::selectable ()
{
	return this != owner->weaponsel && owner->state == CS_ALIVE
	       && !owner->weaponchanging;
}
inline bool weapon::deselectable ()
{
	return !reloading;
}
...
bool weapon::valid ( int id )
{
	return id >= 0 && id < NUMGUNS;
}
...
enum
{
	NS_NONE, NS_ACTIVATED = 0, NS_THROWN, NS_EXPLODED
};
...
void grenadeent::explode ()
{
	...
	splash();
	if ( local )
	    addmsg(SV_EXPLODE, "ri3iv", lastmillis, GUN_GRENADE, millis,
	           hits.length(), hits.length() * sizeof(hitmsg) / sizeof(int),
	           hits.getbuf());
...
void grenadeent::_throw ( const vec &from, const vec &vel )
{
	...
	if ( local ) {
		addmsg(SV_THROWNADE, "ri7", int(o.x * DMF), int(o.y * DMF),
		       int(o.z * DMF), int(vel.x * DMF), int(vel.y * DMF),
		       int(vel.z * DMF), lastmillis - millis);
...
void grenadeent::moveoutsidebbox ( const vec &direction,
                                   playerent *boundingbox )
{
	vel = direction;
	o = boundingbox->o;
	inwater = hdr.waterlevel > o.z;
...
bool grenadeent::applyphysics ()
{
	return nadestate == NS_THROWN;
}
...
grenades::grenades ( playerent *owner ) :
		weapon(owner, GUN_GRENADE), inhandnade(NULL),
		        throwwait( ( 13 * 1000 ) / 40), throwmillis(0), state(GST_NONE)
{
}

inline int grenades::flashtime () const
{
	return 0;
}

inline bool grenades::busy ()
{
	return state != GST_NONE;
}
...
bool grenades::attack ( vec &targ )
{
	...
	bool quickwait = attackmillis * 3 >= gunwait
	        && ! ( m_arena && m_teammode && arenaintermission );
		...
		case GST_THROWING:
			if ( attackmillis >= throwwait )   // throw done
			{
				...
				if ( !mag && this == owner->weaponsel )   // switch to primary immediately
				        {
					owner->weaponchanging = lastmillis - 1
					                        - ( weaponchangetime / 2 );
...
void grenades::thrownade ()
{
	...
	vec vel(sin(RAD * owner->yaw) * speed, -cos(RAD * owner->yaw) * speed,
...
bool gun::attack ( vec &targ )
{
	...
	if ( burstshotssettings[this->type] > 0 && shots
	        >= burstshotssettings[this->type] ) owner->attacking = false;
...
shotgun::shotgun ( playerent *owner ) :
		gun(owner, GUN_SHOTGUN)
{
}
...
{
	loopi(SGRAYS)
		particle_splash(PART_SPARK, 5, 200, sgr[i].rv);

	if ( addbullethole(owner, from, to) )
	    loopk(3)
		    loopi(3)
			    addbullethole(owner, from, sgr[k * SGRAYS + i * SGRAYS / 3].rv,
			                  0, false);
	adddynlight(owner, from, 4, 100, 50, 96, 80, 64);
	attacksound();
}
...
subgun::subgun ( playerent *owner ) :
		gun(owner, GUN_SUBGUN)
{
}
...
{
	return shots > 2 ?
	        70 : ( info.spread + ( shots > 0 ? ( shots == 1 ? 5 : 10 ) : 0 ) );
...
int sniperrifle::dynspread ()
{
	if ( scoped ) {
		...
		if ( scopetime > SCOPESETTLETIME )
			...
			return max(
			        ( info.spread * ( SCOPESETTLETIME - scopetime ) ) / SCOPESETTLETIME,
			        1);
	}
	return info.spread;
}
...
{
	return scoped && lastmillis - scoped_since > SCOPESETTLETIME ?
	        info.recoil / 3 : info.recoil;
}
...
carbine::carbine ( playerent *owner ) :
		gun(owner, GUN_CARBINE)
{
}
...
assaultrifle::assaultrifle ( playerent *owner ) :
		gun(owner, GUN_ASSAULT)
{
}
...
{
	return shots > 2 ?
	        55 : ( info.spread + ( shots > 0 ? ( shots == 1 ? 5 : 15 ) : 0 ) );
}
float assaultrifle::dynrecoil ()
{
	return info.recoil + ( rnd(8) * -0.01f );
}
...
cpistol::cpistol ( playerent *owner ) :
		gun(owner, GUN_CPISTOL), bursting(false)
{
}
...
pistol::pistol ( playerent *owner ) :
		gun(owner, GUN_PISTOL)
{
}
bool pistol::selectable ()
{
	return weapon::selectable() && !m_nopistol;
}
...
bool akimbo::timerout ()
{
	return akimbomillis && akimbomillis <= lastmillis;
}

// knife

knife::knife ( playerent *owner ) :
		weapon(owner, GUN_KNIFE)
{
}

int knife::flashtime () const
{
	return 0;
}
