...
static void extrudeshadowtiles ( int x1,
                                 int y1,
                                 int x2,
                                 int y2,
                                 int x3,
...
static void addshadowtiles ( float x1, float y1, float x2, float y2 )
{
	...
	int tx1 = clamp(int(floor((y1 + 1)/2 * SHADOWCOLUMNS)), 0,
	                SHADOWCOLUMNS - 1), ty1 = clamp(
	        int(floor((x1 + 1)/2 * SHADOWROWS)), 0, SHADOWROWS - 1), tx2 =
                    ...
                    const glmatrixf &mat )
{
	...
	loopi(8)
	{
		vec4 &p = v[i];
		mat.transform(
		        vec(i & 1 ? bbmax.x : bbmin.x, i & 2 ? bbmax.y : bbmin.y,
		            i & 4 ? bbmax.z : bbmin.z),
		        p);
		if ( p.z >= -p.w ) {
	...
	if ( front < 8 || ev.z < -ev.w ) loopi(8)
	{
		...
		if ( p.z >= -p.w ) {
			...
			float t = ev.z / ( ev.z - p.z ), w = ev.w + t * ( p.w - ev.w ), x =( ev.x + t*(p.x - ev.x))/w,
	y = (ev.y + t*(p.y - ev.y))/w;
			sx1 = min(sx1, x);
			sy1 = min(sy1, y);
			sx2 = max(sx2, x);
		...
		loopj(3)
		{
			...
			if ( o.z < -o.w ) continue;
			float t = p.z / ( p.z - o.z ), w = p.w + t * ( o.w - p.w ), x =
			        ( p.x + t * ( o.x - p.x ) ) / w, y = ( p.y
			        + t * ( o.y - p.y ) )
			                                             / w;
			sx1 = min(sx1, x);
			sy1 = min(sy1, y);
		...
		if ( ev.z < -ev.w ) continue;
		float t = p.z / ( p.z - ev.z ), w = p.w + t * ( ev.w - p.w ), x = ( p.x
		        + t * ( ev.x - p.x ) )
		                                                                  / w,
		        y = ( p.y + t * ( ev.y - p.y ) ) / w;
		sx1 = min(sx1, x);
	...
	if ( ev.z >= -ev.w ) {
		...
		if ( ( sx1 >= 1 && x >= 1 ) || ( sy1 >= 1 && y >= 1 )
		     || ( sx2 <= -1 && x <= -1 ) || ( sy2 <= -1 && y <= -1 ) )
		    return false;
		int tx = int(floor(SHADOWCOLUMNS * ( y + 1 ) / 2)), ty = int(
		        floor(SHADOWROWS * ( x + 1 ) / 2)), tx1 = int(
		        floor(SHADOWCOLUMNS * ( sy1 + 1 ) / 2)), ty1 = int(
		        floor(SHADOWROWS * ( sx1 + 1 ) / 2)), tx2 = int(
		        floor(SHADOWCOLUMNS * ( sy2 + 1 ) / 2)), ty2 = int(
		        floor(SHADOWROWS * ( sx2 + 1 ) / 2));
		if ( tx < tx1 ) {
			if ( ty < ty1 ) {
				swap(ty1, ty2);
				tx1--;
...
static void rendershadowtiles ()
{
	...
	float clipx1 = ( shadowy1 + 1 ) / 2, clipy1 = ( shadowx1 + 1 ) / 2, clipx2 =
	        ( shadowy2 + 1 ) / 2, clipy2 = ( shadowx2 + 1 ) / 2;
	if ( !shadowclip ) {
		clipx1 = clipy1 = 0;
		clipx2 = clipy2 = 1;
	}
	...
	loop(y, SHADOWROWS+1)
	{
		...
		while ( mask ) {
			...
			float vx = xstart * tw, vy = y * th, vw = ( x - xstart ) * tw, vh =
			        ( yend - y ) * th, vx1 = max(vx, clipx1), vy1 = max(vy,
			                                                            clipy1),
...
void drawstencilshadows ()
{
	...
	if ( shadowcasters ) {
		...
		glStencilFunc(GL_NOTEQUAL, ( hasST2 || hasSTS ) && !hasSTW ? 128 : 0,
		              ~0U);
