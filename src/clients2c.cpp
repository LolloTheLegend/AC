...
static bool changemapserv ( char *name, int mode, int download, int revision )   // forced map change from the server
{
	...
	else if ( player1->state == CS_EDITING ) { /*conoutf("SANITY drop from EDITING");*/
		toggleedit(true);
	...
	if ( download > 0 ) {
		...
		if ( watchingdemo ) {
			if ( loaded && !revmatch )
			    conoutf(_(
			            "%c3demo was recorded on map revision %d, you have map revision %d"),
			            CC, revision, hdr.maprevision);
		}
		else {
			...
			bool getnewrev = autogetnewmaprevisions
			...
			if ( autogetmap || getnewrev ) {
				if ( !loaded || getnewrev )
					...
					defformatstring(msg)(
					        "map '%s' revision: local %d, provided by server %d",
				...
				if ( !loaded || download < 10 )
					conoutf(_(
					        "\"getmap\" to download the current map from the server"));
				else
					conoutf(_(
					        "\"getmap\" to download a %s version of the current map from the server"),
					        revision == 0 ?
					                _("different") :
					                ( revision > hdr.maprevision ? _("newer") :
					                                               _("older") ));
			}
		}
	}
		...
		switch ( type = getint(p) )
		{
			...
			case SV_POSC: {
				...
				if ( type == SV_POSC ) {
					...
					roll = !q.getbits(1) ? ( q.getbits(6) - 32 ) * 20.0f / 31 :
					                       0.0f;
					if ( !q.getbits(1) ) {
				...
				if ( player1->isspectating() && player1->spectatemode
				        == SM_NONE ) togglespect();
				clamproll((physent *) d);
				break;
			}
...
#define DIST(x) (f0.pos.x - f1.pos.x)
		F2F = ( !numflagspawn[0] || !numflagspawn[1] ) ?
		        1000 * MINFF : DIST(x) * DIST(x) + DIST(y) * DIST(y);
#undef DIST
	}
...
#undef LOGTHISSWITCH
	...
	map_quality =
	        ( !item_fail && F2F > MINFF && MA < MAXMAREA && Mh < MAXMHEIGHT
	          && Hhits < MAXHHITS ) ? MAP_IS_GOOD : MAP_IS_BAD;
	if ( ( !connected || gamemode == GMODE_COOPEDIT ) && map_quality
	        == MAP_IS_BAD ) map_quality = MAP_IS_EDITABLE;
...
static void onCallVote ( int type, int vcn, char *text, char *a )
{
	if ( identexists("onCallVote") ) {
		defformatstring(runas)("%s %d %d [%s] [%s]", "onCallVote", type, vcn,
		                       text, a);
		execute(runas);
	}
}
...
static void parsemessages ( int cn,
                            playerent *d,
                            ucharbuf &p,
                            bool demo = false )
{
	...
	while ( p.remaining() ) {
		...
		switch ( type )
		{
			case SV_SERVINFO:   // welcome message from the server
			{
				...
				if ( prot != CUR_PROTOCOL_VERSION && ! ( watchingdemo
				        && prot == -PROTOCOL_VERSION ) ) {
					conoutf(_(
					        "%c3incompatible game protocol (local protocol: %d :: server protocol: %d)"),
					        CC, CUR_PROTOCOL_VERSION, prot);
			...
			case SV_VOICECOMTEAM: {
				...
				if ( !d || ! ( d->muted || d->ignored ) ) {
					if ( voicecomsounds == 1 || ( voicecomsounds == 2
					        && m_teammode ) ) audiomgr.playsound(t, SP_HIGH);
				}
				break;
			}
			case SV_VOICECOM: {
			...
			case SV_TEAMTEXT: {
				...
				if ( d->ignored )
					...
					if ( m_teammode )
						conoutf(type == SV_TEAMTEXTME ? "\f1%s %s" :
						                                "%s:\f1 %s",
						        colorname(d), highlight(text));
					else
						conoutf(type == SV_TEAMTEXTME ? "\f0%s %s" :
						                                "%s:\f0 %s",
						        colorname(d), highlight(text));
				}
				break;
			...
			case SV_TEXT:
				if ( cn == -1 ) {
					...
					if ( d->ignored && d->clientrole != CR_ADMIN )
						...
						conoutf(type == SV_TEXTME ? "\f0%s %s" : "%s:\f0 %s",
						        colorname(d), highlight(text));
				}
				else
					return;
			...
			case SV_TEXTPRIVATE: {
				...
				if ( d->ignored )
					...
					lastpm = d->clientnum;
					if ( identexists("onPM") ) {
						defformatstring(onpm)("onPM %d [%s]", d->clientnum,
						                      text);
						execute(onpm);
					}
				}
			...
			case SV_MAPCHANGE: {
				...
				localwrongmap = !changemapserv(text, mode, downloadable,
				                               revision);
			...
			case SV_SWITCHNAME:
				...
				if ( d ) {
					if ( strcmp(d->name, text) )
					    conoutf(_("%s is now known as %s"), colorname(d),
					            colorname(d, text));
			...
			case SV_CDIS: {
				...
				if ( identexists("onDisconnect") ) {
					defformatstring(ondisconnect)("onDisconnect %d",
					                              d->clientnum);
			...
			case SV_SPAWNSTATE: {
				if ( map_quality == MAP_IS_BAD ) {
					loopi(6+2*NUMGUNS)
						getint(p);
					conoutf(_(
					        "map deemed unplayable - fix it before you can spawn"));
					break;
				}
				...
				loopi(NUMGUNS)
					player1->ammo[i] = getint(p);
				loopi(NUMGUNS)
					player1->mag[i] = getint(p);
			...
			case SV_SHOTFX: {
				...
				if ( s->weapons[gun] ) {
					...
					s->weapons[gun]->gunwait =
					        s->weapons[gun]->info.attackdelay;
			...
			case SV_GIBDAMAGE:
			case SV_DAMAGE: {
				int tcn = getint(p), acn = getint(p), gun = getint(p), damage =
				        getint(p), armour = getint(p), health = getint(p);
				playerent *target = getclient(tcn), *actor = getclient(acn);
				if ( !target || !actor ) break;
				target->armour = armour;
				target->health = health;
				dodamage(damage, target, actor, -1, type == SV_GIBDAMAGE,
				         false);
...
//                         medals_arrived=1;
			...
			case SV_GIBDIED:
			case SV_DIED: {
				int vcn = getint(p), acn = getint(p), frags = getint(p), gun =
				        getint(p);
			...
			case SV_RESUME: {
				loopi(MAXCLIENTS)
				{
					...
					int state = getint(p), lifesequence = getint(p), primary =
					        getint(p), gunselect = getint(p), flagscore =
					        getint(p), frags = getint(p), deaths = getint(p),
					        health = getint(p), armour = getint(p), points =
					                getint(p);
					int teamkills = 0;
					if ( !demo || !watchingdemo || demoprotocol > 1132 )
					    teamkills = getint(p);
					...
					loopi(NUMGUNS)
						mag[i] = getint(p);
					playerent *d = (
					        cn == getclientnum() ? player1 : newclient(cn) );
					if ( !d ) continue;
			...
			case SV_NEWMAP: {
				...
				if ( size >= 0 )
					...
					empty_world(-1, true);
				if ( d && d != player1 )
				    conoutf(size >= 0 ? _("%s started a new map of size %d") :
				                        _("%s enlarged the map to size %d"),
				            colorname(d), sfactor);
				break;
			}
			...
			case SV_EDITENT:            // coop edit of ent
			{
				...
				if ( ents[i].type == SOUND ) {
					...
					location *loc = audiomgr.locations.find(e.attr1, &entref,
					                                        mapsounds);
			...
			case SV_PONG: {
				int millis = getint(p);
				addmsg(SV_CLIENTPING,
				       "i",
				       player1->ping =
				               max(0,
				                   ( player1->ping * 5 + totalmillis - millis ) / 6));
				break;
			}
			...
			case SV_ARENAWIN: {
				...
				if ( m_botmode && acn == -2 )
					...
					hudoutf(_("team %s has won the round!"),
					        team_string(alive->team));
			...
			case SV_TEAMDENY: {
				...
				if ( m_teammode ) {
					if ( team_isvalid(t) )
					    conoutf(_("you can't change to team %s"),
					            team_string(t));
				}
				else {
					if ( team_isspect(t) )
						conoutf(_("you can't change to spectate mode"));
					else if ( player1->state != CS_ALIVE )
						conoutf(_("you can't change to active mode"));
					else
						conoutf(_("you can't switch teams while being alive"));
				}
				break;
			}
			...
			case SV_SETTEAM: {
				...
				fnt &= 0x0f;
				playerent *d = (
				        fpl == getclientnum() ? player1 : newclient(fpl) );
				if ( d ) {
					...
					if ( m_teammode || team_isspect(fnt) ) {
						if ( d->team == fnt ) {
							...
							if ( you && !watchingdemo ) {
								...
								{
									case FTR_PLAYERWISH:
										conoutf(_("you're now in team %s"),
										        nts);
										break;
									case FTR_AUTOTEAM:
										hudoutf(_(
										        "the server forced you to team %s"),
										        nts);
										break;
								}
							}
								...
								bool et = team_base(
								        player1->team) != team_base(fnt);
								switch ( ftr )
								{
									case FTR_PLAYERWISH:
										conoutf(_(
										        "player %s switched to team %s"),
										        pls, nts);   // new message
										break;
									case FTR_AUTOTEAM:
										if ( watchingdemo || team_isspect(
										        player1->team) )
											conoutf(_(
											        "the server forced %s to team %s"),
											        colorname(d), nts);
										else
											hudoutf(_(
											        "the server forced %s to %s team"),
											        colorname(d),
											        et ? _("the enemy") :
											             _("your"));
										break;
							...
							if ( you && !team_isspect(d->team)
							     && team_isspect(fnt) && d->state == CS_DEAD )
							    spectatemode(SM_FLY);
						}
					}
					else if ( d->team
					        != fnt&& ftr == FTR_PLAYERWISH && !team_isactive(d->team) )
					    conoutf(_("%s changed to active play"),
					            you ? _("you") : colorname(d));
					d->team = fnt;
...
static const char *parseDemoFilename ( char *srvfinfo )
{
	...
	extern const char *getDemoFilename ( int gmode,
	                                     int mplay,
	                                     int mdrop,
	                                     int tstamp,
	                                     char *srvmap );
...
static void receivefile ( uchar *data, int len )
{
	...
	switch ( type )
	{
		...
		case SV_RECVMAP: {
			...
			if ( MAXMAPSENDSIZE < mapsize + cfgsizegz || cfgsize
			        > MAXCFGFILESIZE ) {   // sam's suggestion
