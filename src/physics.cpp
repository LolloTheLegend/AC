                        ...
                        float &lo )          // collide with physent
{
	...
	if ( ( d->type == ENT_PLAYER && o->type == ENT_PLAYER ?
	        dr.sqrxy() < r * r : fabs(dr.x) < r && fabs(dr.y) < r )
	     && dr.dotxy(d->vel) >= 0.0f ) {
...
static bool cornertest ( int mip,
                         int x,
                         int y,
                         int dx,
                         int dy,
                         ...
                         int &bs )   // recursively collide with a mipmapped corner cube
{
	...
	bool stest = SOLID(
	        SWS(w, x+dx, y, mfactor)) && SOLID(SWS(w, x, y+dy, mfactor));
	mip++;
	x /= 2;
	y /= 2;
	...
	loopv(ents)
	{
		...
		if ( e.type == CLIP || ( e.type == PLCLIP
		        && ( d->type == ENT_BOT || d->type == ENT_PLAYER
		             || ( d->type == ENT_BOUNCE && ( (bounceent *) d )
		                      ->plclipped ) ) ) )   // don't allow bots to hack themselves into plclips - Bukz 2011/04/14
		{
			if ( fabs(e.x - d->o.x) < e.attr2 + d->radius && fabs(e.y - d->o.y)
			        < e.attr3 + d->radius ) {
				const float cz = float(S(e.x, e.y)->floor + e.attr1), ch =
				        float(e.attr4);
			...
			if ( fabs(e.x - d->o.x) < r && fabs(e.y - d->o.y) < r ) {
				const float mmz = float(
				        S(e.x, e.y)->floor + mmi.zoff + e.attr3);
...
bool collide ( physent *d, bool spawn, float drop, float rise, int level )   // levels 1 = map, 2 = players, 4 = models, default: 1+2+4
{
	...
	if ( level & 1 ) for ( int y = y1; y <= y2; y++ )
	...
	if ( level & 2 && d->type != ENT_CAMERA ) {
		loopv(players)       // collide with other players
		{
			...
			if ( !o || o == d || ( o == player1 && d->type == ENT_CAMERA ) )
			    continue;
...
static void resizephysent ( physent *pl,
                            int moveres,
                            int curtime,
                            float min,
                            float max )
...
void moveplayer ( physent *pl, int moveres, bool local, int curtime )
{
	...
	const bool specfly = pl->type == ENT_PLAYER
	...
	if ( pl->type == ENT_BOUNCE ) {
		...
		const float speed = curtime * pl->maxspeed
		        / ( water ? 2000.0f : 1000.0f );
		...
		else   // apply gravity
		{
			...
			const float GRAVITY = BOUNCE_MASS * 9.81f / CUBES_PER_METER
			                      / 1000.0f;
	...
	else   // fake physics for player ents to create _the_ cube movement (tm)
	{
		...
		int move =
		        pl->onladder && !pl->onfloor && pl->move == -1 ? 0 : pl->move;   // movement on ladder
		if ( !editfly ) water = hdr.waterlevel > pl->o.z - 0.5f;

		float chspeed = 0.4f;
		if ( ! ( pl->onfloor || pl->onladder ) ) chspeed = 1.0f;

		const bool crouching = pl->crouching
		        || pl->eyeheight < pl->maxeyeheight;
		const float speed = curtime / ( water ? 2000.0f : 1000.0f )
		        * pl->maxspeed
		        * ( crouching && pl->state != CS_EDITING ? chspeed : 1.0f )
		        * ( pl == player1 && isfly ? flyspeed : 1.0f );
		const float friction =
		        water ? 20.0f :
		                ( pl->onfloor || isfly ? 6.0f :
		                                         ( pl->onladder ? 1.5f : 30.0f ) );
				...
				if ( pl->onladder ) {
					...
					if ( pl->type == ENT_BOT && pl->state == CS_ALIVE )
						pl->vel.z = climbspeed;   // bots climb upwards only
					        ...
					        && pl->state == CS_ALIVE ) {
						if ( ( (playerent *) pl )->k_up )
							pl->vel.z = climbspeed;
						else if ( ( (playerent *) pl )->k_down ) pl->vel.z =
						        -climbspeed;
					}
					pl->timeinair = 0;
				}
					...
					if ( pl->onfloor || water ) {
						if ( pl->jumpnext ) {
							pl->jumpnext = false;
							bool doublejump = pl->lastjump
							        && lastmillis - pl->lastjump < 250
							        && pl->strafe != 0
							        && pl->lastjumpheight != 0
	...
	if ( !editfly ) loopi(moveres)   // discrete steps collision detection & sliding
	    {
		    ...
		    if ( !collide(pl, false, drop, rise) )
			    ...
			    collided = true;
		    if ( pl->type == ENT_BOUNCE && cornersurface ) {   // try corner bounce
		    ...
		    if ( pl->type == ENT_CAMERA || ( pl->type == ENT_PLAYER
		            && pl->state == CS_DEAD
		            && ( (playerent *) pl )->spectatemode != SM_FLY ) ) {
			    pl->o.x -= f * d.x;
			    pl->o.y -= f * d.y;
		    ...
		    if ( pl->type != ENT_BOUNCE && hitplayer ) {
			    ...
			    float invdist = ufInvSqrt(dr.sqrxy()), push = (
			            invdist < 10.0f ? dr.dotxy(d) * 1.1f * invdist :
			                              dr.dotxy(d) * 11.0f );
...
//         try just dropping down
	...
	if ( pl->type != ENT_BOUNCE && pl == player1 ) {
		...
		/*extern int recoiltest;
		 ...
		 else*/if ( pl->pitchvel < 0.05f && pl->pitchvel > 0.001f )
...
void physicsframe ()   // optimally schedule physics frames inside the graphics frames
{
	...
	if ( diff <= 0 )
		...
		physframetime = clamp( ( PHYSFRAMETIME * gamespeed ) / 100, 1,
		                      PHYSFRAMETIME);
...
int tsens ( int x )
{
	...
	if ( x == -2000 ) {   // RENDERING PART!!!
		if ( senst ) {
			draw_textf(
			        ...
			        lowtry ? "(TRIED)" : "",
			        hightry && lowtry ?
			                "after trying both choose the one you liked most" :
			                "now you can choose the sensitivity you preferred");
			glPushMatrix();
			glScalef(2, 2, 2);
	...
	if ( x == SDLK_3 || x == SDLK_4 ) {
		if ( !hightry || !lowtry ) {
			if ( !hightry && !lowtry ) {
				...
				conoutf("--- \f3ERROR:\f0try the %s%ser sensitivity too.",
				        hightry ? "" : "high", lowtry ? "" : "low");
			}
		}
		else {
			...
			if ( sensh / sensn > 1.04f ) {
				conoutf("--- \f0you chose the %ser sensitivity.",
				        x == SDLK_3 ? "higher" : "lower");
	...
	if ( x == SDLK_2 ) {
		testsens = sensl;
		conoutf("--- \f0You are now %strying the lower sensitivity.",
		        lowtry ? "re-" : "");
		lowtry = 1;
	}
	if ( x == SDLK_1 ) {
		testsens = sensh;
		conoutf("--- \f0You are now %strying the higher sensitivity.",
		        hightry ? "re-" : "");
		hightry = 1;
	}
	if ( x == -1000 ) {
...
static inline bool zooming ( playerent *plx )
{
	return ( plx->weaponsel->type == GUN_SNIPER
	        && ( (sniperrifle *) plx->weaponsel )->scoped );
}

void mousemove ( int odx, int ody )
{
	static float fdx = 0, fdy = 0;
	if ( intermission || ( player1->isspectating()
	        && player1->spectatemode == SM_FOLLOW1ST ) ) return;
	float dx = odx, dy = ody;
	if ( mfilter > 0.0f ) {
	...
	if ( mouseaccel && curtime && ( dx || dy ) )
	    cursens += 0.02f * mouseaccel * sqrt(dx * dx + dy * dy) / curtime;
