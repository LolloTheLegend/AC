...
COMMANDF(per_idents, "i",
         (int *on) { per_idents = neverpersist ? false : (*on != 0); });
...
static void pushident ( ident &id, char *val, int context = execcontext )
{
	...
	stack->action =
	        id.executing == id.action ? newstring(id.action) : id.action;
...
static ident *newident ( const char *name, int context = execcontext )
{
	...
	if ( !id ) {
		ident init(ID_ALIAS, newstring(name), newstring(""), per_idents,
		           context);
...
void alias ( const char *name, const char *action, bool constant )
{
	...
	if ( !b ) {
		ident b(ID_ALIAS, newstring(name), newstring(action),
		        persistidents && !constant, execcontext);
		b.isconst = constant;
		...
		if ( contextisolated[execcontext] && execcontext > b->context ) {
			conoutf("cannot redefine alias %s in this execution context",
			        b->name);
			scripterr();
			return;
		}
...
COMMANDF(
        alias, "ss",
        (const char *name, const char *action) { alias(name, action, false); });
COMMANDF(const, "ss",
         (const char *name, const char *action) { alias(name, action, true); });
...
COMMANDF(
        isconst,
        "s",
        (const char *name) { ident *id = idents->access(name); intret(id && id->isconst ? 1 : 0); });
...
static void modifyvar ( const char *name, int arg, char op )
{
	...
	if ( ( id->type == ID_VAR && id->minval > id->maxval ) || ( id->type
	        == ID_FVAR
	                                                            && id->minvalf > id
	                                                                   ->maxvalf ) ) {
...
static void addeq ( char *name, int *arg )
{
	modifyvar(name, *arg, '+');
}
static void subeq ( char *name, int *arg )
{
	modifyvar(name, *arg, '-');
}
static void muleq ( char *name, int *arg )
{
	modifyvar(name, *arg, '*');
}
static void diveq ( char *name, int *arg )
{
	modifyvar(name, *arg, '/');
}
static void addeqf ( char *name, float *arg )
{
	modifyfvar(name, *arg, '+');
}
static void subeqf ( char *name, float *arg )
{
	modifyfvar(name, *arg, '-');
}
static void muleqf ( char *name, float *arg )
{
	modifyfvar(name, *arg, '*');
}
static void diveqf ( char *name, float *arg )
{
	modifyfvar(name, *arg, '/');
}
	...
	if ( noproblem )   // if the "seer"-algorithm doesn't object
	{
		for ( bool cont = true; cont; )        // for each ; seperated statement
			...
			if ( !id ) {
				...
				if ( !allowidentaccess(id) ) {
					conoutf("not allowed in this execution context: %s",
					        id->name);
					scripterr();
					continue;
				}
				...
				switch ( id->type )
				{
					case ID_COMMAND:                    // game defined commands
					{
						...
						if ( strstr(id->sig, "v") )
							( (void (__cdecl *) ( char **, int )) id->fun )(
							        &w[1], numargs - 1);
						else if ( strstr(id->sig, "c")
						        || strstr(id->sig, "w") ) {
							char *r = conc(w + 1, numargs - 1,
...
#define ARG(i) (id->sig[i-1] == 'i' ? ((void *)&(ib##i=strtol(w[i], NULL, 0))) : (id->sig[i-1] == 'f' ? ((void *)&(fb##i=atof(w[i]))) : (void *)w[i]))
							...
							// use very ad-hoc function signature, and just call it
							{
								case 0:
									...
									break;
								case 1:
									( (void (__cdecl *) ( void* )) id->fun )(
									        ARG(1));
									break;
								case 2:
									( (void (__cdecl *) ( void*, void* )) id
									    ->fun )(ARG(1), ARG(2));
									break;
								case 3:
									( (void (__cdecl *) ( void*, void*, void* )) id
									    ->fun )(ARG(1), ARG(2), ARG(3));
									break;
								case 4:
									( (void (__cdecl *) ( void*,
									                      void*,
...
#undef ARG
					...
					case ID_VAR:                       // game defined variables
						if ( !w[1][0] )
							conoutf("%s = %d", c, *id->storage.i);   // var with no value just prints its current value
					...
					case ID_FVAR:                      // game defined variables
						if ( !w[1][0] )
							conoutf("%s = %s", c, floatstr(*id->storage.f));   // var with no value just prints its current value
							...
							if ( f1 < id->minvalf || f1 > id->maxvalf ) {
								...
								conoutf("valid range for %s is %s..%s",
								        id->name, floatstr(id->minvalf),
								        floatstr(id->maxvalf));
					...
					case ID_SVAR:                      // game defined variables
						if ( !w[1][0] )
							conoutf(strchr(*id->storage.s, '"') ? "%s = [%s]" :
							                                      "%s = \"%s\"",
							        c, *id->storage.s);   // var with no value just prints its current value
						else {
							*id->storage.s = exchangestr(*id->storage.s,
							                             newstring(w[1]));
							if ( id->fun ) ( (void (__cdecl *) ()) id->fun )();   // call trigger function if available
						}
						break;
					...
					case ID_ALIAS:   // alias, also used as functions and (global) variables
						...
						;
						if ( id->executing != id->action && id->executing
						        != wasexecuting ) delete[] id->executing;
...
enum
{
	COMPLETE_FILE = 0, COMPLETE_LIST, COMPLETE_NICK
};
...
struct completeval
{
	...
	completeval ( int type, const char *dir, const char *ext ) :
			type(type), dir(dir && dir[0] ? newstring(dir) : NULL),
...
{
	return x.type == y.type && ( x.dir == y.dir
	        || ( x.dir && y.dir && !strcmp(x.dir, y.dir) ) )
...
static void commandcomplete ( char *s )
{
	...
	for ( int i = (int) strlen(s) - 1; i > 0; i-- )
		if ( s[i] == ' ' ) {
			cp = s + i;
			break;
		}   //testing for command/argument needs completion
	...
	if ( init && cdata && cdata->type == COMPLETE_FILE ) {
		...
		loopv(cdata->dirlist)
			listfiles(cdata->dirlist[i], cdata->ext, cdata->list);
	}

	if ( *cp == '/' || *cp == ';'
	     || ( cp == s && ( *cp == ' ' || *cp == '(' ) ) ) {   // commandname completion
		int idx = 0;
		enumerate(
		        *idents,
		        ident,
		        id,
		        if ( !strncasecmp(id.name, cp + 1, completesize) && idx++
		                == completeidx ) {
			        cp[1] = '\0'
			        ;
			        strcpy(s + 1, id.name)
			        ;
			        //concatstring/copystring will crash because of overflow
		        } );
		completeidx++;
...
{
	if ( curcontext )
		conoutf("(%s: %s)", curcontext, curinfo);
	else
		conoutf("(from console or builtin)");
}
...
static void ifthen ( char *cond, char *thenp, char *elsep )
{
...
static void continuea ()
{
	if ( loop_level ) loop_skip = true;
}

static void concat ( char *s )
{
	result(s);
}
...
static void swapelements ( char *list, char *v )
{
	...
	for ( int i = 0; i < swap.length(); i += 2 ) {
		if ( elems.inrange(atoi(swap[i])) && elems.inrange(
		        atoi(swap[i + 1])) ) {
...
COMMANDF(
        strreplace,
        "sss",
        (const char *source, const char *search, const char *replace) { string d; result(strreplace(d, source, search, replace)); });
...
COMMANDN(-, sub, "ii");
static void div_ ( int *a, int *b )
{
	intret(*b ? ( *a ) / ( *b ) : 0);
}
COMMANDN(div, div_, "ii");
static void mod_ ( int *a, int *b )
{
	intret(*b ? ( *a ) % ( *b ) : 0);
}
COMMANDN(mod, mod_, "ii");
static void addf ( float *a, float *b )
{
	floatret(*a + *b);
}
COMMANDN(+f, addf, "ff");
static void mulf ( float *a, float *b )
{
	floatret(*a * *b);
}
COMMANDN(*f, mulf, "ff");
static void subf ( float *a, float *b )
{
	floatret(*a - *b);
}
COMMANDN(-f, subf, "ff");
static void divf_ ( float *a, float *b )
{
	floatret(*b ? ( *a ) / ( *b ) : 0);
}
COMMANDN(divf, divf_, "ff");
static void modf_ ( float *a, float *b )
{
	floatret(*b ? fmod(*a, *b) : 0);
}
COMMANDN(modf, modf_, "ff");
static void powf_ ( float *a, float *b )
{
	floatret(powf(*a, *b));
}
COMMANDN(powf, powf_, "ff");
static void not_ ( int *a )
{
	intret((int) ( ! ( *a ) ));
}
COMMANDN(!, not_, "i");
...
COMMANDN(!=, notequal, "ii");
static void lt ( int *a, int *b )
{
	intret((int) ( *a < *b ));
}
COMMANDN(<, lt, "ii");
...
COMMANDN(>, gt, "ii");
static void lte ( int *a, int *b )
{
	intret((int) ( *a <= *b ));
}
COMMANDN(<=, lte, "ii");
static void gte ( int *a, int *b )
{
	intret((int) ( *a >= *b ));
}
COMMANDN(>=, gte, "ii");
...
static void anda ( char *a, char *b )
{
	intret(execute(a) != 0 && execute(b) != 0);
}
static void ora ( char *a, char *b )
{
	intret(execute(a) != 0 || execute(b) != 0);
}
...
void writecfg ()
{
	...
	loopi(CROSSHAIR_NUM)
		if ( crosshairs[i] && crosshairs[i] != notexture ) {
			const char *fname = crosshairs[i]->name
			        + strlen("packages/crosshairs/");
			if ( i == CROSSHAIR_DEFAULT )
				f->printf("loadcrosshair %s\n", fname);
			else
	...
	enumerate(
	        *idents,
	        ident,
	        id,
	        if(!id.persist) continue; switch(id.type) { case ID_VAR: f->printf("%s %d\n", id.name, *id.storage.i); break; case ID_FVAR: f->printf("%s %s\n", id.name, floatstr(*id.storage.f)); break; case ID_SVAR: f->printf("%s [%s]\n", id.name, *id.storage.s); break; });
	f->printf("\n// weapon settings\n\n");
	loopi(NUMGUNS)
		if ( guns[i].isauto ) {
	...
	enumerate(
	        *idents,
	        ...
	        id,
	        if ( id.type == ID_ALIAS && id.persist && id.action[0]
	             && !strncmp(id.name, "demodesc_", 9) && id.action ) {
		        f->printf("%s = [%s]\n", id.name, id.action)
		        ;
	        } );
	f->printf("\n");
...
{
	if ( *context >= 0 && *context < IEXC_NUM && !contextsealed )
	    contextisolated[*context] = true;
}

static void sealcontexts ()
{
	contextsealed = true;
}
...
static void strpos_ ( char *haystack, char *needle, int *occurence )
{
	...
	if ( haystack && needle ) for ( int iocc = *occurence; iocc >= 0; iocc-- ) {
...
static void l0 ( int *p, int *v )
{
	string f;
	string r;
	formatstring(f)("%%0%dd", *p);
	formatstring(r)(f, *v);
	result(r);
}
	...
	if ( curpeer ) {
		switch ( i )
		{
			...
			case 2:   // HOST
			{
				char hn[1024];
				formatstring(r)(
				        "%s", (enet_address_get_host(&curpeer->address, hn, sizeof(hn))==0) ? hn : "unknown");
				break;
			}
			case 3:   // PORT
			{
