...
const char *entmdlnames[] = { "pistolclips", "ammobox", "nade", "health",
...
static void renderent ( entity &e )
{
	...
	float z = (float) ( 1 + sinf(lastmillis / 100.0f + e.x + e.y) / 20 ), yaw =
	        lastmillis / 10.0f;
...
{
	float xradius = max(float(e.attr2), 0.1f), yradius = max(float(e.attr3),
	                                                         0.1f);
	vec bbmin(e.x - xradius, e.y - yradius,
	          float(S(e.x, e.y)->floor + e.attr1)), bbmax(
	        e.x + xradius, e.y + yradius, bbmin.z + max(float(e.attr4), 0.1f));
	...
	loopi(8)
		glVertex3f(i & 2 ? bbmax.x : bbmin.x, i & 4 ? bbmax.y : bbmin.y,
		           i & 1 ? bbmax.z : bbmin.z);
	...
	if ( text && text[0] != '\0' ) {
		...
		do {
			...
			if ( sn != -1 ) {
				...
				sn = tn;
			if ( !k ) {
				if ( sn > 0 && sn < MAXENTTYPES ) eh_ents.add(sn);
			}
			s = strtok(NULL, " ");
		}
		while ( s );
...
static void renderentarrow ( const entity &e, const vec &dir, float radius )
{
	...
	vec target = vec(dir).mul(radius).add(epos), arrowbase = vec(dir).mul(
	        radius - arrowsize).add(epos), spoke;
		...
		switch ( f.state )
		{
			case CTFF_STOLEN:
				if ( f.actor && f.actor != player1 ) {
					...
					rendermodel(
					        path,
					        ANIM_FLAG | ANIM_START | ANIM_DYNALLOC,
					        0,
					        0,
					        vec(f.actor->o).add(
					                vec(0,
					                    0,
					                    0.3f + ( sinf(lastmillis / 100.0f) + 1 ) / 10)),
				...
				if ( f.flagent->spawned )
				    rendermodel(
				            ...
				            vec(f.pos.x,
				                f.pos.y,
				                f.state == CTFF_INBASE ?
				                        (float) S(int(f.pos.x), int(f.pos.y))
				                            ->floor :
				                        f.pos.z),
				            e.attr1 / 4.0f, 0, 120.0f);
				break;
	...
	if ( &is ) {
		if ( d == player1 ) {
			...
			if ( identexists("onPickup") ) {
				...
				if ( tmp ) {
					formatstring(o)(
					        "onPickup %d %d", e.type - 3, m_lss && e.type == I_GRENADE ? 2 : tmp->add);
					execute(o);
				}
			}
		}
			...
			if ( own )   // it's the own flag
			{
				if ( f.state == CTFF_DROPPED )
					flagreturn(flag);
				          ...
				          && of.actor == d && of.ack ) flagscore(of.team);
			}
			else
				flagpickup(flag);
		}
...
{
	loopv(ents)
		if ( ents[i].fitsmode(gamemode) || ( multiplayer(false)
		        && gamespeed != 100 && ( i = -1 ) ) ) {
			ents[i].spawned = true;
...
{
	loopv(ents)
		if ( type < 0 || type == ents[i].type ) ents[i].spawned = false;
...
VARFP(nextprimary, 0, GUN_ASSAULT, NUMGUNS, {
	if ( !selectnextprimary(nextprimary) ) selectnextprimary( ( nextprimary =
	        GUN_ASSAULT ))
	;
});
...
static void entstats ( void )
{
	...
	loopi(MAXENTTYPES)
	{
		if ( entcnt[i] )
		    ...
		    {
			    case MAPMODEL:
				    conoutf(" %d %s, %d clipped", entcnt[i], entnames[i],
				            clipents);
				    break;
			    case PLAYERSTART:
