...
struct console : consolebuffer<cline>
{
	...
	void render ()
	{
		...
		int conheight =
		        min(fullconsole ?
		                ( h * ( fullconsole == 1 ? altconsize : fullconsize ) ) / 100 :
		                FONTH * consize,
		            h);
		...
		if ( !fullconsole && confade ) {
			if ( !conskip ) {
				numl = 0;
				loopvrev(conlines)
					if ( totalmillis - conlines[i].millis < confade * 1000 ) {
						numl = i + 1;
						break;
					}
			}
	...
	console () :
			consolebuffer<cline>(200), fullconsole(false)
	{
	}
};
...
inline static void setconskip ( int *n )
{
	con.setconskip(*n);
}
...
void clientlogf ( const char *s, ... )
{
	...
	const char *ts =
	        scl.logtimestamp ? timestring(true, "%b %d %H:%M:%S ") : "";
	char *p, *l = sp;
...
int rendercommand ( int x, int y, int w )
{
	...
	draw_text(
	        ...
	        0xFF,
	        0xFF,
	        cmdline.pos >= 0 ? cmdline.pos + strlen(useprompt) + 1 :
	                           (int) strlen(s),
	        w);
	return height;
}
...
{
	const char* CONpreSTR[] = { ">",   // ">>>", // "TALK" // "T" // ">"
	        "/",   // "CFG", // "EXEC" // "!" // "!"
	        "%",   // "TEAM" // ">" // "T"
	        };
	return ( n >= 0
	        && size_t(n) < sizeof ( CONpreSTR ) / sizeof ( CONpreSTR[0] ) ) ?
	        CONpreSTR[n] : "#";
}
...
static int rendercommand_wip ( int x, int y, int w )
{
	...
	if ( strlen(cmdline.buf) > 0 ) {
		...
		draw_text(
		        ...
		        0xFF,
		        0xFF,
		        cmdline.pos >= 0 ? cmdline.pos/*+1*/+ getCONlength(ctx) :
		                           (int) strlen(s),
		        w);
	}
	return height;
...
static const char *keycmds[keym::NUMACTIONS] =
        { "bind", "specbind", "editbind" };
static inline const char *keycmd ( int type )
{
...
{
	loopv(keyms)
		if ( !strcasecmp(keyms[i].name, key) ) return &keyms[i];
	return NULL;
}
...
{
	loopv(keyms)
		if ( !strcasecmp(keyms[i].actions[type], action) ) return &keyms[i];
	return NULL;
}
...
{
	loopv(keyms)
		if ( keyms[i].code == code ) return &keyms[i];
	return NULL;
}
...
bool bindkey ( keym *km, const char *action, int type )
{
	...
	if ( type < keym::ACTION_DEFAULT || type >= keym::NUMACTIONS ) {
		conoutf("invalid bind type \"%i\"", type);
		return false;
	}
...
COMMANDF(
        bind,
        "ss",
        (const char *key, const char *action) { bindk(key, action, keym::ACTION_DEFAULT); });
COMMANDF(
        specbind,
        "ss",
        (const char *key, const char *action) { bindk(key, action, keym::ACTION_SPECTATOR); });
COMMANDF(
        editbind,
        "ss",
        (const char *key, const char *action) { bindk(key, action, keym::ACTION_EDITING); });
...
void pasteconsole ( char *dst )
{
	...
	for ( char *cbline = cb, *cbend;
	        commandlen + 1 < MAXSTRLEN && cbline < &cb[cbsize];
	        cbline = cbend + 1 ) {
...
struct hline
{
	...
	hline () :
			buf(NULL), action(NULL), prompt(NULL)
	{
	}
	~hline ()
	{
	...
	bool shouldsave ()
	{
		return strcmp(cmdline.buf, buf) || (
		        cmdaction ? !action || strcmp(cmdaction, action) :
		                    action != NULL )
		       || ( cmdprompt ? !prompt || strcmp(cmdprompt, prompt) :
		                        prompt != NULL );
	}
	...
	void run ()
	{
		...
		if ( action ) {
			...
			toserver(buf);   // execute(buf); // still default to simple "say".
		popscontext();
	}
};
...
static void execbind ( keym &k, bool isdown )
{
	...
	if ( isdown ) {
		...
		char *&action =
		        k.actions[state][0] ? k.actions[state] :
...
static void consolekey ( int code, bool isdown, int cooked )
{
	if ( isdown ) {
			...
			case SDLK_TAB:
				if ( !cmdaction ) {
					...
					if ( cmdline.pos >= 0 && cmdline.pos
					        >= (int) strlen(cmdline.buf) ) cmdline.pos = -1;
				}
				break;
		...
		if ( code == SDLK_RETURN || code == SDLK_KP_ENTER
			...
			if ( code == SDL_AC_BUTTON_LEFT && histpos == history.length() )
			    return;
			...
			if ( cmdline.buf[0] ) {
				if ( history.empty() || history.last()->shouldsave() ) {
					if ( maxhistory && history.length() >= maxhistory ) {
						loopi(history.length()-maxhistory+1)
							delete history[i];
...
void keypress ( int code, bool isdown, int cooked, SDLMod mod )
{
	...
	if ( haskey && haskey->pressed )
		...
		consolekey(code, isdown, cooked);   // keystrokes go to commandline
