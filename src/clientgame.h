...
struct serverstate
{
	...
	void reset ()
	{
		autoteam = mastermode = matchteamsize = 0;
	}
};
...
struct votedisplayinfo
{
	...
	votedisplayinfo () :
			owner(NULL), result(VOTE_NEUTRAL), millis(0),
			        localplayervoted(false)
	{
		loopi(VOTE_NUM)
			stats[i] = VOTE_NEUTRAL;
	}
};
...
extern int lastpm;
extern int minutesremaining, gametimecurrent, gametimemaximum,
        lastgametimeupdate;
