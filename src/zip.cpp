...
struct zipfile
{
	...
	zipfile () :
			name(NULL), header(0), offset(~0U), size(0), compressedsize(0)
	{
	}
	~zipfile ()
	{
...
struct ziparchive
{
	...
	ziparchive () :
			name(NULL), data(NULL), files(512), openfiles(0), type(-1),
			        owner(NULL)
	{
	}
	~ziparchive ()
...
static bool findzipdirectory ( FILE *f, zipdirectoryheader &hdr )
{
	...
	int len = 0, offset = ftell(f), end = max(
	        offset - 0xFFFF - ZIP_DIRECTORY_SIZE, 0);
	const uint signature = lilswap<uint>(ZIP_DIRECTORY_SIGNATURE);

	while ( offset > end ) {
		int carry = min(len, ZIP_DIRECTORY_SIZE - 1), next = min(
		        (int) sizeof ( buf ) - carry, offset - end);
		offset -= next;
		memmove(&buf[next], buf, carry);
		if ( next + carry < ZIP_DIRECTORY_SIZE || fseek(f, offset, SEEK_SET) < 0
		     || (int) fread(buf, 1, next, f) != next ) return false;
		len = next + carry;
	...
	if ( hdr.signature != ZIP_DIRECTORY_SIGNATURE || hdr.disknumber
	        != hdr.directorydisk
	     || hdr.diskentries != hdr.entries ) return false;
...
static bool readzipdirectory ( const char *archname,
                               FILE *f,
                               int entries,
                               int offset,
                               int size,
                               vector<zipfile> &files )
{
	...
	if ( fseek(f, offset, SEEK_SET) < 0 || (int) fread(buf, 1, size, f)
	        != size ) {
		delete[] buf;
		return false;
	}
	loopi(entries)
	{
		...
		if ( !hdr.namelength || !hdr.uncompressedsize
		     || ( hdr.compression && ( hdr.compression != Z_DEFLATED
		             || !hdr.compressedsize ) ) ) {
...
#ifndef STANDALONE
		if ( dbgzip )
		    conoutf("file %s, size %d, compress %d, flags %x", archname, name,
...
{
	loopv(archives)
		if ( !strcmp(name, archives[i]->name) ) return archives[i];
	return NULL;
}

static bool checkprefix ( vector<zipfile> &files,
                          const char *prefix,
                          int prefixlen )
{
	loopv(files)
...
static bool fitspackage ( char *filename, int type )
{
	...
	{
		case PCK_MAPMODEL:
			return !strcmp(extension, "md2") || !strcmp(extension, "md3")
			       || !strcmp(extension, "cfg") || !strcmp(extension, "txt")
			       || !strcmp(extension, "jpg") || !strcmp(extension, "png");
		case PCK_MAP:
			return !strcmp(extension, "cgz") || !strcmp(extension, "cfg");
		case PCK_SKYBOX:
			return !strcmp(extension, "jpg") || !strcmp(extension, "png");
		default:
			return true;
	}
}

static void mountzip ( ziparchive &arch,
                       vector<zipfile> &files,
                       const char *mountdir,
                       const char *stripdir,
                       bool extract )
{
	...
	loopv(files)
	{
		zipfile &f = files[i];
		formatstring(fname)(
		        "%s%s", mdir, striplen && !strncmp(f.name, stripdir, striplen) ? &f.name[striplen] : f.name);
		...
		if ( extract ) {
			if ( fitspackage(fname, arch.type) )
				extractzipfile(&arch, &f, fname);
			else
				conoutf("\f3could not extract \"%s\" : file extension not supported",
				        fname);
		}
	}
}
              ...
              int type )
{
	...
	vector<zipfile> files;
	if ( !findzipdirectory(f, h) || !readzipdirectory(pname, f, h.entries,
	                                                  h.offset, h.size,
	                                                  files) ) {
...
struct zipstream : stream
{
	...
	zipstream () :
			arch(NULL), info(NULL), buf(NULL), reading(-1), ended(false)
	{
		zfile.zalloc = NULL;
		zfile.zfree = NULL;
		zfile.opaque = NULL;
	...
	void readbuf ( uint size = BUFSIZE )
	{
		...
		int n = arch->owner == this ?
		        (int) fread(zfile.next_in + zfile.avail_in, 1,
		                    min(size, remaining), arch->data) :
		        0;
		zfile.avail_in += n;
		reading += n;
	}
	...
	bool open ( ziparchive *a, zipfile *f )
	{
		if ( f->offset == ~0U ) {
			...
			f->offset = f->header + ZIP_LOCAL_FILE_SIZE + h.namelength
			            + h.extralength;
		}
...
#ifndef STANDALONE
		if ( dbgzip )
		    conoutf(info->compressedsize ?
		            "%s: zfile.total_out %d, info->size %d" :
		            "%s: reading %d, info->size %d",
		            info->name,
		            info->compressedsize ? zfile.total_out :
		                                   reading - info->offset,
		            info->size);
#endif
	...
	bool seek ( long pos, int whence )
	{
		...
		if ( !info->compressedsize ) {
			...
			pos = clamp(pos, long(info->offset),
			            long(info->offset + info->size));
			arch->owner = NULL;
		...
		if ( pos >= (long) zfile.total_out )
			...
			if ( zfile.next_in && zfile.total_in
			        <= uint(zfile.next_in - buf) ) {
	...
	int read ( void *buf, int len )
	{
		...
		if ( !info->compressedsize ) {
			...
			int n = (int) fread(
			        buf, 1, min(len, int(info->size + info->offset - reading)),
			        arch->data);
			reading += n;
			if ( n < len ) ended = true;
			return n;
...
static bool extractzipfile ( ziparchive *a, zipfile *f, const char *name )
{
	...
	if ( s->open(a, f) && ( target = fopen(fname, "wb") ) ) {
		size_t len;
		uchar copybuf[1024];
		while ( ( len = s->read(copybuf, 1024) ) )
			fwrite(copybuf, 1, len, target);
	}
	else
		error = true;
	if ( error )
	    conoutf("failed to extract zip file %s from archive %s", fname,
	            a->name);
...
int listzipfiles ( const char *dir, const char *ext, vector<char *> &files )
{
	...
	loopvrev(archives)
	{
		...
		enumerate(
		        ...
		        zipfile,
		        f,
		        { if(strncmp(f.name, dir, dirsize)) continue; const char *name = f.name + dirsize; if(name[0] == PATHDIV) name++; if(strchr(name, PATHDIV)) continue; if(!ext) files.add(newstring(name)); else { int namelength = (int)strlen(name) - extsize; if(namelength > 0 && name[namelength] == '.' && strncmp(name+namelength+1, ext, extsize-1)==0) files.add(newstring(name, namelength)); } });
...
COMMANDF(
        addzip,
        "sss",
        (const char *name, const char *mount, const char *strip) { addzip(name, mount[0] ? mount : NULL, strip[0] ? strip : NULL); });
