...
template<class LINE>
	struct consolebuffer
	{
		...
		consolebuffer ( int maxlines = 100 ) :
				maxlines(maxlines)
		{
		}
		...
		LINE &addline ( const char *sf, int millis )   // add a line to the console buffer
		{
			LINE cl;
			cl.line =
			        conlines.length() > maxlines ? conlines.pop().line :
		...
		{
			maxlines = numlines;
			while ( conlines.length() > maxlines )
...
struct keym
{
	...
	enum
	{
		ACTION_DEFAULT = 0, ACTION_SPECTATOR, ACTION_EDITING, NUMACTIONS
	};
	...
	keym () :
			code(-1), name(NULL), pressed(false)
	{
		loopi(NUMACTIONS)
			actions[i] = newstring("");
	}
