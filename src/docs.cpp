...
static void renderdocsection ( void *menu, bool init );

// TODO: Lollo Needs to import command.h when refactored will be done
...
struct docargument
{
	...
	docargument () :
			token(NULL), desc(NULL), values(NULL)
	{
	}
	;
	~docargument ()
...
struct docref
{
	...
	docref () :
			name(NULL), ident(NULL), url(NULL), article(NULL)
	{
	}
	~docref ()
	{
...
struct dockey
{
	...
	dockey () :
			alias(NULL), name(NULL), desc(NULL)
	{
	}
	~dockey ()
	{
...
static vector<docsection> sections;
static hashtable<const char *, docident> docidents;   // manage globally instead of a section tree to ensure uniqueness
static docsection *lastsection = NULL;
static docident *lastident = NULL;

static void adddocsection ( char *name )
{
	if ( !name ) return;
...
static void adddocident ( char *name, char *desc )
{
...
static void adddocargument ( char *token,
                             char *desc,
                             char *values,
                             char *vararg )
{
...
static void adddocremark ( char *remark )
{
	if ( !lastident || !remark ) return;
	lastident->remarks.add(newstring(remark));
}

static void adddocref ( char *name, char *ident, char *url, char *article )
{
...
static void adddocexample ( char *code, char *explanation )
{
...
static void adddockey ( char *alias, char *name, char *desc )
{
...
static char *cvecstr ( vector<char *> &cvec, const char *substr, int *rline =
        NULL )
{
	char *r = NULL;
	loopv(cvec)
		if ( cvec[i] ) if ( ( r = strstr(cvec[i], substr) ) != NULL ) {
			if ( rline ) *rline = i;
			break;
		}
	return r;
...
static void listundoneidents ( vector<const char *> &inames, int allidents )
{
	enumeratekt(
	        *idents,
	        const char *,
	        ...
	        ident,
	        id,
	        { if((allidents > 0) || id.type != ID_ALIAS) { docident *id = docidents.access(name); if(id)   // search for substrings that indicate undoneness
	        { vector<char *> srch; srch.add(id->name); srch.add(id->desc); loopvj(id->remarks) srch.add(id->remarks[j]); loopvj(id->arguments) { srch.add(id->arguments[j].token); srch.add(id->arguments[j].desc); srch.add(id->arguments[j].values); } loopvj(id->references) { srch.add(id->references[j].ident); srch.add(id->references[j].name); srch.add(id->references[j].url); } if(cvecstr(srch, "TODO") || cvecstr(srch, "UNDONE")) id = NULL; } if(!id) inames.add(name); } });
	inames.sort(stringsort);
}

static void docundone ( int *allidents )
{
...
static void docfind ( char *search )
{
	enumerate(
	        docidents,
	        docident,
	        i,
	        { vector<char *> srch; srch.add(i.name); srch.add(i.desc); loopvk(i.remarks) srch.add(i.remarks[k]);

	        char *r; int rline; if((r = cvecstr(srch, search, &rline))) { const int matchchars = 200; string match; copystring(match, r-srch[rline] > matchchars/2 ? r-matchchars/2 : srch[rline], matchchars/2); conoutf("%-20s%s", i.name, match); } });
}

static char *xmlstringenc ( char *d, const char *s, size_t len )
{
	if ( !d || !s ) return NULL;
	struct spchar
	{
	...
	while ( *sc && (size_t) ( dc - d ) < len - 1 ) {
		bool specialc = false;
		loopi(sizeof(spchars)/sizeof(spchar))
			if ( spchars[i].c == *sc ) {
				specialc = true;
...
static void docwriteref ( int allidents,
                          const char *ref,
                          const char *schemalocation,
                          const char *transformation )
{
	stream *f = openfile(
	        path(allidents < 0 ? "docs/autogenerated_base_reference.xml" :
	...
	if ( allidents < 0 ) {
		...
		f->printf(
		        ...
		        transformation && strlen(transformation) ?
		                transformation : "transformations/cuberef2xhtml.xslt");
		f->printf(
		        "<cuberef xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" name=\"%s\" version=\"v0.1\" xsi:schemaLocation=\"%s\" xmlns=\"http://cubers.net/Schemas/CubeRef\">\n",
	...
	loopv(inames)
	{
		...
		switch ( id->type )
		{
			case ID_VAR:
				f->printf(
				        "\t\t\t\t\t<value %s description=\"TODO\" minValue=\"%i\" maxValue=\"%i\" defaultValue=\"%i\" %s/>\n",
				        id->minval > id->maxval ? "" : "token=\"N\"",
				        id->minval, id->maxval, *id->storage.i,
				        id->minval > id->maxval ? "readOnly=\"true\"" : "");
				break;
			case ID_FVAR:
				f->printf(
				        "\t\t\t\t\t<value %s description=\"TODO\" minValue=\"%s\" maxValue=\"%s\" defaultValue=\"%s\" %s/>\n",
...
static void docwritebaseref ( char *ref,
                              char *schemalocation,
                              char *transformation )
{
...
static void docwritetodoref ( int *allidents )
{
...
static VAR(docvisible, 0, 1, 1);
static VAR(docskip, 0, 0, 1000);
...
void renderdoc ( int x, int y, int doch )
{
	...
	while ( *exp ) {
		exp++;
		o++;
		if ( *exp == ';' || ( *exp == ' ' && f == o - 1 ) ) f = o;
	}
	exp--;

	if ( f > 0 ) {
		for ( int i = o - f - 1; i > 0; i-- )
			exp--;
		if ( o > f + 1 ) exp++;
	}
			...
			if ( ident ) {
				...
				if ( ident->arguments.length() > 0 )   // args
				{
					...
					if ( args ) {
						...
						if ( arg >= ident->arguments.length() && ident
						         ->arguments.last().vararg )
						    arg = ident->arguments.length() - 1;
					}
					...
					loopvj(ident->arguments)
					{
						...
						if ( !a ) continue;
						formatstring(doclines.add(newstringbuf()))(
						        "~\f%d%-8s%s %s%s%s", j == arg ? 4 : 5, a->token, a->desc,
				...
				{
					doclines.add(
					        ident->examples.length() == 1 ? "Example:" :
					                                        "Examples:");
				...
				{
					doclines.add(
					        ident->keys.length() == 1 ? "Default key:" :
					                                    "Default keys:");
					loopvj(ident->keys)
					{
						dockey &k = ident->keys[j];
						defformatstring(line)("~%-10s %s",
						                      k.name ? k.name : k.alias,
						                      k.desc ? k.desc : "");
				...
				if ( ident->references.length() )   // references
				{
					struct category
					{
						...
						string refs;
					        ...
					        { { "related identifiers", "" }, { "web resources",
					                                           "" },
					          { "wiki articles", "" }, { "other", "" } };
					loopvj(ident->references)
					{
						...
						if ( j < ident->references.length() - 1 )
						    concatstring(ref, ", ");
					}
					loopj(sizeof(categories)/sizeof(category))
					{
						...
						formatstring(doclines.add(newstringbuf()))(
						        "~%s: %s", categories[j].label, categories[j].refs);
					}
				}
				...
				int offset = min(docskip, doclines.length() - 1), maxl = offset,
				        cury = 0;
				...
				if ( offset > 0 && maxl >= doclines.length() ) {
					for ( int j = offset - 1; j >= 0; j-- ) {
						...
						int width = 0, height = FONTH;
						if ( str )
						    text_bounds(*str == '~' ? str + 1 : str, width,
						                height,
						                *str == '~' ? -1 : VIRTW * 4 / 3);
						if ( cury + height > doch ) break;
						cury += height;
				...
				draw_text("\f4disable doc reference (F1)", x,
				          y + doch + 2 * FONTH);
				return;
			}
		}
...
static int msectionsort ( const msection *a, const msection *b )
{
...
static void renderdocsection ( void *menu, bool init )
{
	...
	loopv(sections)
	{
		...
		loopv(msections)
		{
			menumanual(menu, msections[i].name, msections[i].cmd);
		}
		return;
	}
}
