...
static void FUNCNAME(halvetexture) ( uchar *src, uint sw, uint sh, uchar *dst )
{
	...
	for ( uchar *yend = &src[sh * stride]; src < yend; ) {
		for ( uchar *xend = &src[stride]; src < xend;
		        src += 2 * BPP, dst += BPP ) {
...
static void FUNCNAME(shifttexture) ( uchar *src,
                                     uint sw,
                                     uint sh,
                                     uchar *dst,
                                     uint dw,
                                     uint dh )
{
	uint stride = sw * BPP, wfrac = sw / dw, hfrac = sh / dh, wshift = 0,
	        hshift = 0;
	while ( dw << wshift < sw )
		wshift++;
	while ( dh << hshift < sh )
	...
	for ( uchar *yend = &src[sh * stride]; src < yend; ) {
		for ( uchar *xend = &src[stride]; src < xend; src += wfrac * BPP, dst +=
		        BPP ) {
#define OP(c, n) c##t = 0
			DEFPIXEL
#undef OP
			for ( uchar *ycur = src, *xend = &ycur[wfrac * BPP], *yend =
			        &src[hfrac * stride]; ycur < yend; ycur += stride, xend +=
			        stride ) {
...
static void FUNCNAME(scaletexture) ( uchar *src,
                                     uint sw,
                                     uint sh,
                                     uchar *dst,
                                     uint dw,
                                     uint dh )
{
	uint stride = sw * BPP, wfrac = ( sw << 12 ) / dw, hfrac = ( sh << 12 )
	        / dh;
	for ( uint dy = 0, y = 0, yi = 0; dy < dh; dy++ ) {
		uint y2 = y + hfrac, yi2 = y2 >> 12, h = y2 - y, ih = yi2 - yi, ylow,
		        yhigh;
		if ( yi < yi2 ) {
		...
		for ( uint dx = 0, x = 0, xi = 0; dx < dw; dx++ ) {
			uint x2 = x + wfrac, xi2 = x2 >> 12, w = x2 - x, iw = xi2 - xi, iy =
			        0, xlow, xhigh;
			if ( xi < xi2 ) {
...
#define OP(c, n) c##t = 0
			DEFPIXEL
#undef OP
			for ( uchar *ycur = &src[xi * BPP],
			        *xend = &ycur[max(iw, 1U) * BPP], *yend = &ycur[stride
			                * ( max(ih, 1U) + ( yhigh ? 1 : 0 ) )]; ycur < yend;
...
static void resizetexture ( int w,
                            int h,
                            bool mipmap,
                            bool canreduce,
                            GLenum target,
                            ...
                            int &th )
{
	int hwlimit = hwtexsize, sizelimit =
	        mipmap && maxtexsize ? min(maxtexsize, hwlimit) : hwlimit;
...
static void uploadtexture ( GLenum target,
                            GLenum internal,
                            int tw,
                            int th,
                            GLenum format,
                     ...
                     GLenum format )
{
	...
	uploadtexture(GL_TEXTURE_2D, format, tw, th, format, GL_UNSIGNED_BYTE,
	              pixels, w, h, mipmap);
}
...
SDL_Surface *wrapsurface ( void *data, int width, int height, int bpp )
{
	...
	{
		case 3:
			return SDL_CreateRGBSurfaceFrom(data, width, height, 8 * bpp,
			                                bpp * width, RGBMASKS);
		case 4:
			return SDL_CreateRGBSurfaceFrom(data, width, height, 8 * bpp,
			                                bpp * width, RGBAMASKS);
...
{
	SDL_Surface *ns = SDL_CreateRGBSurface(SDL_SWSURFACE, os->w, os->h, 24,
	                                       RGBMASKS);
...
{
	SDL_Surface *ns = SDL_CreateRGBSurface(SDL_SWSURFACE, os->w, os->h, 32,
	                                       RGBAMASKS);
	if ( ns ) {
		SDL_SetAlpha(os, 0, 0);
...
static bool checkgrayscale ( SDL_Surface *s )
{
	...
	if ( s->format->palette ) {
		if ( s->format->palette->ncolors != 256 || s->format->colorkey )
		    return false;
...
static int fixcl ( SDL_Surface *s,
                   bool check = true,
                   Uint8 value = 0,
                   Uint8 mlimit = 255 )
{
...
static SDL_Surface *fixsurfaceformat ( SDL_Surface *s )
{
	...
	switch ( s->format->BytesPerPixel )
	{
		case 1:
			if ( !checkgrayscale(s) )
			    return s->format->colorkey ? forcergbasurface(s) :
			                                 forcergbsurface(s);
			break;
		case 3:
			if ( s->format->Rmask != rgbmasks[0] || s->format->Gmask
			        != rgbmasks[1]
			     || s->format->Bmask != rgbmasks[2] ) return forcergbsurface(s);
			break;
		case 4:
			if ( s->format->Rmask != rgbamasks[0] || s->format->Gmask
			        != rgbamasks[1]
			     || s->format->Bmask != rgbamasks[2]
			     || s->format->Amask != rgbamasks[3] )
			    return s->format->Amask ? forcergbasurface(s) :
			                              forcergbsurface(s);
			break;
	}
	return s;
...
{
	SDL_Surface *m = SDL_CreateRGBSurface(SDL_SWSURFACE, s->w, s->h, 16, 0, 0,
	                                      0, 0);
...
static GLuint loadsurface ( const char *texname,
                            int &xs,
                            int &ys,
                            int &bpp,
                            int clamp = 0,
                            ...
                            bool trydl = false )
{
	...
	if ( texname[0] == '<' ) {
		file = strchr(texname, '>');
		if ( !file ) {
			if ( !silent_texture_load )
			    conoutf("could not load texture %s", texname);
			return 0;
		}
		file++;
	}
	...
	Uint8 x = 0;
	if ( strstr(texname, "playermodel") && ( x = fixcl(s) ) > 35 ) {
		fixcl(s, false, x, 35);
	}
	else if ( strstr(texname, "skin") && strstr(texname, "weapon") && ( x =
	        fixcl(s) )
	                                                                  > 40 ) {
		fixcl(s, false, x, 40);
	}
	...
	createtexture(tnum, s->w, s->h, s->pixels, clamp, mipmap, canreduce,
	              format);
	xs = s->w;
	ys = s->h;
...
static void draw_envbox_face ( float s0,
                               float t0,
                               float x0,
                               float y0,
                               float z0,
...
void draw_envbox ( int w )
{
	...
	float zclip =
	        skyclip && skyfloor >= camera1->o.z ?
	                0.5f + float(skyfloor - camera1->o.z) / w : 0.0f, vclip = 1
	        - zclip, z = 2 * w * ( vclip - 0.5f );
	...
	draw_envbox_face(1.0f, vclip, -w, -w, z, 0.0f, vclip, w, -w, z, 0.0f, 0.0f,
	                 w, -w, -w, 1.0f, 0.0f, -w, -w, -w,
	                 sky[2] ? sky[2] : notexture);

	draw_envbox_face(1.0f, vclip, +w, w, z, 0.0f, vclip, -w, w, z, 0.0f, 0.0f,
	                 -w, w, -w, 1.0f, 0.0f, +w, w, -w,
	                 sky[3] ? sky[3] : notexture);

	if ( !zclip )
	    draw_envbox_face(0.0f, 1.0f, -w, w, w, 0.0f, 0.0f, +w, w, w, 1.0f, 0.0f,
	                     +w, -w, w, 1.0f, 1.0f, -w, -w, w,
	                     sky[4] ? sky[4] : notexture);

	draw_envbox_face(0.0f, 1.0f, +w, w, -w, 0.0f, 0.0f, -w, w, -w, 1.0f, 0.0f,
	                 -w, -w, -w, 1.0f, 1.0f, +w, -w, -w,
	                 sky[5] ? sky[5] : notexture);
...
static void committmufunc ( bool rgb, tmufunc &dst, tmufunc &src )
{
	if ( dst.combine != src.combine )
	    glTexEnvi(GL_TEXTURE_ENV,
	              rgb ? GL_COMBINE_RGB_ARB : GL_COMBINE_ALPHA_ARB, src.combine);
	loopi(3)
	{
...
{
	uchar *dstp = (uchar *) dst->pixels + y * dst->pitch + x * 4, *srcp =
	        (uchar *) src->pixels;
	int dstpitch = dst->pitch - 4 * src->w, srcpitch = src->pitch - 4 * src->w;
	loop(dy, src->h)
	{
		loop(dx, src->w)
		{
			uint k1 = ( 255U - srcp[3] ) * dstp[3], k2 = srcp[3] * 255U, kmax =
			        max(dstp[3], srcp[3]), kscale = max(kmax * 255U, 1U);
