...
static vector<char *> packagedirs;

static char *makerelpath ( const char *dir,
                           const char *file,
                           const char *prefix,
                           const char *cmd )
{
		...
		for ( char *prevdir = NULL, *curdir = s;; ) {
			...
			if ( prevdir + 1 == curdir && prevdir[0] == '.' ) {
				...
				curdir = prevdir;
			        ...
			        == '.'&& curdir[2]=='.' && curdir[3]==PATHDIV ) {
				if ( prevdir + 2 == curdir && prevdir[0] == '.'
				     && prevdir[1] == '.' ) continue;
...
const char *findfile ( const char *filename, const char *mode )
{
	...
	if ( mode[0] == 'w' || mode[0] == 'a' ) {   // create missing directories, if necessary
		string dirs;
		copystring(dirs, s);
...
static const int64_t MINFSSIZE = 50000000;         // 50MB
#endif
...
struct filestream : stream
{
	...
	bool open ( const char *name, const char *mode )
	{
		...
		if ( file && strchr(mode, 'w') ) {
			...
			if ( fail || (int64_t) buf.f_frsize * (int64_t) buf.f_bavail
			        < MINFSSIZE ) {
				close();
				return false;
			}
...
struct gzstream : stream
{
	enum
	{
		MAGIC1 = 0x1F, MAGIC2 = 0x8B, BUFSIZE = 16384, OS_UNIX = 0x03
	};
	...
	gzstream () :
			file(NULL), buf(NULL), reading(false), writing(false),
			        autoclose(false), crc(0), headersize(0)
	{
		zfile.zalloc = NULL;
		zfile.zfree = NULL;
		zfile.opaque = NULL;
	...
	{
		uchar header[] =
		        { MAGIC1, MAGIC2, Z_DEFLATED, 0, 0, 0, 0, 0, 0, OS_UNIX };
	...
	bool open ( stream *f, const char *mode, bool needclose, int level )
	{
		...
		if ( reading ) {
			if ( inflateInit2(&zfile, -MAX_WBITS) != Z_OK ) reading = false;
		        ...
		        && deflateInit2(&zfile, level, Z_DEFLATED, -MAX_WBITS, min(MAX_MEM_LEVEL, 8), Z_DEFAULT_STRATEGY) != Z_OK )
		    writing = false;
	...
	void finishreading ()
	{
		...
		if ( dbggz ) {
			...
			if ( checkcrc != crc )
			    conoutf("gzip crc check failed: read %X, calculated %X",
			            checkcrc, crc);
			if ( checksize != zfile.total_out )
			    conoutf("gzip size check failed: read %d, calculated %d",
			            checksize, zfile.total_out);
		}
#endif
	}
	...
	void finishwriting ()
	{
		...
		uchar trailer[8] = { uchar(crc & 0xFF), uchar( ( crc >> 8 ) & 0xFF),
		                     uchar( ( crc >> 16 ) & 0xFF), uchar(
		                             ( crc >> 24 ) & 0xFF),
		                     uchar(zfile.total_in & 0xFF), uchar(
		                             ( zfile.total_in >> 8 ) & 0xFF),
		                     uchar( ( zfile.total_in >> 16 ) & 0xFF), uchar(
		                             ( zfile.total_in >> 24 ) & 0xFF) };
	...
	bool flush ()
	{
		if ( zfile.next_out && zfile.avail_out < BUFSIZE ) {
			if ( file->write(buf, BUFSIZE - zfile.avail_out) != int(
			        BUFSIZE - zfile.avail_out) ) return false;
		}
		zfile.next_out = buf;
