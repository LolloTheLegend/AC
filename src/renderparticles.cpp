...
static void subdivide ( int depth, int face )
{
	...
	int idx[6];
	loopi(3)
		idx[i] = hemiindices[face + i];
	loopi(3)
	{
		int vert = heminumverts++;
		hemiverts[vert] = vec(hemiverts[idx[i]]).add(
		        hemiverts[idx[ ( i + 1 ) % 3]]).normalize();   //push on to unit sphere
		idx[3 + i] = vert;
...
static GLuint createexpmodtex ( int size, float minval )
{
	...
	loop(y, size)
		loop(x, size)
		{
			float dx = 2 * float(x) / ( size - 1 ) - 1, dy = 2 * float(y)
			        / ( size - 1 )
			                                                 - 1;
...
static struct parttype
{
	...
	int gr, tex;
	float sz;
} parttypes[] = { { PT_PART, 0.4f, 0.4f, 0.4f, 2, 0, 0.06f },   // yellow: sparks
		...
		if ( typemask & ( 1 << parttypes[i].type ) && parlist[i] ) {
			...
			for ( particle *p, **pp = &parlist[i]; ( p = *pp ); ) {
				switch ( pt.type )
				{
					case PT_HUDFLASH:
					case PT_FLASH: {
						vec corners[4] = { vec(-camright.x + camup.x,
						                       -camright.y + camup.y,
						                       -camright.z + camup.z),
						                   vec(camright.x + camup.x,
						                       camright.y + camup.y,
						                       camright.z + camup.z),
						                   vec(camright.x - camup.x,
						                       camright.y - camup.y,
						                   ...
						                   vec(-camright.x - camup.x,
						                       ...
						                       -camright.z - camup.z) };
						loopk(4)
							corners[k].rotate(p->d.x, p->d.y, camdir).mul(
							        sz * p->d.z).add(p->o);
					...
					case PT_FIREBALL: {
						sz = 1.0f + ( pt.sz - 1.0f )
						        * min(p->fade, lastmillis - p->millis)
						            / p->fade;
						glPushMatrix();
						...
						if ( reflecting && !refracting )
						    oc.z = p->o.z - ( hdr.waterlevel - 0.3f );
						glRotatef(
						        inside ? camera1->yaw - 180 :
						                 atan2(oc.y, oc.x) / RAD - 90,
						        0, 0, 1);
						glRotatef(
						        ( inside ? camera1->pitch :
						                   asin(oc.z / oc.magnitude()) / RAD )
						        - 90,
						        1, 0, 0);

						glRotatef(lastmillis / 7.0f, 0, 0, 1);
						glScalef(-sz, sz, -sz);
						drawexplosion(inside, pt.r, pt.g, pt.b,
						              1.0f - sz / pt.sz);
						glPopMatrix();
					...
					case PT_DECAL: {
						...
						glColor4f(
						        ...
						        s->b / 127.5,
						        max(0.0f,
						            min( ( p->millis + p->fade - lastmillis ) / 1000.0f,
						                0.7f)));
					...
					case PT_BULLETHOLE: {
						float blend =
						        max(0.0f,
						            min( ( p->millis + p->fade - lastmillis ) / 1000.0f,
						                1.0f));
						glColor4f(pt.r * blend, pt.g * blend, pt.b * blend,
						          blend);
						vec dx(0, 0, 0), dy(0, 0, 0);
						int tx = 0, ty = 1;
						loopk(3)
					...
					case PT_BLOOD: {
						int n = detrnd((size_t )p, 4), o = detrnd((size_t )p,
						                                          11);
						;
						float tx = 0.5f * ( n & 1 ), ty = 0.5f
						        * ( ( n >> 1 ) & 1 ), tsz = 0.5f;
						static const int tc[4][2] = { { 0, 1 }, { 1, 1 },
						                              { 1, 0 }, { 0, 0 } };
						glTexCoord2f(tx + tsz * tc[o % 4][0],
						             ty + tsz * tc[o % 4][1]);
						glVertex3f(p->o.x + ( -camright.x + camup.x ) * sz,
						           p->o.y + ( -camright.y + camup.y ) * sz,
						           p->o.z + ( -camright.z + camup.z ) * sz);
				...
				if ( time < 0 )
					...
					*pp = p->next;
					...
					if ( pt.gr )
					    p->o.z -= ( ( lastmillis - p->millis ) / 3.0f ) * time
					            / ( pt.gr * 10000 );
					...
					if ( pt.type == PT_BLOOD ) {
						...
						if ( s->type == SPACE && p->o.z <= s->floor ) {
							*pp = p->next;
							p->next = parempty;
							parempty = p;
							newparticle(vec(p->o.x, p->o.y, s->floor + 0.005f),
							            vec(0, 0, 1), bloodttl, 8);
							continue;
						}
					}
					pp = &p->next;
...
void particle_emit ( int type, int *args, int basetime, int seed, const vec &p )
{
	...
	if ( pt.type == PT_FIREBALL )
		...
		if ( lastmillis - basetime < args[0] )
		    particle_flash(type, args[1] > 0 ? args[1] / 100.0f : 1.0f,
		                   seed % 360, p);
	}
	else
...
void particle_cube ( int type, int num, int fade, int x, int y )
{
	...
	const float floor = s->floor
	        - ( s->type == FHF ? float(s->vdelta) / 4 : 0 );
	if ( SOLID(s) || fc < 1 ) return;
	loopi(num * fc / 12)
	{
		vec p(float(rnd(100)) / 100.0 + x, float(rnd(100)) / 100.0 + y,
		      float(rnd(100) * fc) / 100 + floor), d(0, 0, 0);
		newparticle(p, d, rnd(fade * 3), type);
	}
}
...
void particle_trail ( int type, int fade, const vec &s, const vec &e )
{
	...
	{
		p.add(v);
		vec tmp((float) ( rnd(11) - 5 ), (float) ( rnd(11) - 5 ),
		        (float) ( rnd(11) - 5 ));
...
{
	newparticle(o, vec(0, 0, 0),
	            (int) ( ( parttypes[type].sz - 1.0f ) * 100.0f ), type);
}
                     ...
                     int type )
{
	...
	if ( surface.iszero() || ( radius > 0
	        && ( dist < mag - radius || dist > mag + radius ) ) ) return false;
	vec o(from);
	o.add(ray.mul(dist));
	...
	if ( noisy && bulletbouncesound && bulletbouncesoundrad && d != player1
		...
		audiomgr.playsound(
		        o.z < hdr.waterlevel ? S_BULLETWATERHIT : S_BULLETHIT, &o,
		        SP_LOW);
	}
	return true;
}
...
void addshotline ( dynent *pl, const vec &from, const vec &to )
{
	...
	bool fx = shotlinettl > 0
	        && ( player1->isspectating() || !multiplayer(false) );   // allow fx only in spect mode and locally
	...
	if ( fd <= (float) bulletairsoundsourcerad || td
	        <= (float) bulletairsounddestrad ) return;   // ignore nearly fired/detonated shots
	vec soundpos(unitv);
