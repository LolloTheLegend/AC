...
static void render_seg_new ( float vx,
                             float vy,
                             float vh,
                             int mip,
                             int x,
                             ...
                             int ys )
{
	...
	for ( int oy = y; oy < ys; oy++ )   // first collect occlusion information for this block
		...
		for ( int ox = x; ox < xs; ox++, s++ )
			s->occluded = isoccluded(camera1->o.x, camera1->o.y,
			                         (float) ( ox << mip ),
			                         (float) ( oy << mip ), fsize);
	}
...
#define LOOPD sqr *t = SWS(s,1,0,mfactor); \
	...
	int rendered = 0;
	LOOPH   // floors
				if ( s->defer && mip && xx >= lx && xx < rx && yy >= ly
				     && yy < ry ) {
					...
					render_seg_new(vx, vy, vh, mip - 1, start * 2, yy * 2,
					               xx * 2 + 2, yy * 2 + 2);
					continue;
				}
				rendered++;
				switch ( s->type )
				{
					...
					case CHF:
						if ( s->floor <= vh && render_floor ) {
							sqr *v = SWS(s, 0, 1, mfactor);
							render_flat(s->ftex, xx << mip, yy << mip, 1 << mip,
							            s->floor, s, s + 1, v + 1, v, false);
							if ( s->floor < hdr.waterlevel && !reflecting )
							    addwaterquad(xx << mip, yy << mip, 1 << mip);
						}
						break;
					case FHF: {
						LOOPD
						render_flatdelta(s->ftex, xx << mip, yy << mip,
						                 1 << mip, df(s), df(t), df(u), df(v),
						                 s, t, u, v, false);
						if ( s->floor - s->vdelta / 4.0f < hdr.waterlevel && !reflecting )
						    addwaterquad(xx << mip, yy << mip, 1 << mip);
						break;
					}
				}
				...
				switch ( s->type )
				{
					...
					case FHF:
						if ( s->ceil >= vh && render_ceil ) {
							sqr *v = SWS(s, 0, 1, mfactor);
							render_flat(s->ctex, xx << mip, yy << mip, 1 << mip,
							            s->ceil, s, s + 1, v + 1, v, true);
						}
						break;
					case CHF: {
						LOOPD
						render_flatdelta(s->ctex, xx << mip, yy << mip,
						                 1 << mip, dc(s), dc(t), dc(u), dc(v),
						                 s, t, u, v, true);
						break;
					}
				}
	...
	LOOPH   // walls
				...
				if ( s->type == CORNER ) {
					...
					if ( SOLID(z) ) {
						if ( SOLID(w) ) {
							render_wall(w, h2 = s, xx + 1, yy, xx, yy + 1, mip,
							            t, v, false, 4);
							topleft = false;
						}
						else if ( SOLID(v) ) {
							render_wall(v, h2 = s, xx, yy, xx + 1, yy + 1, mip,
							            s, u, false, 5);
						}
					}
					else if ( SOLID(t) ) {
						if ( SOLID(w) ) {
							render_wall(w, h1 = s, xx + 1, yy + 1, xx, yy, mip,
							            u, s, false, 6);
						}
						else if ( SOLID(v) ) {
							render_wall(v, h1 = s, xx, yy + 1, xx + 1, yy, mip,
							            v, t, false, 7);
							topleft = false;
						}
					}
						...
						if ( z->ceil - z->floor < t->ceil - t->floor ) {
							if ( wv ) {
								render_wall(h1 = s, h2 = v, xx + 1, yy, xx,
								            yy + 1, mip, t, v, false, 4);
								topleft = false;
							}
							else {
								render_wall(h1 = s, h2 = w, xx, yy, xx + 1,
								            yy + 1, mip, s, u, false, 5);
							}
						}
						else {
							if ( wv ) {
					...
					render_tris(xx << mip, yy << mip, 1 << mip, topleft, h1, h2,
					            s, t, u, v);
				}
				...
				if ( normalwall ) {
					if ( xx >= vxx && xx != 0
					     ...
					     && ( z->type != SEMISOLID || issemi(mip, xx - 1, yy, 1,
					                                         0, 1, 1) ) )
					...
					if ( xx <= vxx && xx != sz - 1
					     ...
					     && ( t->type != SEMISOLID || issemi(mip, xx + 1, yy, 0,
					                                         0, 0, 1) ) )
					    render_wall(s, t, xx + 1, yy, xx + 1, yy + 1, mip, t, u,
					                false, 1);   // right
					if ( yy >= vyy && yy != 0
					     && !SOLID(w)
					     && ( !SOLID(s) || w->type != CORNER )
					     && ( w->type != SEMISOLID || issemi(mip, xx, yy - 1, 0,
					                                         1, 1, 1) ) )
					...
					if ( yy <= vyy && yy != sz - 1
					     ...
					     && ( v->type != SEMISOLID || issemi(mip, xx, yy + 1, 0,
					                                         0, 1, 0) ) )
					    render_wall(s, v, xx, yy + 1, xx + 1, yy + 1, mip, v, u,
					                true, 3);   // bot
				}
			}
		}
	...
	if ( editmode && !showtagclipfocus && showtagclips ) LOOPH   // tag clips
				if ( mip ) {
					int ix1 = xx << mip, ix2 = ( xx + 1 ) << mip, iy1 = yy
					        << mip, iy2 = ( yy + 1 ) << mip;
					sqr *tt = S(ix1, iy1), *t;
                    ...
                    int h )
{
	loopi(LARGEST_FACTOR)
		stats[i] = 0;
	min_lod =
	        minimap || ( player1->isspectating()
	                && player1->spectatemode == SM_OVERVIEW ) ?
	                MAX_LOD : MIN_LOD + abs(pitch) / 12;
	yaw = 360 - yaw;
	...
	{
		min_lod = max(min_lod,
		              (int) ( MIN_LOD + ( 10 - cdist ) / 1.0f * widef ));   // less if lod worked better
		widef = 1.0f;
	}
