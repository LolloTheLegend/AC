...
{
	const char *mapstartonce = getalias("mapstartonce"), *onquit = getalias(
	        "onQuit");
...
const char *screenshotpath ( const char *imagepath, const char *suffix )
{
	...
	if ( imagepath && imagepath[0] )
		copystring(buf, imagepath);
	else {
		if ( getclientmap()[0] )
			formatstring(buf)(
			        "screenshots/%s_%s_%s.%s", timestring(), behindpath(getclientmap()), modestr(gamemode, true), suffix);
			else
...
void bmp_screenshot ( const char *imagepath, bool mapshot = false )
{
	...
	if ( mapshot ) {
		...
		if ( screenshotscale != 1.0f ) {
			...
			glReadPixels(0, 0, tw, th, GL_RGB, GL_UNSIGNED_BYTE, tmp);
	}
	loopi(th)
	{
	...
	if ( !file )
		conoutf("failed to create: %s", filename);
	else {
...
void jpeg_screenshot ( const char *imagepath, bool mapshot = false )
{
	...
	if ( mapshot ) {
		...
		if ( screenshotscale != 1.0f ) {
			...
			glReadPixels(0, 0, tw, th, GL_BGR, GL_UNSIGNED_BYTE, pixels);
		...
		for ( int row = 0; row < th / 2; row++ ) {
			...
			memcpy(pixels + row * stride, pixels + ( th - row - 1 ) * stride,
			       stride);
...
void writepngchunk ( stream *f, const char *type, uchar *data = NULL, uint len =
        0 )
{
	f->putbig<uint>(len);
	f->write(type, 4);
...
void screenshot ( const char *imagepath )
{
	...
	{
		case 2:
			png_screenshot(imagepath, false);
			break;
		case 1:
			jpeg_screenshot(imagepath, false);
			break;
		case 0:
		default:
			bmp_screenshot(imagepath, false);
			break;
	}
}
...
void mapshot ()
{
	...
	{
		case 2:
			copystring(suffix, "png");
			break;
		case 1:
			copystring(suffix, "jpg");
			break;
		case 0:
		default:
			copystring(suffix, "bmp");
			break;
	}
	defformatstring(buf)("screenshots/mapshot_%s_%s.%s",
	                     behindpath(getclientmap()), timestring(), suffix);
	switch ( screenshottype )
	{
		case 2:
			png_screenshot(buf, true);
			break;
		case 1:
			jpeg_screenshot(buf, true);
			break;
		case 0:
		default:
			bmp_screenshot(buf, true);
			break;
	}
}
...
void screenres ( int w, int h )
{
	...
	SDL_Surface *surf = SDL_SetVideoMode(
	        w, h, 0,
	        SDL_OPENGL | SDL_RESIZABLE | ( screen->flags & SDL_FULLSCREEN ));
	if ( !surf ) return;
	screen = surf;
	scr_w = screen->w;
	scr_h = screen->h;
...
void setupscreen ( int &usedcolorbits, int &useddepthbits, int &usedfsaa )
{
	...
	bool hasbpp = true;
	if ( colorbits && modes )
	    hasbpp = SDL_VideoModeOK(
	            modes != (SDL_Rect **) -1 ? modes[0]->w : scr_w,
...
#endif
	static int configs[] = { 0x7, /* try everything */
	...
	if ( !screen )
		...
		if ( stencilbits && ( config & 2 ) == 0 )
		    conoutf("%d bit stencil buffer not supported - disabling",
		            stencilbits);
...
void resetgl ()
{
	...
	c2skeepalive();
	if ( !reloadtexture(*notexture) || !reloadtexture(
	        "packages/misc/startscreen.png") )
...
bool interceptkey ( int sym )
{
	...
	if ( sym != SDLK_UNKNOWN )
	    for ( int i = len; i < events.length(); i++ ) {
		    if ( events[i].type == SDL_KEYDOWN && events[i].key.keysym.sym
		            == sym ) {
			    events.remove(i);
			    return true;
		    }
...
#endif
		if ( event.motion.x == screen->w / 2 && event.motion.y
		        == screen->h / 2 ) return true;   // ignore any motion events generated SDL_WarpMouse
	}
#endif
	return false;
}
...
void checkinput ()
{
	...
	while ( events.length() || SDL_PollEvent(&event) ) {
		...
		switch ( event.type )
		{
			...
			case SDL_KEYUP:
				extern bool senst;
				if ( event.key.keysym.sym <= SDLK_5 && event.key.keysym.sym
				        >= SDLK_1
				     && senst ) {
...
void connectprotocol ( char *protocolstring,
                       string &servername,
                       int &serverport,
                       string &password,
                       bool &direct_connect )
{
	...
	if ( !len ) {
		conoutf("\f3bad commandline syntax", protocolstring);
		return;
	}
...
void initclientlog ()   // rotate old logfiles and create new one
{
	...
	if ( clientlogfile ) {
		if ( bootclientlog && bootclientlog->length() )
		    clientlogfile->write(bootclientlog->getbuf(),
		                         bootclientlog->length());
	}
	else
				...
				switch ( argv[i][1] )
				{
					case '-':
						if ( !strncmp(argv[i], "--home=", 7) ) {
							...
							conoutf("\f3unknown commandline switch: %s",
							        argv[i]);
						break;
					case 'd':
						dedicated = true;
					...
					case 'e':
						...
						break;
					default:
						conoutf("\f3unknown commandline option: -%c",
						        argv[i][1]);
				}
			else if ( !strncmp(argv[i], "assaultcube://", 14) )   // browser direct connection
			                   {
				connectprotocol(argv[i], servername, serverport, password,
				                direct_connect);
			}
			else
...
#define SDLVERSIONSTRING  STRINGIFY(SDL_MAJOR_VERSION) "." STRINGIFY(SDL_MINOR_VERSION) "." STRINGIFY(SDL_PATCHLEVEL)
	...
	if ( SDL_COMPILEDVERSION != SDL_VERSIONNUM(sdlver->major, sdlver->minor,
	                                           sdlver->patch) )
	    clientlogf(
	            "SDL: compiled version " SDLVERSIONSTRING ", linked version %u.%u.%u",
	...
	if ( !dedicated )
	    initlog("net (" STRINGIFY(ENET_VERSION_MAJOR) "." STRINGIFY(ENET_VERSION_MINOR) "." STRINGIFY(ENET_VERSION_PATCH) ")");
	...
	if ( !notexture )
	    fatal("could not find core textures (hint: run AssaultCube from the parent of the bin directory)");
	...
	extern void *scoremenu, *servmenu, *searchmenu, *serverinfomenu, *kickmenu,
	        *banmenu, *forceteammenu, *giveadminmenu, *docmenu, *applymenu,
	        *downloaddemomenu;
	scoremenu = addmenu("score", "columns", false, renderscores, NULL, false,
	                    true);
	servmenu = addmenu("server", NULL, true, refreshservers, serverskey);
	searchmenu = addmenu("search", NULL, true, refreshservers, serverskey);
	serverinfomenu = addmenu("serverinfo", NULL, true, refreshservers,
	                         serverinfokey);
	...
	static char resdata[] = { 112, 97, 99, 107, 97, 103, 101, 115, 47, 116, 101,
	                          120, 116, 117, 114, 101, 115, 47, 107, 117, 114,
	                          116, 47, 107, 108, 105, 116, 101, 50, 46, 106,
	                          112, 103, 0 };
	...
	int lastflush = 0;
#endif
	if ( scl.logident[0] ) {   // "-Nxxx" sets the number of client log lines to xxx (after init)
