...
vec Normalize ( vec v );
inline void makevec ( vec *v, float x, float y, float z )
{
	v->x = x;
	v->y = y;
	v->z = z;
}
inline bool UnderWater ( const vec &o )
{
	return hdr.waterlevel > o.z - 0.5f;
}
inline bool InWater ( const vec &o )
{
	return hdr.waterlevel >= o.z;
}
...
template<class C>
	...
	public:
		...
		void DeleteEntry ( C Entry )
		{
			...
			{
				if ( pNodeList == pLastNode ) pLastNode = NULL;

				pNodeList = pNodeList->next;
				if ( pNodeList ) pNodeList->prev = NULL;
				pNode->next = NULL;
				delete pNode;
				pNode = NULL;
				iNodeCount--;
		...
		void DeleteNode ( node_s *pNode )
		{
			...
			{
				if ( pNodeList == pLastNode ) pLastNode = pNodeList->next;
		...
		node_s *SearchNode ( C Entry )
		{
			node_s *pNode = pNodeList;
			while ( pNode ) {
				if ( pNode->Entry == Entry ) return pNode;
				pNode = pNode->next;
			}
			return NULL;
		}
		...
		C Pop ( void )
		{
			if ( !pNodeList ) return static_cast<C>(NULL);
...
template<class C, class D = int>
	...
	public:
		struct node_s
		{
			...
			node_s ( C Ent, D Prior ) :
					Entry(Ent), Priority(Prior), next(NULL)
			{
			}
			;
		};
		...
		C Pop ( void )
		{
			if ( !pHeadNode ) return static_cast<C>(NULL);
		...
		bool IsInList ( C Entry, D Prior )
		{
			node_s *pNode = pHeadNode;
			while ( pNode ) {
				if ( pNode->Entry == Entry ) return true;

				if ( Prior < pNode->Priority ) return false;
	...
	private:
		...
		node_s *GetPrevNode ( node_s *pNode )
		{
			...
			while ( pTemp ) {
				if ( pTemp->next == pNode ) return pTemp;
				pTemp = pTemp->next;
			}
		...
		void DeleteNode ( node_s *pNode )
		{
			...
			{
				if ( pHeadNode == pLastNode ) pLastNode = pHeadNode->next;
...
template<class C>
	...
	public:
		...
		bool GetSelection ( C &Var )
		{
			...
			typename TLinkedList<SMultiChoice*>::node_s *pNode = pChoiceList
			    ->GetFirst();

			while ( pNode ) {
				if ( ( Choice >= pNode->Entry->MinVal ) && ( Choice
				        < pNode->Entry->MaxVal ) ) {
