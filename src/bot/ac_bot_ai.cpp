...
bool CACBot::ChoosePreferredWeapon ()
{
	...
	for ( int i = 0; i < MAX_WEAPONS; i++ ) {
		...
		if ( !m_pMyEnt->mag[i] && WeaponInfoTable[i].eWeaponType
		        != TYPE_MELEE ) {
			continue;
		}
		else {
		...
		if ( ( flDist >= WeaponInfoTable[i].flMinDesiredDistance ) && ( flDist
		        <= WeaponInfoTable[i].flMaxDesiredDistance ) ) {
			// In desired range for this weapon
			sWeaponScore += 5;   // Increase score much

			if ( i == GUN_PISTOL || WeaponInfoTable[i].eWeaponType
			        == TYPE_MELEE ) {
				...
				if ( WeaponInfoTable[i].eWeaponType == TYPE_MELEE && WeaponInfoTable[m_pMyEnt
				                                                         ->primary]
				                                                         .eWeaponType
				                                                     == TYPE_SHOTGUN ) {
		...
		if ( flIdealDiff < 0.5f )   // Close to ideal distance
			sWeaponScore += 4;
		else if ( flIdealDiff <= 1.0f ) sWeaponScore += 2;
		...
		if ( WeaponInfoTable[i].sMinDesiredAmmo > 0 ) {
			...
			if ( flDesiredPercent >= 400.0f )
				...
				sWeaponScore += 3;
			else if ( flDesiredPercent >= 100.0f ) sWeaponScore += 1;
		}
		else {
...
entity *CACBot::SearchForEnts ( bool bUseWPs, float flRange, float flMaxHeight )
{
	...
	if ( ( WaypointClass.m_iWaypointCount >= 1 ) && bUseWPs ) pWptNearBot =
	        GetNearestWaypoint(15.0f);

#ifdef WP_FLOOD
	if ( !pWptNearBot && bUseWPs ) pWptNearBot = GetNearestFloodWP(5.0f);
#endif
	...
	loopv(ents)
	{
		...
		if ( !bInteresting ) continue;   // Not an interesting item, skip
	...
	if ( pNewTargetEnt ) {
		...
		m_vGoal.z = S(pNewTargetEnt->x, pNewTargetEnt->y)->floor
		        + player1->eyeheight;
	}
