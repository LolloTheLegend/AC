...
CWaypointClass::CWaypointClass ( void ) :
		m_bDrawWaypoints(false), m_bDrawWPPaths(true), m_bAutoWaypoint(false),
		        m_bAutoPlacePaths(true), m_bDrawWPText(false),
		        m_vLastCreatedWP(g_vecZero), m_bFlooding(false),
		        m_bFilteringNodes(false), m_iFloodStartTime(0), m_iCurFloodX(0),
		        m_iCurFloodY(0), m_iFloodSize(0), m_iFilteredNodes(0),
		        m_iWaypointCount(0)
{
	loopi(MAX_MAP_GRIDS)
	{
...
bool CWaypointClass::LoadWaypoints ()
{
			...
			if ( strcmp(header.szMapName, m_szMapName) == 0 ) {
				...
				if ( header.iFileVersion == 1 ) {
					...
					for ( i = 0; i < header.iWPCount; i++ ) {
						...
						node_s *pNode = new node_s(WPs[i].v_origin,
						                           WPs[i].iFlags, 0);
					...
					for ( i = 0; i < m_iWaypointCount; i++ ) {
						...
						for ( j = 0; j < num; j++ ) {
							...
							node_s *pTo = GetWaypointFromVec(
							        WPs[path_index].v_origin);
					...
					for ( i = 0; i < header.iWPCount; i++ ) {
						...
						if ( ( BotManager.m_sCurrentTriggerNr == -1 ) || ( triggernr
						        < BotManager.m_sCurrentTriggerNr ) )
					...
					for ( i = 0; i < header.iWPCount; i++ ) {
						...
						if ( !pCurrent ) {
							...
							for ( j = 0; j < num; j++ )
								bfp->read(&to, sizeof ( to ));   // Read rest of block
							continue;
						}
				...
				conoutf("Waypoints aren't for map %s but for map %s",
				        m_szMapName, header.szMapName);
				delete bfp;
				return false;
			}
		}
	...
	if ( BotManager.m_sCurrentTriggerNr == -1 ) BotManager.m_sCurrentTriggerNr =
	        0;
	...
	loopi(MAX_MAP_GRIDS)
	{
		loopj(MAX_MAP_GRIDS)
		{
			...
			while ( p ) {
				...
				bfp->write(&p->Entry->sTriggerNr,
				           sizeof ( p->Entry->sTriggerNr ));   // Write trigger nr
	...
	loopi(MAX_MAP_GRIDS)
	{
		loopj(MAX_MAP_GRIDS)
		{
			...
			while ( p ) {
				...
				while ( pPath != NULL ) {
					bfp->write(&pPath->Entry->v_origin,
					           sizeof ( pPath->Entry->v_origin ));
...
bool CWaypointClass::LoadWPExpFile ()
{
	...
	if ( m_iWaypointCount == 0 ) return false;
	...
	if ( bfp != NULL ) {
		...
		if ( strcmp(header.szFileType, "cube_bot") == 0 ) {
			...
			if ( strcmp(header.szMapName, m_szMapName) == 0 ) {
				...
				conoutf("Waypoint experience file isn't for map %s but for map %s",
				        ( m_szMapName ), ( header.szMapName ));
				fclose(bfp);
				return false;
			}
...
void CWaypointClass::SaveWPExpFile ()
{
	...
	loopi(MAX_MAP_GRIDS)
	{
		loopj(MAX_MAP_GRIDS)
		{
			...
			while ( p ) {
				// count the number of paths from this node...
				TLinkedList<node_s *>::node_s *p2 = p->Entry->FailedGoalList
				    .GetFirst();
...
void CWaypointClass::Think ()
{
	...
	if ( m_bAutoWaypoint )   // is auto waypoint on?
	{
		...
		bool NoLastCreatedWP = ( ( m_vLastCreatedWP.x == 0 )
		        && ( m_vLastCreatedWP.y == 0 ) && ( m_vLastCreatedWP.z == 0 ) );
...
#ifdef WP_FLOOD
		if ( !nearestwp ) nearestwp = GetNearestFloodWP(player1, 10.0f);
#endif

		if ( nearestwp ) {
			char szWPInfo[256];
			sprintf(szWPInfo, "Distance nearest waypoint: %f",
			        GetDistance(player1->o, nearestwp->v_origin));
			...
			if ( nearestwp->iFlags & W_FL_JUMP ) strcat(szWPInfo, "Jump ");
			...
			if ( strlen(szWPInfo) == 7 ) strcat(szWPInfo, "None");
...
void CWaypointClass::DrawNearWaypoints ()
{
	...
	short i, j, MinI, MaxI, MinJ, MaxJ, Offset = (short) ceilf(
	        15.0f / MAX_MAP_GRIDS);
	...
	if ( MinI < 0 ) MinI = 0;
	if ( MaxI > MAX_MAP_GRIDS - 1 ) MaxI = MAX_MAP_GRIDS - 1;
	if ( MinJ < 0 ) MinJ = 0;
	if ( MaxJ > MAX_MAP_GRIDS - 1 ) MaxJ = MAX_MAP_GRIDS - 1;

	node_s *nearestwp = WaypointClass.GetNearestWaypoint(curselection, 20.0f);

#ifdef WP_FLOOD
	if ( !nearestwp ) nearestwp = GetNearestFloodWP(player1, 20.0f);
#endif
		...
		for ( int y = MinJ; y <= MaxJ; y++ ) {
			...
			while ( pNode ) {
				...
				if ( flDist <= 15 ) {
					...
					line(int(o.x), int(o.y), int(o.z), int(e.x), int(e.y),
					     int(e.z));
	...
	if ( pNearest ) {
		...
		if ( pNearest->sYaw != -1 ) {
			...
			line(int(from.x), int(from.y), int(from.z), int(end.x), int(end.y),
			     int(end.z));
		}
	}
	...
	if ( m_bDrawWPPaths ) {
		...
		if ( bots.length() && bots[0] && bots[0]->pBot
		     && bots[0]->pBot->m_pCurrentWaypoint
		     && bots[0]->pBot->m_pCurrentGoalWaypoint ) {
			CBot *pB = bots[0]->pBot;
			if ( !pB->m_bCalculatingAStarPath && !pB->m_AStarNodeList.Empty() ) {
				TLinkedList<waypoint_s *>::node_s *pNode = pB->m_AStarNodeList
				    .GetFirst(), *pNext;
				...
				while ( pNode && pNode->next ) {
					...
					line(int(v1.x), int(v1.y), int(v1.z), int(v2.x), int(v2.y),
					     int(v2.z));
...
void CWaypointClass::ManuallyCreatePath ( vec v_src, int iCmd, bool TwoWay )
{
	...
	if ( iCmd == 2 )   // assign dest of path and make path
	{
		...
		if ( TwoWay ) AddPath(waypoint2, waypoint1);
	}
}
...
void CWaypointClass::ManuallyDeletePath ( vec v_src, int iCmd, bool TwoWay )
{
	...
	if ( iCmd == 2 )   // assign dest of path and delete path
	{
		...
		if ( TwoWay ) DeletePath(waypoint2, waypoint1);
	}
}
...
node_s *CWaypointClass::GetNearestWaypoint ( vec v_src, float flRange )
{
	TLinkedList<node_s *>::node_s *pNode;
	node_s *pNearest = NULL;
	short i, j, MinI, MaxI, MinJ, MaxJ, Offset = (short) ceil(
	        flRange / MAX_MAP_GRIDS);
	float flNearestDist = 9999.99f, flDist;
...
node_s *CWaypointClass::GetNearestTriggerWaypoint ( vec v_src, float flRange )
{
	...
	for ( int x = MinI; x <= MaxI; x++ ) {
		for ( int y = MinJ; y <= MaxJ; y++ ) {
			...
			while ( pNode ) {
				if ( ( pNode->Entry->iFlags & W_FL_FLOOD ) || ! ( pNode->Entry
				                                                      ->iFlags
				                                                  & W_FL_TRIGGER ) ) {
					pNode = pNode->next;
					continue;
...
node_s *CWaypointClass::GetWaypointFromVec ( const vec &v_src )
{
	...
	while ( pNode ) {
		if ( pNode->Entry->v_origin == v_src ) return pNode->Entry;
...
void CWaypointClass::FloodThink ()
{
	...
	if ( !m_bFilteringNodes ) {
		...
		for ( x = m_iCurFloodX; x < ( ssize - MINBORD ); x += 4 ) {
			...
			for ( y = MINBORD; y < ( ssize - MINBORD ); y += 4 ) {
				...
				if ( S((int)from.x, (int)from.y)->tag ) flags |=
				        W_FL_INTAG;
	...
	for ( x = m_iCurFloodX; x < MAX_MAP_GRIDS; x++ ) {
		if ( count > 3 ) {
			...
			AddScreenText("adding costs... %.2f %%",
			              ( (float) x / float(MAX_MAP_GRIDS) ) * 100.0f);
			m_iCurFloodX = x;
			return;
		}
		...
		for ( y = 0; y < MAX_MAP_GRIDS; y++ ) {
			TLinkedList<node_s *>::node_s *pNode = m_Waypoints[x][y].GetFirst(),
			        *pTemp;
			while ( pNode ) {
				if ( pNode->Entry->ConnectedWPs.Empty() || pNode->Entry
				         ->ConnectedWPsWithMe.Empty() ) {
	...
	conoutf("Added %d wps in %d milliseconds", m_iWaypointCount,
	        SDL_GetTicks() - m_iFloodStartTime);
...
node_s *CWaypointClass::GetNearestFloodWP ( vec v_origin,
                                            float flRange,
                                            node_s *pIgnore,
                                            bool SkipTags )
{
	...
	for ( int x = MinI; x <= MaxI; x++ ) {
		for ( int y = MinJ; y <= MaxJ; y++ ) {
			...
			while ( p ) {
				if ( ( p->Entry == pIgnore ) || ( ! ( p->Entry->iFlags
				        & W_FL_FLOOD ) ) ) {
					p = p->next;
					continue;
				}
...
void CWaypointClass::GetNodeIndexes ( const vec &v_origin, short *i, short *j )
{
	...
	*j = abs((int) ( ( v_origin.y ) / MAX_MAP_GRIDS ));
	...
	if ( *i > MAX_MAP_GRIDS - 1 ) *i = MAX_MAP_GRIDS - 1;
	if ( *j > MAX_MAP_GRIDS - 1 ) *j = MAX_MAP_GRIDS - 1;
}
...
void setdebuggoal ( void )
{
	v_debuggoal = player1->o;
}
;
...
bool CBot::FindWaypoint ()
{
	...
	TLinkedList<node_s *>::node_s *pNode = m_pCurrentWaypoint->pNode
	    ->ConnectedWPs.GetFirst();

	while ( pNode ) {
		if ( ( pNode->Entry->iFlags & W_FL_INTAG ) && SOLID(
		        S((int)pNode->Entry->v_origin.x, (int)pNode->Entry->v_origin.y)) ) {
			pNode = pNode->next;
			continue;
		}
		...
		if ( ( wp != m_pCurrentWaypoint ) && ( wp != m_pPrevWaypoints[0] )
		     && ( wp != m_pPrevWaypoints[1] ) && ( wp != m_pPrevWaypoints[2] )
...
{
	if ( !m_pCurrentWaypoint ) return false;   // Can't head to waypoint
	...
	// are we close enough to a target waypoint...
		...
		if ( !m_pCurrentGoalWaypoint || ( m_pCurrentWaypoint
		        != m_pCurrentGoalWaypoint )
		     || IsVisible(m_vGoal) || ( WPDist <= 1.5f ) ) Touching = true;
		// If the bot has a goal check if he can see his next wp
		if ( m_pCurrentGoalWaypoint && ( m_pCurrentGoalWaypoint
		        != m_pCurrentWaypoint )
	...
	if ( Touching ) {
		...
		if ( m_pCurrentWaypoint->pNode->sYaw != -1 ) {
			// UNDONE: Inhuman
			m_pMyEnt->yaw = m_pMyEnt->targetyaw = m_pCurrentWaypoint->pNode
			    ->sYaw;
		}
	...
	if ( m_fYawToTurn <= 25.0f ) m_iWaypointHeadLastTurnLessTime = lastmillis;

	// Bot had to turn much for a while?
	if ( ( m_iWaypointHeadLastTurnLessTime > 0 ) && ( m_iWaypointHeadLastTurnLessTime
	        < ( lastmillis - 1000 ) ) ) {
	...
	if ( m_iWaypointHeadPauseTime >= lastmillis ) {
		...
		float flEndDist;
		if ( !IsVisible(from, FORWARD, 3.0f, false, &flEndDist) && ( GetDistance(
		        from, m_pCurrentWaypoint->pNode->v_origin)
		                                                             > flEndDist ) ) {
			m_pMyEnt->jumpnext = true;
	...
	if ( m_pCurrentGoalWaypoint ) {
		if ( ReachedGoalWP() ) {
			...
			if ( CurrentWPIsValid() ) {
				...
				if ( !pWP || ( pWP == m_pCurrentWaypoint ) ) return false;
...
bool CBot::AStar ()
{
	...
	if ( !m_bCalculatingAStarPath ) {
		if ( ( BotManager.m_sUsingAStarBotsCount + 1 ) > BotManager
		         .m_sMaxAStarBots ) {
			return true;
		}
		...
		m_pCurrentGoalWaypoint->pParent[0] =
		        m_pCurrentGoalWaypoint->pParent[1] = NULL;
		...
		m_AStarOpenList[0].AddEntry(
		        m_pCurrentWaypoint,
		        GetDistance(m_pCurrentGoalWaypoint->pNode->v_origin));
		m_AStarOpenList[1].AddEntry(
		        m_pCurrentGoalWaypoint,
		        GetDistance(m_pCurrentGoalWaypoint->pNode->v_origin));

		m_pCurrentWaypoint->bIsOpen[0] = m_pCurrentGoalWaypoint->bIsOpen[1] =
		        true;
		m_pCurrentWaypoint->bIsOpen[1] = m_pCurrentGoalWaypoint->bIsOpen[0] =
		        false;
		m_pCurrentWaypoint->bIsClosed[0] =
		        m_pCurrentGoalWaypoint->bIsClosed[1] = false;
		m_pCurrentWaypoint->bIsClosed[1] =
		        m_pCurrentGoalWaypoint->bIsClosed[0] = false;
	}
	...
	while ( !m_AStarOpenList[0].Empty() ) {
		...
		if ( n->pNode->FailedGoalList.IsInList(
		        m_pCurrentGoalWaypoint->pNode) ) {
			bPathFailed = true;
		...
		while ( pPath ) {
			if ( ( pPath->Entry->iFlags & W_FL_INTAG ) && SOLID(
			        S((int)pPath->Entry->v_origin.x, (int)pPath->Entry->v_origin.y)) ) {
				pPath = pPath->next;
				continue;
			}
...
{
	// UNDONE?
	return ( GetDistance(pWP1->pNode->v_origin, pWP2->pNode->v_origin)
	        * pWP2->pNode->sCost );
}
...
{
	if ( ( !m_pCurrentWaypoint ) || ( !m_pCurrentGoalWaypoint ) ) return false;
...
waypoint_s *CBot::GetWPFromNode ( node_s *pNode )
{
	...
	while ( p ) {
		if ( p->Entry->pNode == pNode ) return p->Entry;
...
#ifndef RELEASE_BUILD
				if ( !GetWPFromNode(p->Entry) )
				    condebug("Error adding bot wp!");
#endif
	...
	for ( int x = MinI; x <= MaxI; x++ ) {
		for ( int y = MinJ; y <= MaxJ; y++ ) {
			...
			while ( p ) {
				...
				if ( ( flDist < flNearestDist ) && ( flDist <= flRange ) ) {
					if ( ::IsVisible(v_origin, p->Entry->pNode->v_origin,
					                 NULL) ) {
						pNearest = p->Entry;
						flNearestDist = flDist;
					}
