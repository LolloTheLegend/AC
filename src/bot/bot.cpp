...
void CBot::Spawn ()
{
	...
	m_bShootAtFeet = ( RandomLong(1, 100)
	...
	m_iLastJumpPad = 0;
	m_pTargetEnt = NULL;
	while ( !m_UnreachableEnts.Empty() )
		delete m_UnreachableEnts.Pop();
...
void CBot::AimToVec ( const vec &o )
{
	...
	m_pMyEnt->targetyaw = -(float) atan2(o.x - m_pMyEnt->o.x,
	                                     o.y - m_pMyEnt->o.y)
	                      / PI
	                      * 180
	                      + 180;
...
{
	if ( m_iAimDelay > lastmillis ) return;
	...
	m_fPitchToTurn = fabs(
	        ( m_pMyEnt->targetpitch + RealXOffset ) - m_pMyEnt->pitch);
	m_fYawToTurn = fabs( ( m_pMyEnt->targetyaw + RealYOffset ) - m_pMyEnt->yaw);

	float flIdealPitch = ChangeAngle(AimXSpeed,
	                                 m_pMyEnt->targetpitch + RealXOffset,
	                                 m_pMyEnt->pitch);
	float flIdealYaw = ChangeAngle(AimYSpeed, m_pMyEnt->targetyaw + RealYOffset,
	                               m_pMyEnt->yaw);
...
float CBot::ChangeAngle ( float speed, float ideal, float current )
{
	...
	if ( diff < speed ) speed = diff;   // just need to turn a little bit (less than max)

	// check if the bot is already facing the idealpitch direction...
	if ( diff <= 0.5f ) return ideal;
...
bool CBot::SelectGun ( int Gun )
{
	...
	if ( m_pMyEnt->weaponsel->reloading || m_pMyEnt->weaponchanging )
	    return false;
                       ...
                       float *pEndDist )
{
	...
	if ( pEndDist ) *pEndDist = GetDistance(o, tr.end);
...
void CBot::SetMoveDir ( int iMoveDir, bool add )
{
	if ( iMoveDir & FORWARD )
		...
		m_pMyEnt->move = -1;
	else if ( !add ) m_pMyEnt->move = 0;

	if ( iMoveDir & LEFT )
		m_pMyEnt->strafe = 1;
	else if ( m_iMoveDir & RIGHT )
		m_pMyEnt->strafe = -1;
	else if ( !add ) m_pMyEnt->strafe = 0;

	if ( iMoveDir & UP ) m_pMyEnt->jumpnext = true;
}
