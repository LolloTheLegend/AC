                 ...
                 bool SkipTags )
{
	...
	if ( CheckPlayers ) {
		...
		loopv(players)
		{
			...
			if ( ( flDist < flNearestDist ) && ( intersect(d, from, to,
			                                               &tr->end) ) ) {
		...
		if ( d && ( d != pTracer ) && !BotManager.m_pBotToView
		     && ( d->state == CS_ALIVE ) ) {
			flDist = GetDistance(from, d->o);
	...
	if ( !steps )   // If from and to are on the same cube...
	{
		if ( GetDistance(from, to) < flNearestDist ) {
			...
			if ( solid || ( to.z < flr ) || ( to.z > s->ceil ) ) {
				tr->collided = true;
				tr->end.z =
				        ( fabs(to.z - s->ceil) < fabs(to.z - flr) ) ? s->ceil :
				                                                      flr;
			}
		}
		return;
...
vec PredictPos ( vec pos, vec vel, float Time )
{
	...
	if ( flVelLength <= 1.0 ) return pos;   // don't bother with low velocities...
...
{
	vec cross(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z,
	          a.x * b.y - a.y * b.x);
	return cross;
}
