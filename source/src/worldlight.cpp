                       ...
                       bool flicker = false )   // done in realtime, needs to be fast
{
	...
	if ( maxtmus ) {
		...
		if ( light.attr3 || light.attr4 )   // coloured light version, special case because most lights are white
		{
			if ( flicker ) {
				int dimness = rnd(
				        ( ( ( 255 << PRECBITS ) - ( int(light.attr2)
				                + int(light.attr3) + int(light.attr4) )
				                                  * fadescale
				                                  / 3 )
				          >> ( PRECBITS + 4 ) )
				        + 1);
...
static void calclightsource ( const persistent_entity &l,
                              float fade = 1,
                              bool flicker = true )
{
	int reach = l.attr1;
...
void fullbrightlight ( int level )
{
	...
	loopi(mipsize)
		world[i].r = world[i].g = world[i].b = level;
...
void calclight ()
{
	...
	uchar r = ( hdr.ambient >> 16 ) & 0xFF, g = ( hdr.ambient >> 8 ) & 0xFF, b =
	        hdr.ambient & 0xFF;
	if ( !r && !g ) {
		if ( !b ) b = 10;
		r = g = b;
	...
	block bb = { mapdims.x1 - 1, mapdims.y1 - 1, mapdims.xspan + 2, mapdims
	                                                                    .yspan
	                                                                + 2 };
	postlightarea(bb);
	setvar("fullbright", 0);
...
{
	block area = { (int) d.o.x - d.reach, (int) d.o.y - d.reach, d.reach * 2
	        + 1,
	               d.reach * 2 + 1 };
...
void dodynlights ()
{
	...
	{
		dlight &d = dlights[i];
		persistent_entity l((int) d.o.x, (int) d.o.y, (int) d.o.z, LIGHT,
		                    d.reach, d.r, d.g, d.b);
