...
enum
{
	NOT_INITING = 0, INIT_LOAD, INIT_RESET
};
enum
{
	CHANGE_GFX = 1 << 0, CHANGE_SOUND = 1 << 1
};

bool initwarning ( const char *desc, int level = INIT_RESET, int type =
        CHANGE_GFX );
...
enum
{
	M_FLAGGRAB = 0, M_LASTMINUTE1, M_LASTMINUTE2
};
...
enum
{
	SP_LOW = 0, SP_NORMAL, SP_HIGH, SP_HIGHEST
};
...
public:
	...
	enum worldobjtype
	{
		WR_CAMERA, WR_PHYSENT, WR_ENTITY, WR_STATICPOS
	};
	int type;

	worldobjreference ( int t ) :
			type(t)
	{
	}
	...
	virtual bool operator!= ( const worldobjreference &other )
	{
		return ! ( *this == other );
	}
	virtual void attach ()
...
struct locvector : vector<location *>
{
	...
	location *find ( int sound,
	                 worldobjreference *ref/* = NULL*/,
	                 const vector<soundconfig> &soundcollection /* = gamesounds*/);
	void delete_ ( int i );
	void replaceworldobjreference ( const worldobjreference &oldr,
