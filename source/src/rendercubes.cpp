...
void renderstripssky ()
{
	if ( skystrips.tris.first.empty() && skystrips.tristrips.first.empty()
	     && skystrips.quads.first.empty() ) return;
...
static void addstrip ( int type, int tex, int start, int n )
{
	...
	if ( tex == DEFAULT_SKY ) {
		...
		sb = &skystrips;
		loopi(n)
			skyfloor = min(skyfloor, verts[start + i].z);
	}
	else {
...
enum
{
	STRIP_FLOOR = 1, STRIP_DELTA, STRIP_WALL
};
...
static const char *cubetypename ( int t )
{
	return t >= 0 && t < SEMISOLID ? cubetypes[t] : "unknown";
}

void mipstats ( const int a[] )
{
	if ( showm && !showef )
	    hudeditf(HUDMSG_MIPSTATS, "1x1/2x2/4x4/8x8: %d / %d / %d / %d", a[0],
	             a[1], a[2], a[3]);
}
...
bool editfocusdetails ( sqr *s )
{
	if ( showef ) {
		...
		hudeditf(HUDMSG_EDITFOCUS,
		         "%s: \fs%s%d..%d\fr W:%d \fs%sU:%d F:%d C:%d v:%d t:0x%X\fr",
		         cubetypename(s->type), g, s->floor, s->ceil, s->wtex, g,
		         s->utex, s->ftex, s->ctex, s->vdelta, s->tag);
		return true;
	}
	return false;
}
                   ...
                   bool isceil )   // floor/ceil quads
{
	...
	else        // continue strip
	{
		...
		if ( ( abs(ol1r - l3->r) < lighterr && abs(ol2r - l4->r) < lighterr   // skip vertices if light values are close enough
		       ...
		       && abs(ol1b - l3->b) < lighterr && abs(ol2b - l4->b) < lighterr )
		     || !wtex ) {
                        ...
                        bool isceil )   // floor/ceil quads on a slope
{
	...
	bool first = striptype != STRIP_DELTA || x != ox + size || striptex != wtex
	             || y != oy;
...
static void render_2tris ( sqr *h,
                           sqr *s,
                           int x1,
                           int y1,
                           int x2,
                     ...
                     int dir )   // wall quads
{
	...
	bool first = striptype != STRIP_WALL || striptex != wtex || ox != x1
	             || oy != y1 || ofloor != floor1 || oceil != ceil1
	             || odir != dir, hf = floor1 != floor2 || ceil1 != ceil2;
	...
	{
		int lighterr = lighterror * 2;
		if ( ( !hf && !ohf ) && ( ( abs(ol1r - l2->r) < lighterr
		        && abs(ol1g - l2->g) < lighterr && abs(ol1b - l2->b) < lighterr )
...
static void rendershadow_tri ( sqr *h,
                               int x1,
                               int y1,
                               int x2,
                               int y2,
...
#define df(x) s->floor-(x->vdelta/4.0f)
	...
	for ( int yy = y; yy < ys; yy++ )
		for ( int xx = x; xx < xs; xx++ ) {
			...
			if ( s->type == SPACE || s->type == CHF ) {
				...
				sqr *t = SW(s, 1, 0), *v = SW(s, 0, 1), *w = SW(s, 0, -1), *z =
				        SW(s, -1, 0);
				bool topleft = true;
				sqr *h1 = NULL, *h2 = NULL;
				if ( SOLID(z) ) {
	...
	for ( shadowvertex *v = shadowverts.getbuf(), *end =
	        &v[shadowverts.length()]; v < end; v++ ) {
		float vx = v->x, vy = v->y;
