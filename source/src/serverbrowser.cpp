...
bool resolverwait ( const char *name, ENetAddress *address )
{
	...
	for ( ;; ) {
		...
		show_out_of_renderloop_progress(
		        min(float(timeout) / RESOLVERLIMIT, 1.0f), text);
		SDL_Event event;
		while ( SDL_PollEvent(&event) ) {
			if ( event.type == SDL_KEYDOWN && event.key.keysym.sym
			        == SDLK_ESCAPE ) timeout = RESOLVERLIMIT + 1;
		}
...
int connectwithtimeout ( ENetSocket sock,
                         const char *hostname,
                         ENetAddress &address )
{
	if ( isdedicated ) {
	...
	for ( ;; ) {
		...
		while ( SDL_PollEvent(&event) ) {
			if ( event.type == SDL_KEYDOWN && event.key.keysym.sym
			        == SDLK_ESCAPE ) timeout = CONNLIMIT + 1;
		}
...
{
	loopv(servers)
		if ( servers[i]->address.host == address.host && servers[i]->port
		        == address.port ) return servers[i];
	return NULL;
}
...
static serverinfo *newserver ( const char *name,
                               uint ip = ENET_HOST_ANY,
                               ...
                               int weight = 0 )
{
	...
	if ( name )
		copystring(si->name, name);
	        ...
	        || enet_address_get_host_ip(&si->address, si->name,
	                                    sizeof ( si->name ))
	           < 0 ) {
		delete si;
		return NULL;
...
void addserver ( const char *servername, int serverport, int weight )
{
	...
	loopv(servers)
		if ( strcmp(servers[i]->name, servername) == 0 && servers[i]->port
		        == serverport ) return;
...
static void checkpings ()
{
	...
	while ( enet_socket_wait(pingsock, &events, 0) >= 0 && events ) {
		...
		serverinfo *si = NULL;
		loopv(servers)
			if ( addr.host == servers[i]->address.host && addr.port
			        == servers[i]->address.port ) {
				si = servers[i];
				break;
			}
		if ( !si && searchlan )
		    si = newserver(NULL, addr.host,
		                   CUBE_SERVINFO_TO_SERV_PORT(addr.port));
		if ( !si ) continue;
					...
					case EXTPING_NAMELIST: {
						...
						loopi(si->numplayers)
						{
							...
							if ( text[0] && !p.overread() ) {
								si->playernames.add(
								        (const char *) si->namedata + q.length());
								sendstring(text, q);
							}
							else
								break;
					...
					case EXTPING_MAPROT: {
						...
						while ( p.remaining() ) {
							...
							if ( *text && !p.overread() ) {
								text[MAXINFOLINELEN] = '\0';
								loopi(n)
									concatformatstring(text, ", %d", getint(p));
								ADDINFOLINE(text);
							}
							else
								break;
					...
					case EXTPING_UPLINKSTATS: {
						...
						if ( si->maxclients > 3 ) {
							...
							if ( maxs < 0x50 )
								...
								si->uplinkqual = 5;   // choke_percentage = 1 / 2 ^ (15 - tc), so 0x03 is 0.02%
							else if ( maxc < 6 )
								si->uplinkqual = 4;       // 0.2%
							else if ( maxc == 6 )
								si->uplinkqual = 3;
						...
						if ( si->getinfo == EXTPING_UPLINKSTATS ) {
							...
							if ( si->maxclients < 4 ) {
								...
								ADDINFOLINE(
								        "\f1server uplink quality and usage statistics:");
								ADDINFOLINE("");
									...
									loopj(15)
										concatformatstring(
										        ...
										        + 2 * ( ( si->uplinkstats[i]
										                & 0xF0 )
										                > ( j << 4 ) ));
									concatformatstring(msg, "\t\t [%02X]",
									                   si->uplinkstats[i]);
									ADDINFOLINE(msg);
								}
								ADDINFOLINE("");
								ADDINFOLINE(
								        "\f4red bar: error rate, green bar: time played");
							}
						}
						break;
					}
...
#undef ADDINFOLINE
		...
		if ( si->pongflags > 0 ) {
			...
			if ( si->pongflags & ( 1 << PONGFLAG_BANNED ) ) sp =
...
static int sicompare ( serverinfo **ap, serverinfo **bp )
{
	...
	if ( a->weight < b->weight ) return dir;
	enet_uint32 ai = ENET_NET_TO_HOST_32(a->address.host), bi =
	        ENET_NET_TO_HOST_32(b->address.host);
	...
	switch ( serversort )
	{
		...
		case SBS_MODE:   // mode
		{
			const char *am = modestr(a->mode, modeacronyms > 0), *bm = modestr(
			        b->mode, modeacronyms > 0);
...
static const char *fc_als[] = { "weight", "tag", "desc", "red", "green", "blue",
                                "alpha", "keys", "ignore", "image" };
enum
{
	FC_WEIGHT = 0,
	FC_TAG,
...
VARF(showonlyfavourites,
     0,
     0,
     100,
     {
	     if ( showonlyfavourites > favcats.length() ) {
		     conoutf("showonlyfavourites: %d out of range (0..%d)",
		             showonlyfavourites, favcats.length())
		     ;
...
static void addfavcategory ( const char *refdes )
{
	...
	loopi(FC_NUM)
		alx[i] = getalias(favcatargname(text, i)) ? 1 : 0;
	loopi(3)
	{
	...
	const int defk[] = { FC_WEIGHT, FC_DESC, FC_TAG, FC_KEYS, FC_IGNORE,
	                     FC_ALPHA };
...
static const char *favcatcheck ( serverinfo &si,
                                 const char *ckeys,
                                 char *autokeys = NULL )
{
...
static void serverbrowseralternativeviews ( int shiftdirection )
{
	...
	while ( k ) {
		loopv(favcats)
			if ( !strcmp(favcats[i], k) ) cats.add(i + 1);
		k = strtok(NULL, sep);
	}
	delete[] keys;
	if ( cats.length() ) {
		loopv(cats)
		{
			if ( cats[i] == showonlyfavourites ) {
				showonlyfavourites =
				        cats[ ( i + shiftdirection + cats.length() ) % cats
				                 .length()];
				return;
			}
		}
...
static bool assignserverfavourites ()
{
	...
	loopvj(favcats)
	{
		...
		while ( k ) {
			...
			{
				serverinfo &si = *servers[i];
				if ( si.address.host == ENET_HOST_ANY || si.ping
				        == 9999|| si.protocol != PROTOCOL_VERSION ) continue;
				if ( ( !alxn[FC_IGNORE] || showonlythiscat ) && favcatcheckkey(
				        si, k) ) {
					...
					if ( si.favcat == -1 || showonlythiscat ) {
						...
						if ( alxn[FC_ALPHA] ) {
							...
							new (si.bgcolor) color(
							        ( (float) alxn[FC_RED] ) / 100,
							        ( (float) alxn[FC_GREEN] ) / 100,
							        ( (float) alxn[FC_BLUE] ) / 100,
							        ( (float) alxn[FC_ALPHA] ) / 100);
						}
...
void refreshservers ( void *menu, bool init )
{
	...
	if ( init ) {
		loopi(PINGBUFSIZE)
			if ( pingbuf[i] < totalmillis ) pingbuf[i] = 0;
	...
	if ( isinfo ) {
		if ( lastselectedserver ) {
			bool found = false;
			loopv(servers)
				if ( lastselectedserver == servers[i] ) {
					found = true;
					break;
				}
		...
		if ( lastselectedserver ) {
			...
			if ( si.infotexts.length() && !pinglastselected ) {
				...
				if ( si.getinfo == EXTPING_NOP ) si.getinfo =
				        EXTPING_SERVERINFO;
				if ( init || pinglastselected
				     || totalmillis - lastinfo >= servpingrate )
	...
	if ( ( init && issearch ) || totalmillis - lastinfo
	        >= ( servpingrate * ( issearch ? 2 : 1 ) ) / (
	                maxservpings ?
	                        max(1,
	                            ( servers.length() + maxservpings - 1 ) / maxservpings) :
	                        1 ) )
	...
	if ( !init && menu )   // && servers.inrange(((gmenu *)menu)->menusel))
	{
		...
		loopv(servers)
			if ( servers[i]->menuline_from == ( (gmenu *) menu )->menusel && servers[i]
			                                                                     ->menuline_to
			                                                                 > servers[i]
			                                                                     ->menuline_from ) {
		...
		if ( foundserver ) {
			if ( ( usedselect || ( (gmenu *) menu )->menusel > 0 ) ) oldsel =
			        foundserver;
	...
	if ( menu ) {
		...
		formatstring(title)(
		        titles[serversort], showfavtag ? "fav\t" : "", issearch ? "      search results for \f3" : "     (F1: Help/Settings)", issearch ? cursearch : "");
		...
		{
			serverinfo &si = *servers[i];
			si.menuline_to = si.menuline_from =
			        ( (gmenu *) menu )->items.length();
			if ( ( !showallservers && si.lastpingmillis <= servermenumillis ) || ( si
			                                                                           .maxclients
			                                                                       > MAXCL
			                                                                       && searchlan < 2 ) )
			...
			bool showthisone = ! ( banned && showonlygoodservers )
			        && ! ( showonlyfavourites > 0 && si.favcat
			                != showonlyfavourites - 1 );
			bool serverfull = si.numplayers >= si.maxclients;
			bool needspasswd = ( si.pongflags & ( 1 << PONGFLAG_PASSWORD ) )
			        > 0;
			int mmode = ( si.pongflags >> PONGFLAG_MASTERMODE ) & MM_MASK;
			char basecolor =
			        banned ? '4' : ( curserver == servers[i] ? '1' : '5' );
			char plnumcolor =
			        serverfull ?
			                '2' :
			                ( needspasswd ?
			                        '3' : ( mmode != MM_OPEN ? '1' : basecolor ) );
			const char *favimage = NULL;
			if ( si.address.host != ENET_HOST_ANY && si.ping != 9999 ) {
				if ( si.protocol != PROTOCOL_VERSION ) {
					if ( !showonlygoodservers )
						formatstring(si.full)(
						        "%s:%d [%s]", si.name, si.port, si.protocol<0 ? "modded version" : (si.protocol<PROTOCOL_VERSION ? "older protocol" : "newer protocol"));
			...
			if ( issearch && showthisone ) {
				...
				loopvj(si.playernames)
					if ( matchplayername(si.playernames[j]) ) {
						found = true;
						break;
					};
			...
			if ( showthisone ) {
				...
				if ( sbconnectexists ) {
					...
					text[30] = '\0';
					formatstring(si.cmd)(
					        "sbconnect %s %d %d %d %d %d \"%s\"", si.name, si.port, serverfull ?1:0, needspasswd ?1:0, mmode, banned, text);
				}
				else formatstring(si.cmd)("connect %s %d", si.name, si.port);
				menuimagemanual(menu, favimage, "serverquality", si.full,
				                si.cmd, si.bgcolor, si.description);
				if ( !issearch && servers[i] == oldsel ) {
					( (gmenu *) menu )->menusel = ( (gmenu *) menu )->items
					                                  .length()
					                              - 1;
					usedselect = true;
				...
				if ( ( shownamesinbrowser && servers[i] == oldsel
					...
					{
						if ( cur == 0 ) {
							if ( namelists.length() < ++curnl )
							    namelists.add(newstringbuf());
							t = namelists[curnl - 1];
		...
		if ( issearch ) {
			if ( curnl == 0 ) {
				formatstring(notfoundmsg)(
				        "\t\tpattern \fs\f3%s\fr not found.", cursearch);
...
{
	const int fk[] = { SDLK_1, SDLK_2, SDLK_3, SDLK_4, SDLK_5, SDLK_6, SDLK_7,
	                   SDLK_8, SDLK_9, SDLK_0 };
	if ( !isdown ) return false;
	loopi(sizeof(fk)/sizeof(fk[0]))
		if ( code == fk[i] && favcats.inrange(i) ) {
			int sel = ( (gmenu *) menu )->menusel;
			loopvj(servers)
				if ( menu && ( servers[j]->menuline_from <= sel
				        && servers[j]->menuline_to > sel ) ) {
					string ak;
					ak[0] = '\0';
					const char *keyalias = favcatargname(favcats[i], FC_KEYS),
					        *key = getalias(keyalias), *rest = favcatcheck(
					                *servers[j], key, ak), *desc = getalias(
					                favcatargname(favcats[i], FC_DESC));
					if ( !desc ) desc = "";
					if ( *ak ) {   // server was automatically added to this favourite group, don't remove
						conoutf(_(
						        "server \"%cs%s%cr\" is in category '%cs%s%cr' because of key '%s', please remove manually"),
						        CC, servers[j]->sdesc, CC, CC, desc, CC, ak);
					}
					else if ( rest ) {   // remove from favourite group
						conoutf(_(
						        "removing server \"%cs%s%cr\" from favourites category '%cs%s%cr' (rest '%s')"),
						        CC, servers[j]->sdesc, CC, CC, desc, CC, rest);
						alias(keyalias, rest);
					}
					else {   // add IP:port to group
						defformatstring(text)("%s:%d", servers[j]->name,
						                      servers[j]->port);
						if ( key && *key ) {
							char *newkey = newstring(
							        key, strlen(text) + 1 + strlen(key));
							strcat(newkey, " ");
							strcat(newkey, text);
							...
							alias(keyalias, text);
						conoutf(_(
						        "adding server \"%cs%s%cr\" to favourites category '%cs%s%cr' (new '%s')"),
						        CC, servers[j]->sdesc, CC, CC, desc, CC,
						        getalias(keyalias));
					}
					return true;
				}
...
bool serverinfokey ( void *menu, int code, bool isdown, int unicode )
{
	...
	switch ( code )
	{
		...
		case SDLK_F5:
			if ( lastselectedserver ) lastselectedserver->getinfo =
			        EXTPING_SERVERINFO;
...
static int progress_callback_retrieveservers ( void *clientp,
                                               double dltotal,
                                               double dlnow,
                                               double ultotal,
                                               double ulnow )
{
	...
	show_out_of_renderloop_progress(
	        min(float(rd->timeout) / RETRIEVELIMIT, 1.0f), rd->text);
...
static void retrieveservers ( vector<char> &data )
{
	if ( mastertype == AC_MASTER_HTTP ) {
		...
		string request;
		sprintf(request,
		        "http://%s/retrieve.do?action=list&name=%s&version=%d&build=%d",
		        mastername, pname, AC_VERSION, getbuildtype() | ( 1 << 16 ));
		curl_free(pname);
		...
		formatstring(rd->text)(
		        "retrieving servers from %s:%d... (esc to abort)", mastername, masterport);
		...
		curl_easy_setopt(curl, CURLOPT_PROGRESSFUNCTION,
		                 progress_callback_retrieveservers);
		...
		if ( result == CURLE_OPERATION_TIMEDOUT || result
...
void updatefrommaster ( int force )
{
	...
	if ( !force && lastupdate
	     && totalmillis - lastupdate < masterupdatefrequency * 1000 ) return;
	...
	if ( data.empty() ) {
		...
		if ( !cllock ) {
			execute(data.getbuf());
			if ( curserver )
			    addserver(curname, curserver->port, curserver->msweight);
		}
