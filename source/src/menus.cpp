...
static char *getfiledesc ( const char *dir, const char *name, const char *ext )
{
	...
	if ( !strcmp(ext, "dmo") ) {
		...
		if ( !f ) return NULL;
		demoheader hdr;
		if ( f->read(&hdr, sizeof(demoheader)) != sizeof(demoheader) || memcmp(
		        hdr.magic, DEMO_MAGIC, sizeof ( hdr.magic )) ) {
			delete f;
			return NULL;
		}
		delete f;
		...
		if ( hdr.version == DEMO_VERSION ) {
			if ( hdr.protocol == PROTOCOL_VERSION )
				tag = "";
			else if ( hdr.protocol == -PROTOCOL_VERSION ) tag =
			        "(recorded on modded server) ";
		}
		...
		if ( !f ) return NULL;
		header hdr;
		if ( f->read(&hdr, sizeof(header)) != sizeof(header) || ( strncmp(
		        hdr.head, "CUBE", 4)
		                                                          && strncmp(
		                                                                  hdr
		                                                                      .head,
		...
		formatstring(text)(
		        "%s%s", (hdr.version>MAPVERSION) ? "(incompatible file) " : "", hdr.maptitle);
		text[DHDR_DESCCHARS - 1] = '\0';
		return newstring(text);
	}
	return NULL;
...
struct mitemmanual : mitem
{
	...
	mitemmanual ( gmenu *parent,
	              char *text,
	              char *action,
	              char *hoveraction,
	              color *bgcolor,
	              const char *desc ) :
			mitem(parent, bgcolor, mitem::TYPE_MANUAL), text(text),
			        action(action), hoveraction(hoveraction), desc(desc)
	{
	}
...
struct mitemtextvar : mitemmanual
{
	string dtext;
	const char *textexp;
	mitemtextvar ( gmenu *parent,
	               const char *evalp,
	               char *action,
	               char *hoveraction ) :
			mitemmanual(parent, dtext, action, hoveraction, NULL, NULL)
	{
		dtext[0] = '\0';
		textexp = evalp;
	}
...
struct mitemimagemanual : mitemmanual
{
	...
	{
		if ( image && *text != '\t' )
		    return ( FONTH * image->xs ) / image->ys + FONTH / 2
		           + mitemmanual::width();
	...
	virtual void render ( int x, int y, int w )
	{
		...
		if ( image || altfont ) {
			...
			if ( image ) {
				...
				glTexCoord2f(1, 1);
				glVertex2f(x + xs, y + FONTH);
				glEnd();
				xtraverts += 4;
			}
			draw_text(text, !image || *text == '\t' ? x : x + xs + FONTH / 2,
			          y);
			...
			if ( image && isselection() && !hidebigmenuimages && image->ys > FONTH ) {
				w += FONTH;
				int xs = ( 2 * VIRTW - w ) / 5, ys = ( xs * image->ys )
				        / image->xs;
				x = ( 6 * VIRTW + w - 2 * xs ) / 4;
				y = VIRTH - ys / 2;
				blendbox(x - FONTH, y - FONTH, x + xs + FONTH, y + ys + FONTH,
				         false);
				glBindTexture(GL_TEXTURE_2D, image->id);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				glColor3f(1, 1, 1);
...
struct mitemimage : mitemimagemanual
{
	mitemimage ( gmenu *parent,
	             ...
	             color *bgcolor,
	             const char *desc = NULL ) :
			        mitemimagemanual(parent, filename, NULL, text, action,
			                         hoveraction, bgcolor, desc)
	{
	}
	virtual ~mitemimage ()
	{
...
struct mitemmaploadmanual : mitemmanual
{
	...
	mitemmaploadmanual ( gmenu *parent,
	                     const char *filename,
	                     const char *altfontname,
	                     char *text,
	                     char *action,
	                     ...
	                     color *bgcolor,
	                     const char *desc = NULL ) :
			mitemmanual(parent, text, action, NULL, NULL, NULL),
			        filename(filename)
	{
		image = NULL;
		...
		if ( filename ) {
			...
			if ( d ) {
				formatstring(maptitle)("%s", d[0] ? d : "-n/a-");}
			else
			{
			...
			formatstring(maptitle)("-n/a-:%s", filename); image = textureload("packages/misc/nopreview.png", 3);}
		copystring(mapstats, "");
	}
...
struct mitemmapload : mitemmaploadmanual
{
	mitemmapload ( gmenu *parent,
	               ...
	               color *bgcolor,
	               const char *desc = NULL ) :
			        mitemmaploadmanual(parent, filename, NULL, text, action,
			                           hoveraction, bgcolor, desc)
	{
	}
...
struct mitemtextinput : mitemtext
{
	...
	mitemtextinput ( gmenu *parent,
			...
			mitemtext(parent, text, action, hoveraction, bgcolor),
			        defaultvalueexp(value), modified(false), hideinput(false)
	{
	...
	virtual void render ( int x, int y, int w )
	{
		...
		draw_text(
		        ...
		        255,
		        selection ?
		                ( input.pos >= 0 ? ( input.pos > sc ? sc : input.pos ) :
		                                   cibl ) :
		                -1);
	}
...
struct mitemslider : mitem
{
	...
	mitemslider ( gmenu *parent,
	              ...
	              char *action,
	              color *bgcolor ) :
			mitem(parent, bgcolor, mitem::TYPE_SLIDER), min_(min_), max_(max_),
			        step(step), value(min_), maxvaluewidth(0), text(text),
			        valueexp(value), display(display), action(action)
	{
		if ( sliderwidth == 0 )
		    sliderwidth = max(VIRTW / 4, 15 * text_width("w"));   // match slider width with width of text input fields
	}
	...
	virtual int width ()
	{
		return text_width(text) + sliderwidth + maxvaluewidth + 2 * FONTH;
	}
	...
	virtual void render ( int x, int y, int w )
	{
		...
		blendbox(x + w - sliderwidth, y + FONTH / 3, x + w, y + FONTH * 2 / 3,
		         false, -1, &gray);
		int offset = (int) ( cval / ( (float) range ) * sliderwidth );
		blendbox(x + w - sliderwidth + offset - FONTH / 6, y,
		         x + w - sliderwidth + offset + FONTH / 6, y + FONTH, false, -1,
		         sel ? &whitepulse : &white);
	}
...
struct mitemkeyinput : mitem
{
	...
	mitemkeyinput ( gmenu *parent, char *text, char *bindcmd, color *bgcolor ) :
			mitem(parent, bgcolor, mitem::TYPE_KEYINPUT), text(text),
			        bindcmd(bindcmd), keyname(NULL), capture(false)
	{
	}
	;
	...
	virtual void render ( int x, int y, int w )
	{
		...
		if ( isselection() )
		    blendbox(x + w - tk - FONTH, y - FONTH / 6, x + w + FONTH,
		             y + FONTH + FONTH / 6, false, -1,
		             capture ? &capturec : NULL);
		draw_text(text, x, y);
	...
	virtual void key ( int code, bool isdown, int unicode )
	{
		...
		keym *km;
		while ( ( km = findbinda(bindcmd) ) ) {
			bindkey(km, "");
		}   // clear existing binds to this cmd
		if ( bindc(code, bindcmd) )
			parent->init();   // re-init all bindings
		else
...
struct mitemcheckbox : mitem
{
	...
	mitemcheckbox ( gmenu *parent,
	                char *text,
	                char *value,
	                char *action,
	                color *bgcolor ) :
			mitem(parent, bgcolor, mitem::TYPE_CHECKBOX), text(text),
			        valueexp(value), action(action), checked(false)
	{
	}
	;
	...
	virtual int width ()
	{
		return text_width(text) + FONTH + FONTH / 3;
	}
	...
	virtual void render ( int x, int y, int w )
	{
		...
		if ( checked ) {
			int x1 = x + w - boxsize - FONTH / 6, x2 = x + w + FONTH / 6, y1 = y
			        - FONTH / 6, y2 = y + boxsize + FONTH / 6;
                       ...
                       const char *desc )
{
	gmenu &m = *(gmenu *) menu;
	m.items.add(
	        new mitemimagemanual(&m, filename, altfontname, text, action, NULL,
	                             bgcolor, desc));
}
...
static void lastmenu_header ( char *header, char *footer )
{
	if ( lastmenu ) {
		...
		conoutf("no last menu to apply to");
}
...
static void menuitem ( char *text, char *action, char *hoveraction )
{
	...
	lastmenu->items.add(
	        new mitemtext(lastmenu, t, newstring(action[0] ? action : text),
	                      hoveraction[0] ? newstring(hoveraction) : NULL,
	                      NULL));
}
...
static void menuitemvar ( char *eval, char *action, char *hoveraction )
{
	...
	lastmenu->items.add(
	        new mitemtextvar(lastmenu, t, action[0] ? newstring(action) : NULL,
	                         hoveraction[0] ? newstring(hoveraction) : NULL));
}

static void menuitemimage ( char *name,
                            char *text,
                            char *action,
                            char *hoveraction )
{
	...
	if ( fileexists(name, "r") || findfile(name, "r") != name )
		lastmenu->items.add(
		        new mitemimage(lastmenu, newstring(name), newstring(text),
		                       action[0] ? newstring(action) : NULL,
		                       hoveraction[0] ? newstring(hoveraction) : NULL,
		                       NULL));
	else
		lastmenu->items.add(
		        new mitemtext(lastmenu, newstring(text),
		                      newstring(action[0] ? action : text),
		                      hoveraction[0] ? newstring(hoveraction) : NULL,
		                      NULL));
}
...
static void menuitemmapload ( char *name, char *text )
{
	...
	lastmenu->items.add(
	        new mitemmapload(lastmenu, newstring(name), newstring(name),
	                         newstring(caction), NULL, NULL, NULL));
}

static void menuitemtextinput ( char *text,
                                char *value,
                                char *action,
                                char *hoveraction,
                                int *maxchars,
                                int *maskinput )
{
	...
	lastmenu->items.add(
	        new mitemtextinput(lastmenu, newstring(text), newstring(value),
	                           action[0] ? newstring(action) : NULL,
	                           hoveraction[0] ? newstring(hoveraction) : NULL,
	                           NULL, *maxchars, *maskinput));
}

static void menuitemslider ( char *text,
                             int *min_,
                             int *max_,
                             char *value,
                             int *step,
                             ...
                             char *action )
{
	if ( !lastmenu ) return;
	lastmenu->items.add(
	        new mitemslider(lastmenu, newstring(text), *min_, *max_, *step,
	                        newstring(value),
	                        display[0] ? newstring(display) : NULL,
	                        action[0] ? newstring(action) : NULL, NULL));
}
...
static void menuitemkeyinput ( char *text, char *bindcmd )
{
	if ( !lastmenu ) return;
	lastmenu->items.add(
	        new mitemkeyinput(lastmenu, newstring(text), newstring(bindcmd),
	                          NULL));
}
...
static void menuitemcheckbox ( char *text, char *value, char *action )
{
	if ( !lastmenu ) return;
	lastmenu->items.add(
	        new mitemcheckbox(lastmenu, newstring(text), newstring(value),
	                          action[0] ? newstring(action) : NULL, NULL));
}
...
static void chmenumdl ( char *menu,
                        char *mdl,
                        char *anim,
                        int *rotspeed,
                        int *scale )
...
static bool parsecolor ( color *col,
                         const char *r,
                         const char *g,
                         const char *b,
                         const char *a )
...
bool menukey ( int code, bool isdown, int unicode, SDLMod mod )
{
	...
	if ( isdown ) {
		bool hasdesc = false;
		loopv(curmenu->items)
			if ( curmenu->items[i]->getdesc() ) {
				hasdesc = true;
				break;
			}
		//int pagesize = MAXMENU - (curmenu->header ? 2 : 0) - (curmenu->footer || hasdesc ? 2 : 0); // FIXME: footer-length
		int pagesize = MAXMENU
		        - ( curmenu->header ? 2 : 0 )
		        - ( curmenu->footer ?
		                ( curmenu->footlen ? ( curmenu->footlen + 1 ) : 2 ) :
		                ( hasdesc ? 2 : 0 ) );   // FIXME: footer-length

		if ( curmenu->items.inrange(menusel) ) {
			mitem *m = curmenu->items[menusel];
			if ( m->type == mitem::TYPE_KEYINPUT && ( (mitemkeyinput *) m )
			         ->capture
			     && code != SDLK_ESCAPE ) {
		...
		switch ( code )
		{
			case SDLK_PAGEUP:
				...
				break;
			case SDLK_PAGEDOWN:
				if ( menusel + pagesize >= n && menusel / pagesize
				        != ( n - 1 ) / pagesize )
					menusel = n - 1;
				else
					menusel += pagesize;
			...
			case SDL_AC_BUTTON_WHEELUP:
				if ( iskeypressed(SDLK_LCTRL) )
				    return menukey(SDLK_LEFT, isdown, 0);
			...
			case SDL_AC_BUTTON_WHEELDOWN:
				if ( iskeypressed(SDLK_LCTRL) )
				    return menukey(SDLK_RIGHT, isdown, 0);
				if ( iskeypressed(SDLK_LALT) )
			...
			default: {
				if ( !curmenu->allowinput ) return false;
				if ( curmenu->keyfunc && ( *curmenu->keyfunc )(curmenu, code,
				                                               isdown,
				                                               unicode) )
				    return true;
		...
		if ( !curmenu->allowinput || !curmenu->items.inrange(menusel) )
		    return false;
		mitem &m = *curmenu->items[menusel];
		if ( code == SDLK_RETURN || code == SDLK_SPACE
		     || code == SDL_AC_BUTTON_LEFT || code == SDL_AC_BUTTON_MIDDLE ) {
			m.select();
			if ( m.getaction() != NULL && !strcmp(m.getaction(), "-1") )
			    return true;   // don't playsound S_MENUENTER if menuitem action == -1 (null/blank/text only item) - Bukz 2013feb13
...
void rendermenumdl ()
{
	...
	bool isplayermodel = !strncmp(m.mdl, "playermodels",
	                              strlen("playermodels"));
	...
	if ( dirlist && ( ( dirlist->dir != NULL ) && ( dirlist->ext != NULL ) ) ) {
		...
		loopv(files)
		{
			...
			if ( dirlist->image ) {
				...
				items.add(
				        new mitemimage(this, newstring(fullname), f,
				                       newstring(dirlist->action), NULL, NULL,
				                       d));
			}
			else if ( !strcmp(
			        dirlist->ext,
			        "cgz") /*&& (fileexists(jpgname, "r") || findfile(jpgname, "r") != jpgname)*/) {
				...
				defformatstring(fullname)(
				        "%s%s%s",
				        dirlist->dir[0] ? dirlist->dir + diroffset : "",
				        dirlist->dir[0] ? "/" : "", f);
				defformatstring(title)("%s", f);
				items.add(
				        new mitemmapload(this, newstring(fullname),
				                         newstring(title),
				                         newstring(dirlist->action), NULL, NULL,
				                         NULL));
			}
				...
				items.add(
				        new mitemtext(this, f, newstring(dirlist->action), NULL,
				                      NULL, d));
		}
	}
	loopv(items)
...
void gmenu::render ()
{
	...
	int hitems =
	        ( header ? 2 : 0 ) + (
	                footer ? ( footlen ? ( footlen + 1 ) : 2 ) :
	                         ( hasdesc ? 2 : 0 ) ), pagesize = MAXMENU - hitems,
	        offset = menusel - ( menusel % pagesize ), mdisp = min(
	                items.length(), pagesize), cdisp = min(
	...
	if ( !hotkeys )
	    renderbg(x - FONTH * 3 / 2, y - FONTH, x + w + FONTH * 3 / 2,
	             y + h + FONTH, true);
	if ( offset > 0 )
			...
			if ( text_width(footer) > w ) {
				...
				while ( keepon ) {
					...
					if ( cflc <= strlen(footer) ) {
						if ( cflc == strlen(footer) || cflc >= MAXSTRLEN )
						    keepon = false;
						cflc--;
						cflw = 0;
						formatstring(bufl)("%s", footer+tflo);
...
{
	gmenu *m = (gmenu *) menu;
	if ( !m || ( !init && needsapply.length() != m->items.length() - 3 ) )
	    return;
	m->items.deletecontents();
	loopv(needsapply)
		m->items.add(
	...
	m->items.add(
	        new mitemtext(m, newstring("Yes"), newstring("resetgl"), NULL,
	                      NULL));
	m->items.add(
	        new mitemtext(
	                m,
	                newstring("No"),
	                newstring(
	                        "echo [..restart AssaultCube to apply the new settings]"),
	                NULL, NULL));
