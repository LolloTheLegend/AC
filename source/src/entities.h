...
enum
{
	MAP_IS_BAD, MAP_IS_EDITABLE, MAP_IS_GOOD
};
...
struct persistent_entity        // map entity
{
	...
	persistent_entity ( short x,
	                    ...
	                    uchar attr3,
	                    uchar attr4 ) :
			x(x), y(y), z(z), attr1(attr1), type(type), attr2(attr2),
...
struct entity : persistent_entity
{
	...
	entity ( short x,
	         ...
	         uchar attr3,
	         uchar attr4 ) :
			persistent_entity(x, y, z, type, attr1, attr2, attr3, attr4),
			        spawned(false)
	{
	}
	entity ()
	...
	{
		return !m_noitems && isitem(type)
		       && ! ( m_noitemsnade && type != I_GRENADE )
	...
	void transformtype ( int gamemode )
	{
		if ( m_noitemsnade && type == I_CLIPS )
			type = I_GRENADE;
		else if ( m_pistol && ( type == I_AMMO || type == I_GRENADE ) ) type =
		        I_CLIPS;
	}
};
...
{
	string modelname;
	short sound, reload, reloadtime, attackdelay, damage, piercing, projspeed,
	        part, spread, recoil, magsize, mdl_kick_rot, mdl_kick_back,
	        recoilincrease, recoilbase, maxrecoil, recoilbackfade, pushfactor;
	bool isauto;
};
extern guninfo guns[NUMGUNS];

static inline int reloadtime ( int gun )
{
	return guns[gun].reloadtime;
}
static inline int attackdelay ( int gun )
{
	return guns[gun].attackdelay;
}
static inline int magsize ( int gun )
{
	return guns[gun].magsize;
}
...
enum
{
	ENT_PLAYER = 0, ENT_BOT, ENT_CAMERA, ENT_BOUNCE
};
enum
{
	CS_ALIVE = 0, CS_DEAD, CS_SPAWNING, CS_LAGGED, CS_EDITING, CS_SPECTATE
};
enum
{
	CR_DEFAULT = 0, CR_ADMIN
};
enum
{
	SM_NONE = 0,
...
public:
	...
	bool inwater;
	bool onfloor, onladder, jumpnext, crouching, crouchedinair, trycrouch,
	        cancollide, stuck, scoping, shoot;
	int lastjump;
	...
	physent () :
			o(0, 0, 0), deltapos(0, 0, 0), newpos(0, 0, 0), yaw(270), pitch(0),
			        roll(0), pitchvel(0), crouching(false),
			        crouchedinair(false), trycrouch(false), cancollide(true),
			        stuck(false), scoping(false), shoot(false), lastjump(0),
			        lastjumpheight(0), lastsplash(0), state(CS_ALIVE),
			        last_pos(0)
	{
		reset();
	}
	...
	void reset ()
	{
		vel.x = vel.y = vel.z = eyeheightvel = vel_t.x = vel_t.y = vel_t.z =
		        0.0f;
		move = strafe = 0;
		timeinair = lastjump = lastsplash = 0;
		onfloor = onladder = inwater = jumpnext = crouching = crouchedinair =
		        trycrouch = stuck = false;
		last_pos = 0;
	}
...
public:
	...
	playerstate () :
			armour(0), primary(GUN_ASSAULT), nextprimary(GUN_ASSAULT),
			        akimbo(false)
	{
	}
	virtual ~playerstate ()
	...
	bool canpickup ( int type )
	{
		...
		{
			case I_CLIPS:
				return ammo[akimbo ? GUN_AKIMBO : GUN_PISTOL] < ammostats[
				        akimbo ? GUN_AKIMBO : GUN_PISTOL].max;
			case I_AMMO:
	...
	virtual void spawnstate ( int gamemode )
	{
		...
		if ( !m_nopistol ) {
			ammo[GUN_PISTOL] = ammostats[GUN_PISTOL].start
			        - magsize(GUN_PISTOL);   //ammostats[GUN_PISTOL].max-magsize(GUN_PISTOL);
...
public:
	...
	bool allowmove ()
	{
		return state != CS_DEAD || spectatemode == SM_FLY;
	}

	weapon *weapons[NUMGUNS];
	weapon *prevweaponsel, *weaponsel, *nextweaponsel, *primweap, *nextprimweap,
	        *lastattackweapon;
	...
	playerent () :
			curskin(0), clientnum(-1), lastupdate(0), plag(0), ping(0),
			        address(0), lifesequence(0), frags(0), flagscore(0),
			        deaths(0), points(0), tks(0), lastpain(0), lastvoicecom(0),
			        clientrole(CR_DEFAULT), team(TEAM_SPECT),
			        spectatemode(SM_NONE), eardamagemillis(0), respawnoffset(0),
			        prevweaponsel(NULL), weaponsel(NULL), nextweaponsel(NULL),
			        primweap(NULL), nextprimweap(NULL), lastattackweapon(NULL),
			        smoothmillis(-1), head(-1, -1, -1), ignored(false),
			        muted(false)
	{
		type = ENT_PLAYER;
		name[0] = 0;
	...
	void damageroll ( float damage )
	{
		...
		roll += roll > 0 ?
		        damroll :
		        ( roll < 0 ? -damroll : ( rnd(2) ? damroll : -damroll ) );   // give player a kick
		clamproll(this);
	}
	...
	{
		primweap = weapons[ ( primary = w )];
	}
	void setnextprimary ( int w )
	{
		nextprimweap = weapons[ ( nextprimary = w )];
	}
	bool isspectating ()
	{
		return state == CS_SPECTATE || ( state == CS_DEAD
		        && spectatemode > SM_NONE );
	}
	...
	int skin ( int t = -1 )
	{
		return nextskin[team_base(t < 0 ? team : t)];
	}
...
enum
{
	CTFF_INBASE = 0, CTFF_STOLEN, CTFF_DROPPED, CTFF_IDLE
};
...
struct flaginfo
{
	...
	bool ack;
	flaginfo () :
			flagent(0), actor(0), state(CTFF_INBASE), ack(false)
	{
	}
};
...
public:
	...
	bounceent () :
			bouncetype(BT_NONE), rotspeed(1.0f), plclipped(false), owner(NULL)
	{
		type = ENT_BOUNCE;
		maxspeed = 40;
		radius = 0.2f;
	...
	bool isalive ( int lastmillis )
	{
		return lastmillis - millis < timetolive;
	}
...
struct pckserver
{
	...
	pckserver () :
			addr(NULL), pending(false), responsive(true), ping(-1)
	{
	}
};
...
enum
{
	PCK_TEXTURE, PCK_SKYBOX, PCK_MAPMODEL, PCK_AUDIO, PCK_MAP, PCK_NUM
};
...
struct package
{
	...
	package () :
			name(NULL), type(-1), number(0), pending(false), source(NULL),
			        curl(NULL)
	{
	}
};
