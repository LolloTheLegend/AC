...
int getclientnum ()
{
	return player1 ? player1->clientnum : -1;
}
...
static void connectserv_ ( const char *servername,
                           int serverport = 0,
                           ...
                           int role = CR_DEFAULT )
{
	...
	if ( servername ) {
		...
		conoutf(_("%c2attempting to %sconnect to %c5%s%c4:%d%c2"), CC,
		        role == CR_DEFAULT ? "" : "\f8admin\f2", CC, servername, CC,
		        serverport, CC);
		if ( !resolverwait(servername, &address) ) {
			conoutf(_("%c2could %c3not resolve%c2 server %c5%s%c2"), CC, CC, CC,
			        CC, servername, CC);
			clientpassword[0] = '\0';
			connectrole = CR_DEFAULT;
			return;
...
static void modconnectadmin ( char *servername,
                              int *serverport,
                              char *password )
{
	modprotocol = true;
...
static void _toserver ( char *text, int msg, int msgt )
{
	...
	if ( *text ) {
		if ( msg == SV_TEXTME )
			conoutf("\f%d%s %s", toteam ? 1 : 0, colorname(player1),
			        highlight(text));
		else
...
void c2sinfo ( playerent *d )                  // send update to the server
{
	...
	if ( d->state == CS_ALIVE || d->state == CS_EDITING ) {
		...
		int cn = d->clientnum,
		        ...
		        y = (int) ( d->o.y * DMF ), z =
		                (int) ( ( d->o.z - d->eyeheight ) * DMF ), ya =
		                (int) ( ( 512 * d->yaw ) / 360.0f ), pi =
		                (int) ( ( 127 * d->pitch ) / 90.0f ),
		        r = (int) ( 31 * d->roll / 20 ), dxt =
		                (int) ( d->vel.x * DVELF ), dyt = (int) ( d->vel.y
		                * DVELF ), dzt = (int) ( d->vel.z * DVELF ), dx = dxt
		                - d->vel_t.i[0], dy = dyt - d->vel_t.i[1], dz = dzt
		                - d->vel_t.i[2],
		        // pack rest in 1 int: strafe:2, move:2, onfloor:1, onladder: 1
		        f = ( d->strafe & 3 ) | ( ( d->move & 3 ) << 2 )
		            | ( ( (int) d->onfloor ) << 4 )
		            | ( ( (int) d->onladder ) << 5 )
		            | ( ( d->lifesequence & 1 ) << 6 )
		            | ( ( (int) d->crouching ) << 7 ), g = ( dx ? 1 : 0 )
		                | ( ( dy ? 1 : 0 ) << 1 ) | ( ( dz ? 1 : 0 ) << 2 )
		...
		d->vel_t.i[1] = dyt;
		d->vel_t.i[2] = dzt;
		int usefactor = sfactor < 7 ? 7 : sfactor, sizexy = 1
		        << ( usefactor + 4 );
		if ( cn >= 0 && cn < 32
		     && usefactor <= 7 + 3
		     &&       // map size 7..10
		     x >= 0
		     && x < sizexy && y >= 0 && y < sizexy && z >= -2047
...
int getbuildtype ()
{
	return ( isbigendian() ? 0x80 : 0 ) | ( adler((unsigned char *) guns,
	                                              sizeof ( guns ))
	                                        % 31
	                                        << 8 )
	       |
			...
			case ENET_EVENT_TYPE_DISCONNECT: {
				...
				if ( event.peer == connpeer ) {
					...
					if ( !discmillis || event.data )
					    conoutf(_(
					            "%c3server network error, disconnecting (%s) ..."),
					            CC, disc_reason(event.data));
					disconnect();
				}
				return;
			}
...
{
	loopv(authkeys)
		if ( !strcmp(authkeys[i]->desc, desc) && !strcmp(authkeys[i]->name,
		                                                 player1->name) )
		    return authkeys[i];
	loopv(authkeys)
		if ( !strcmp(authkeys[i]->desc, desc) ) return authkeys[i];
	return NULL;
}
...
COMMANDF(hasauthkey, "ss",
         (char *name, char *desc) { intret(_hasauthkey(name, desc) ? 1 : 0); });
...
static void resetsecuremaps ()
{
	securemaps.deletearrays();
}
static void securemap ( char *map )
{
	if ( map ) securemaps.add(newstring(map));
}
bool securemapcheck ( const char *map, bool msg )
{
	if ( strstr(map, "maps/") == map || strstr(map, "maps\\") == map ) map +=
	        strlen("maps/");
	loopv(securemaps)
		if ( !strcmp(securemaps[i], map) ) {
			if ( msg ) {
				conoutf(_(
				        "%c3Map %c4%s%c3 is secured. This means you CAN'T send, receive or overwrite it."),
				        CC, CC, map, CC);
				if ( connected ) {
					conoutf(_(
					        "%c3If you get this error often, you (or the server) may be running an outdated game."),
					        CC);
					conoutf(_(
					        "%c3You can check for updates at %c1http://assault.cubers.net/download.html"),
					        CC, CC);
				}
			}
			return true;
...
static void sendmap ( char *mapname )
{
	...
	putint(p, cfgsizegz);
	putint(p, revision);
	if ( MAXMAPSENDSIZE - p.length() < mapsize + cfgsizegz || cfgsize
	        > MAXCFGFILESIZE ) {
...
void getdemo ( int *idx, char *dsp )
{
	if ( !multiplayer(false) ) {
		conoutf("%c3Getting demo from server is not available in singleplayer",
		        CC);
		return;
	}
	if ( dsp && dsp[0] )
...
{
	if ( !multiplayer(false) ) {
		conoutf("%c3Listing demos from server is not available in singleplayer",
		        CC);
		return;
	}
...
static int pingpckservers ( void *data )
{
	...
	loopv(pckservers)
		serverstoping.add(*pckservers[i]);
	...
	loopv(serverstoping)
		loopvj(pckservers)
			if ( !strcmp(serverstoping[i].addr, pckservers[j]->addr) )
			    *pckservers[j] = serverstoping[i];
	...
	loopv(pckservers)
	{
		...
		if ( serv->ping > 0 )
			clientlogf("%d. %s (%d ms)", i + 1, serv->addr, serv->ping);
		else
			clientlogf("%d. %s (did not reply)", i + 1, serv->addr);
	}
	SDL_mutexV(pingpcksrvlock);
...
static int progress_callback_dlpackage ( void *clientp,
                                         double dltotal,
                                         double dlnow,
                                         double ultotal,
                                         double ulnow )
{
	...
	loadingscreen(
	        _("downloading package %d out of %d...\n%s %.0f/%.0f KB (%.1f%%)\n(ESC to cancel)"),
	        pck->number + 1, pck->number + pendingpackages.numelems, pck->name,
	        dlnow / double(1024.0), dltotal / double(1024.0),
	        dltotal == 0 ? 0 : ( dlnow / dltotal * double(100.0) ));
...
static double dlpackage ( package *pck )
{
	...
	sprintf(req,
	        "%s/%s%s",
	        pck->source->addr,
	        strreplace(pckname, pck->name, " ", "%20"),
	        ( pck->type == PCK_MAP || pck->type == PCK_MAPMODEL
	          || pck->type == PCK_SKYBOX ) ? ".zip" : "");
	...
	curl_easy_setopt(pck->curl, CURLOPT_PROGRESSFUNCTION,
	                 progress_callback_dlpackage);
	...
	if ( !result && httpresult == 200 )
		...
		conoutf(_("\f2request for %s failed (cURL %d, HTTP %d)"), req, result,
		        httpresult);
...
int downloadpackages ()
{
	...
	enumerate(pendingpackages, package *, pck, {
		if ( !canceldownloads ) {
			if ( connected ) c2skeepalive()
			;
			// try to avoid time out
...
{
	if ( !havecurl || canceldownloads || type < 0 || type >= PCK_NUM
	     || pendingpackages.access(path) ) return false;
