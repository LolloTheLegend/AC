...
static ENetAddress masteraddress = { ENET_HOST_ANY, ENET_PORT_ANY },
...
ENetSocket connectmaster ()
{
	...
	if ( sock == ENET_SOCKET_NULL || connectwithtimeout(sock, mastername,
	                                                    masteraddress)
	                                 < 0 ) {
		logline(ACLOG_WARNING,
		        sock == ENET_SOCKET_NULL ? "could not open socket" :
		                                   "could not connect");
		return ENET_SOCKET_NULL;
	}
...
static void processmasterinput ()
{
	...
	char *input = &masterin[masterinpos], *end = (char *) memchr(
	        input, '\n', masterin.length() - masterinpos);
	while ( end ) {
		*end++ = '\0';

		const char *args = input;
		while ( args < end && !isspace(*args) )
			args++;
		...
		if ( !strncmp(input, "failreg", cmdlen) )
			logline(ACLOG_WARNING, "master server registration failed: %s",
			        args);
                ...
                int protocol_version )
{
	...
	if ( mastersock != ENET_SOCKET_NULL && ENET_SOCKETSET_CHECK(sockset,
	                                                            mastersock) )
	    flushmasterinput();
}
                    ...
                    bool listen )
{
	...
	if ( listen ) {
		...
		if ( pongsock != ENET_SOCKET_NULL && enet_socket_bind(pongsock,
		                                                      &address)
		                                     < 0 ) {
		...
		if ( lansock != ENET_SOCKET_NULL && ( enet_socket_set_option(
		        lansock, ENET_SOCKOPT_REUSEADDR, 1)
		                                      < 0
		                                      || enet_socket_bind(lansock,
		                                                          &address)
		                                         < 0 ) ) {
			...
			if ( *a && clfilenesting < 3 ) {
				...
				if ( cfg.buf ) {
					printf("reading commandline parameters from file '%s'\n",
					       a);
					for ( char *p = cfg.buf, *l; p < cfg.buf + cfg.filelen;
					        line++ ) {
						l = p;
						p += strlen(p) + 1;
