...
struct animstate           // used for animation blending of animated characters
{
	...
	{
		return frame == o.frame && range == o.range && basetime == o.basetime
		       && speed == o.speed;
	}
	bool operator!= ( const animstate &o ) const
	{
...
struct modelattach
{
	...
	modelattach () :
			tag(NULL), name(NULL), pos(NULL), m(NULL)
	{
	}
	modelattach ( const char *tag, const char *name ) :
			tag(tag), name(name), pos(NULL), m(NULL)
	{
	}
	modelattach ( const char *tag, vec *pos ) :
			tag(tag), name(NULL), pos(pos), m(NULL)
	{
	}
};
...
struct model
{
	...
	model () :
			cullface(true), vertexlight(false), alphablend(false),
			        alphatest(0.9f), translucency(0.25f), scale(1), radius(0),
			        shadowdist(0), translate(0, 0, 0), cachelimit(8), batch(-1)
	{
	}
	virtual ~model ()
	{
	...
	virtual void rendershadow ( int anim,
	                            int varseed,
	                            float speed,
	                            int basetime,
	                            const vec &o,
