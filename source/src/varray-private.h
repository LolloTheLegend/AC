...
namespace varray
{
	struct attribinfo
	{
		...
		attribinfo () :
				type(0), size(0), formatsize(0), offset(0), format(GL_FALSE)
		{
		}
	...
	static int enabled = 0, numattribs = 0, attribmask = 0, numlastattribs = 0,
	        lastattribmask = 0, vertexsize = 0, lastvertexsize = 0;
	...
	int end ()
	{
		...
		bool forceattribs = numattribs != numlastattribs
		        || vertexsize != lastvertexsize || buf != lastbuf;
