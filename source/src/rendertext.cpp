...
static VARP(allowblinkingtext, 0, 0, 1);   // if you're so inclined
static VARP(__fontsetting, 0, 0, 2);

static void newfont ( char *name,
                      char *tex,
                      int *defaultw,
                      int *defaulth,
                      int *offsetx,
...
static void fontchar ( int *x, int *y, int *w, int *h )
{
	if ( !fontdef ) return;
...
static void newsetfont ( const char *name )
{
...
bool setfont ( const char *name )
{
	...
	if ( strcmp(name, "default") == 0 )
		...
		v = 1;
	else if ( strcmp(name, "mono") == 0 ) v = 2;
...
// TODO: Lollo: Fix this crap by importing proper include file
                       ...
                       int y )
{
	...
	glTexCoord2f(tc_left, tc_top);
	glVertex2f(x, y);
	glTexCoord2f(tc_right, tc_top);
	glVertex2f(x + info.w, y);
	glTexCoord2f(tc_right, tc_bottom);
	glVertex2f(x + info.w, y + info.h);
	glTexCoord2f(tc_left, tc_bottom);
	...
	if ( c == 's' )   // save color
	{
		...
		switch ( abs(stack[sp]) )
		{
			...
			case '2':
				...
				break;   // yellow: gameplay action messages, only actions done by players - 230 230 20 too bright
			case '3':
...
#define TEXTWORD TEXTWORDSKELETON
	...
	if ( cursor >= 0 ) {
		...
		int cw =
		        curfont->chars.inrange(cc - 33) ?
		                curfont->chars[cc - 33].w + 1 : curfont->defaultw;
