...
static void drawicon ( Texture *tex,
                       float x,
                       float y,
                       float s,
                       int col,
                       ...
                       float ts )
{
	if ( tex && tex->xs == tex->ys )
	    quad(tex->id, x, y, s, ts * col, ts * row, ts);
}
...
static void drawctficon ( float x,
                          float y,
                          float s,
                          int col,
                          int row,
...
void drawscope ( bool preload )
{
	...
	float sz = min(VIRTW, VIRTH), x1 = VIRTW / 2 - sz / 2, x2 = VIRTW / 2
	        + sz / 2, y1 = VIRTH / 2 - sz / 2, y2 = VIRTH / 2 + sz / 2, border =
	        ( 512 - 64 * 2 ) / 512.0f;
	...
	loopi(8+1)
	{
		float c = 0.5f * ( 1 + border * cos(i * 2 * M_PI / 8.0f) ), s = 0.5f
		        * ( 1 + border * sin(i * 2 * M_PI / 8.0f) );
		glTexCoord2f(c, s);
...
const char *crosshairnames[CROSSHAIR_NUM] = { "default", "teammate", "scope",
                                              "knife", "pistol", "carbine",
                                              "shotgun", "smg", "sniper", "ar",
...
static void loadcrosshair ( char *c, char *name )
{
	...
	for ( int i = 0; i < CROSSHAIR_NUM; i++ ) {
		if ( strcmp(crosshairnames[i], name) == 0 ) {
			n = i;
			break;
		}
	}
...
void drawcrosshair ( playerent *p, int n, color *c, float size )
{
	...
	if ( !crosshair ) {
		...
		if ( !crosshair ) crosshair = crosshairs[CROSSHAIR_DEFAULT] =
	...
	float chsize = s
	        * ( p->weaponsel->type == GUN_ASSAULT && p->weaponsel->shots > 3 ?
	                1.4f : 1.0f )
...
static void drawdmgindicator ()
{
	...
	loopi(4)
	{
		if ( !damagedirections[i] || damagedirections[i] < lastmillis )
		    continue;
		float t = damageindicatorsize
		        / (float) ( damagedirections[i] - lastmillis );
		glPushMatrix();
...
static void drawequipicons ( playerent *p )
{
	...
	if ( p->weaponsel && p->weaponsel->type >= GUN_KNIFE
	     && p->weaponsel->type < NUMGUNS )
	    drawequipicon(
	            1020,
	            1650,
	            c,
	            r,
	            ( !p->weaponsel->mag && p->weaponsel->type != GUN_KNIFE
	              && p->weaponsel->type != GUN_GRENADE ));
	glEnable(GL_BLEND);
}

static void drawradarent ( float x,
                           float y,
                           float yaw,
                           int col,
                           int row,
                           ...
                           ... )
{
	...
	if ( pulse )
		glColor4f(1.0f, 1.0f, 1.0f,
		          0.2f + ( sin(lastmillis / 30.0f) + 1.0f ) / 2.0f);
	else
		glColor4f(1, 1, 1, 1);
	glTranslatef(x, y, 0);
	glRotatef(yaw, 0, 0, 1);
...
struct hudmessages : consolebuffer<hudline>
{
	...
	{
		if ( conlines.length() && ( ( conlines[0].type & HUDMSG_TYPE )
		        == ( type & HUDMSG_TYPE )
	...
	void render ()
	{
		...
		loopi(min(conlines.length(), 3))
			if ( totalmillis - conlines[i].millis < dispmillis ) {
				...
				draw_text(
				        c.line,
				        int(tw > VIRTW * 0.9f ? 0 : ( VIRTW * 0.9f - tw ) / 2),
				        int( ( ( VIRTH * 0.9f ) / 4 * 3 ) + FONTH * i
				            + pow( ( totalmillis - c.millis ) / (float) dispmillis,
				                  4)
				              * VIRTH * 0.9f
				              / 4.0f));
			}
...
static bool isattacking ( playerent *p )
{
	return lastmillis - p->lastaction < 500;
}
...
static void drawradar_showmap ( playerent *p, int w, int h )
{
	...
	if ( showmapbackdrop ) {
		...
		loopi(2)
		{
			int cg =
			        i ? ( showmapbackdrop == 2 ?
			                ( (int) ( 255
			                        * ( 100 - showmapbackdroptransparency )
			                          / 100.0f ) ) :
			                0 ) :
			            ( showmapbackdrop == 2 ?
			                    ( (int) ( 255
	...
	if ( team_isactive(p->team) )
	    drawradarent(ppv.x, ppv.y, p->yaw,
	                 p->state == CS_ALIVE ? ( isattacking(p) ? 2 : 0 ) : 1, 2,
	                 iconsize, isattacking(p), "%s", colorname(p));   // local player
	loopv(players)   // other players
	{
		playerent *pl = players[i];
		if ( !pl || pl
		        == p|| !isteam(p->team, pl->team) || !team_isactive(pl->team) )
		    continue;
		vec rtmp = vec(pl->o).sub(mdd).mul(coordtrans);
		drawradarent(rtmp.x, rtmp.y, pl->yaw,
		             pl->state == CS_ALIVE ? ( isattacking(pl) ? 2 : 0 ) : 1,
		             team_base(pl->team), iconsize, isattacking(pl), "%s",
		             colorname(pl));
	}
	if ( m_flags ) {
		...
		loopi(2)   // flag items
		{
			...
			if ( f.state != CTFF_STOLEN && ! ( m_ktf && f.state == CTFF_IDLE ) ) {
				...
				drawradarent(cpos.x + flgoff, cpos.y - flgoff, 0, 3,
				             m_ktf ? 2 : f.team, iconsize, false);   // draw on entity pos
			}
			...
			if ( f.state == CTFF_STOLEN ) {
				...
				if ( f.actor ) {
					...
					if ( tm ) {
						...
						drawradarent(apos.x, apos.y, 0, 3, m_ktf ? 2 : f.team,
...
static void drawradar_vicinity ( playerent *p, int w, int h )
{
	...
	if ( showradarvalues ) {
		...
		conoutf("U4C: %.2f %.2f | %.2f %.2f", usecenter.x, usecenter.y,
		        usecenter.x / gdim, usecenter.y / gdim);
		//showradarvalues = 0;
	}
	glDisable(GL_BLEND);
	circle(minimaptex, halfviewsize, halfviewsize, halfviewsize,
	       usecenter.x / (float) gdim, usecenter.y / (float) gdim, scaleh, 31);   //Draw mimimaptext as radar background
	glTranslatef(halfviewsize, halfviewsize, 0);
	if ( team_isactive(p->team) )
			...
			if ( f.state != CTFF_STOLEN && ! ( m_ktf && f.state == CTFF_IDLE ) ) {
				if ( cpos.magnitude() < d2s ) {
					...
					drawradarent(cpos.x + offx, cpos.y - offy, camera1->yaw, 3,
					             m_ktf ? 2 : f.team, iconsize, false);   // draw flag on entity pos
				}
			}
			if ( m_ktf && f.state == CTFF_IDLE ) continue;
			if ( f.state == CTFF_STOLEN ) {
				...
				if ( f.actor ) {
					...
					if ( tm ) {
						...
						if ( apos.magnitude() < d2s ) {
							apos.mul(scaled);
							drawradarent(apos.x, apos.y, camera1->yaw, 3,
							             m_ktf ? 2 : f.team, iconsize, true);   // draw near flag thief
						}
					}
				}
			}
	...
	if ( !bordertex )
	    bordertex = textureload("packages/misc/compass-base.png", 3);
...
static void drawteamicons ( int w, int h, bool spect )
{
	...
	quad(icons->id,
	     ...
	     10,
	     VIRTH / 12,
	     team_base(spect ? players[player1->followplayercn]->team : player1->team) ?
	             0.5f : 0,
	     0, 0.49f, 1.0f);
}
...
static const char *medal_str[] = { "Best Fragger", "Dude that dies a lot" };   //just some medals string tests, nothing serious
...
static void CSgetEngineState ()
{
	result(enginestateinfo);
}
                  ...
                  bool underwater )
{
	...
	if ( underwater ) {
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		glColor4ub(hdr.watercolor[0], hdr.watercolor[1], hdr.watercolor[2],
		           102);
	...
	if ( lastmillis < damageblendmillis ) {
		...
		if ( !damagetex )
		    damagetex = textureload("packages/misc/damage.png", 3);
	...
	bool reloading = lastmillis
	        < p->weaponsel->reloading + p->weaponsel->info.reloadtime;
	...
	if ( command )
		commandh -= rendercommand(20, 1570, VIRTW);
	else if ( infostr )
		draw_text(infostr, 20, 1570);
	else if ( targetplayer && showtargetname )
	    draw_text(colorname(targetplayer), 20, 1570);
	glLoadIdentity();
	...
	formatstring(enginestateinfo)(
	        "%d %d %d %d %d", curfps, lod_factor(), nquads, curvert, xtraverts);
	if ( showstats ) {
		if ( showstats == 2 ) {
			const int left = ( VIRTW - 225 - 10 ) * 2, top = ( VIRTH * 7 / 8 )
			        * 2;
			const int ttll = VIRTW * 2 - 3 * FONTH / 2;
			blendbox(left - 24, top - 24, VIRTW * 2 - 72, VIRTH * 2 - 48, true,
			         -1);
			int c_num;
			int c_r = 255;
			int c_g = 255;
...
#define TXTCOLRGB \
			...
			draw_text("lod", left - ( text_width("lod") + FONTH / 2 ),
			          top + 80);
			draw_text("wqd", left - ( text_width("wqd") + FONTH / 2 ),
			          top + 160);
			draw_text("wvt", left - ( text_width("wvt") + FONTH / 2 ),
			          top + 240);
			draw_text("evt", left - ( text_width("evt") + FONTH / 2 ),
			          top + 320);
			...
			c_num = curfps > 150 ?
			        0 : ( curfps > 100 ? 1 : ( curfps > 30 ? 2 : 3 ) );
			TXTCOLRGB
			draw_text(c_val, ttll - text_width(c_val), top, c_r, c_g, c_b);

			int lf = lod_factor();
			formatstring(c_val)("%d", lf);
			c_num = lf > 199 ? ( lf > 299 ? ( lf > 399 ? 3 : 2 ) : 1 ) : 0;
			TXTCOLRGB
			draw_text(c_val, ttll - text_width(c_val), top + 80, c_r, c_g, c_b);

			formatstring(c_val)("%d", nquads);
			c_num = nquads > 3999 ?
			        ( nquads > 5999 ? ( nquads > 7999 ? 3 : 2 ) : 1 ) : 0;
			TXTCOLRGB
			draw_text(c_val, ttll - text_width(c_val), top + 160, c_r, c_g,
			          c_b);

			formatstring(c_val)("%d", curvert);
			c_num = curvert > 3999 ?
			        ( curvert > 5999 ? ( curvert > 7999 ? 3 : 2 ) : 1 ) : 0;
			TXTCOLRGB
			draw_text(c_val, ttll - text_width(c_val), top + 240, c_r, c_g,
			          c_b);

			formatstring(c_val)("%d", xtraverts);
			c_num = xtraverts > 3999 ?
			        ( xtraverts > 5999 ? ( xtraverts > 7999 ? 3 : 2 ) : 1 ) : 0;
			TXTCOLRGB
			draw_text(c_val, ttll - text_width(c_val), top + 320, c_r, c_g,
			          c_b);

			if ( unsavededits )
			    draw_text("U", ttll - text_width("U"), top - 90);
		}
		else {
			if ( dbgpos ) {
				pushfont("mono");
				defformatstring(o_yw)("%05.2f YAW", player1->yaw);
				draw_text(o_yw, VIRTW * 2 - ( text_width(o_yw) + FONTH ),
				          VIRTH * 2 - 15 * FONTH / 2);
				defformatstring(o_p)("%05.2f PIT", player1->pitch);
				draw_text(o_p, VIRTW * 2 - ( text_width(o_p) + FONTH ),
				          VIRTH * 2 - 13 * FONTH / 2);
				defformatstring(o_x)("%05.2f X  ", player1->o.x);
				draw_text(o_x, VIRTW * 2 - ( text_width(o_x) + FONTH ),
				          VIRTH * 2 - 11 * FONTH / 2);
				defformatstring(o_y)("%05.2f Y  ", player1->o.y);
				draw_text(o_y, VIRTW * 2 - ( text_width(o_y) + FONTH ),
				          VIRTH * 2 - 9 * FONTH / 2);
				defformatstring(o_z)("%05.2f Z  ", player1->o.z);
				draw_text(o_z, VIRTW * 2 - ( text_width(o_z) + FONTH ),
				          VIRTH * 2 - 7 * FONTH / 2);
				popfont();
			}
	...
	if ( !intermission && lastgametimeupdate != 0 ) {
		int cssec = ( gametimecurrent + ( lastmillis - lastgametimeupdate ) )
		        / 1000;
		int gtsec = cssec % 60;
		int gtmin = cssec / 60;
		if ( !clockcount ) {
		...
		if ( clockdisplay )
		    draw_text(
		            gtime,
		            ( VIRTW - 225 - 10 ) * 2 - ( text_width(gtime) / 2
		                    + FONTH / 2 ),
		            20);
	}
	...
	if ( hidevote < 2 && multiplayer(false) ) {
		...
		if ( curvote && curvote->millis >= totalmillis
			...
			formatstring(str)(
			        "%d yes vs. %d no", curvote->stats[VOTE_YES], curvote->stats[VOTE_NO]);
			...
			switch ( curvote->result )
			{
				case VOTE_NEUTRAL:
					...
					if ( !curvote->localplayervoted )
					    draw_text("\f3press F1/F2 to vote yes or no", left,
					              top + 560, 255, 255, 255, votealpha);
					break;
				default:
					drawvoteicon(left, top, ( curvote->result - 1 ) & 1, 1,
					             false);
					formatstring(str)(
					        "\f3vote %s", curvote->result == VOTE_YES ? "PASSED" : "FAILED");
	...
	if ( !hidespecthud && spectating && player1->spectatemode != SM_DEATHCAM ) {
		...
		if ( is_spect ) {
			defformatstring(name)("Player %s",
			                      players[player1->followplayercn]->name);
	...
	if ( p->state == CS_ALIVE ) {
		...
		if ( !hidehudequipment ) {
			...
			if ( p->weaponsel && p->weaponsel->type >= GUN_KNIFE
				...
				if ( p->weaponsel->type != GUN_GRENADE )
					...
					p->prevweaponsel->renderstats();
				if ( p->mag[GUN_GRENADE] )
				    p->weapons[GUN_GRENADE]->renderstats();
				glMatrixMode(GL_PROJECTION);
			}
			popfont();
		}
		...
		if ( m_flags && !hidectfhud ) {
			...
			loopi(2)   // flag state
			{
				drawctficon(i * 120 + VIRTW / 4.0f * 3.0f, 1650, 120, i, 0,
				            1 / 4.0f,
				            flaginfos[i].state == CTFF_INBASE ? 255 : 100);
				if ( m_teammode ) {
					defformatstring(count)("%d", flagscores[i]);
					int cw, ch;
					text_bounds(count, cw, ch);
					draw_textf(count,
					           i * 120 + VIRTW / 4.0f * 3.0f + 60 - cw / 2,
					           1590);
				}
			}
			...
			if ( ( flaginfos[0].state == CTFF_STOLEN && flaginfos[0].actor == p
			       && flaginfos[0].ack )
			     || ( flaginfos[1].state == CTFF_STOLEN && flaginfos[1].actor
			             == p
			          && flaginfos[1].ack && ++ft ) ) {
				drawctficon(VIRTW - 225 - 10, VIRTH * 5 / 8, 225, ft, 1,
				            1 / 2.0f,
				            ( sin(lastmillis / 100.0f) + 1.0f ) * 128);
			}
		}
	}
...
static void bar ( float bar, int o, float r, float g, float b )
{
	...
	loopk(10)
	{
		float c = 1.2f * cos(M_PI / 2 + k / 9.0f * M_PI), s = 1
		        + 1.2f * sin(M_PI / 2 + k / 9.0f * M_PI);
	...
	loopk(10)
	{
		float c = cos(M_PI / 2 + k / 9.0f * M_PI), s = 1
		        + sin(M_PI / 2 + k / 9.0f * M_PI);
		glVertex2f(x2 - c * FONTH, y1 + s * FONTH);
		glVertex2f(x1 + c * FONTH, y1 + s * FONTH);
	}
	glEnd();
