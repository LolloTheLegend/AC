...
void toggleedit ( bool force )
{
	...
	player1->state =
	        player1->state == CS_SPECTATE ? CS_SPECTATE :
	                                        ( editing ? CS_EDITING : CS_ALIVE );
...
char *editinfo ()
{
	...
	if ( e >= 0 ) {
		entity &c = ents[e];
		formatstring(info)(
		        "closest entity = %s (%d, %d, %d, %d), ", entnames[c.type], c.attr1, c.attr2, c.attr3, c.attr4);
	}
	if ( selset() )
		concatformatstring(info, "selection = (%d, %d)", ( sels.last() ).xs,
		                   ( sels.last() ).ys);
	else
		concatformatstring(info, "no selection");
	sqr *s;
...
{
	loopv(sels)
		if ( !correctsel(sels[i]) ) sels.remove(i);
}
...
static void makesel ( bool isnew )
{
	if ( isnew || sels.length() == 0 )
		addselection(min(lastx, cx), min(lasty, cy), abs(lastx - cx) + 1,
		             abs(lasty - cy) + 1, max(lasth, ch));
	else {
		block &cursel = sels.last();
		cursel.x = min(lastx, cx);
		cursel.y = min(lasty, cy);
		cursel.xs = abs(lastx - cx) + 1;
		cursel.ys = abs(lasty - cy) + 1;
...
void cursorupdate ()                              // called every frame from hud
{
	...
	editaxis =
	        editmode ?
	                ( fabs(camera1->pitch) > 65 ?
	                        13 : ( cyaw < 45 || cyaw > 135 ? 12 : 11 ) ) :
	                0;
		...
		for ( int ix = cx - GRIDSIZE; ix <= cx + GRIDSIZE; ix++ )
			for ( int iy = cy - GRIDSIZE; iy <= cy + GRIDSIZE; iy++ ) {
				...
				if ( sparkletime && showtagclips && showtagclipfocus
				     && s->tag & TAGANYCLIP )
				    particle_cube(s->tag & TAGCLIP ? PART_EPICKUP : PART_EMODEL,
				                  tagnum, taglife, ix, iy);
				if ( s->tag )
	...
	if ( selset() ) {
		...
		loopv(sels)
			box(sels[i], (float) sels[i].h, (float) sels[i].h,
	...
	if ( !showtagclipfocus && showtagclips ) {
		...
		loopv(tagclipcubes)   // all non-solid & have clips
		{
			...
			bool clipped[9];
			loopj(9)
				clipped[j] = !SOLID(o[j]) && ( o[j]->tag & TAGANYCLIP ) > 0;
			int h = s->floor - ( s->type == FHF ? ( s->vdelta + 3 ) / 4 : 0 ),
			        c = s->ceil + ( s->type == CHF ? ( s->vdelta + 3 ) / 4 : 0 );
			loopk(4)
				if ( ( clipped[k] == clipped[k + 1] ) && ! ( clipped[k]
				        && clipped[k + 5] ) )
				    line(x + xo[k + 1], y + yo[k + 1], h, x + xo[k + 1],
				         y + yo[k + 1], c);
			for ( ; h < c; h++ ) {
				int k = ( h + x + y ) & 3;
				if ( k < 2 && !clipped[k] )
				    line(x + xo[k], y + yo[k], h + 1, x + xo[k + 1],
				         y + yo[k + 1], h);
				k = ( h - x - y ) & 3;
				if ( k > 1 && !clipped[k] )
				    line(x + xo[k], y + yo[k], h + 1, x + xo[k + 1],
...
void restoreeditundo ( ucharbuf &q )
{
	...
	while ( !q.overread() && ( type = getuint(q) ) ) {
		int bx = getuint(q), by = getuint(q), bxs = getuint(q), bys = getuint(
		        q);
		short pp[5];
		loopi(5)
			pp[i] = getint(q);
		len = getuint(q);
		if ( ( bx | by | ( bx + bxs ) | ( by + bys ) ) & ~ ( ssize - 1 ) )
		    continue;
		explen = bxs * bys;
	...
	if ( undos.length() || redos.length() )
	    conoutf("restored editing history: %d undos and %d redos",
...
void editdrag ( bool isdown )
{
	...
	if ( !isdown ) for ( int i = sels.length() - 2; i >= 0; i-- ) {
		...
		if ( a.x == b.x && a.y == b.y && a.xs == b.xs && a.ys == b.ys ) {   // making a selection twice will deselect both of it
			sels.drop();
			sels.remove(i);
			break;
		}
...
static void heightfield ( int *t )
{
	edittype(*t == 0 ? FHF : CHF);
}
static void solid ( int *t )
{
...
static void archvertex ( int *span, int *vert, int *delta )
{
	...
	if ( *span >= MAXARCHVERT || *vert >= MAXARCHVERT || *span < 0
	     || *vert < 0 ) return;
...
static void perlinarea ( block &b, int scale, int seed, int psize )
{
	seed = rnd(10000);
	if ( !scale ) scale = 10;
	for ( int y = b.y; y <= b.y + b.ys; y++ )
		for ( int x = b.x; x <= b.x + b.xs; x++ ) {
			sqr *s = S(x, y);
			if ( !SOLID(s) && x != b.x + b.xs && y != b.y + b.ys ) s->type =
			        FHF;
...
static void selfliprotate ( block &sel, int dir )
{
	...
	int x1 = sel.x, x2 = sel.x + sel.xs - 1, y1 = sel.y, y2 = sel.y + sel.ys
	        - 1, x, y;
	switch ( dir )
	{
...
COMMANDF(
        select,
        "iiii",
        (int *x, int *y, int *xs, int *ys) { resetselections(); addselection(*x, *y, *xs, *ys, 0); });
COMMANDF(
        addselection,
        "iiii",
        (int *x, int *y, int *xs, int *ys) { addselection(*x, *y, *xs, *ys, 0); });
