...
enum
{
	VOTE_NEUTRAL = 0, VOTE_YES, VOTE_NO, VOTE_NUM
};
enum
{
	VOTEE_DISABLED = 0,
...
enum
{
	MM_OPEN = 0, MM_PRIVATE, MM_MATCH, MM_NUM
};
enum
{
	MM_MASK = 0x03
...
enum
{
	AT_DISABLED = 0, AT_ENABLED = 1, AT_SHUFFLE = 2, AT_NUM
};
enum
{
	FA_PICKUP = 0,
...
enum
{
	FTR_INFO = 0, FTR_PLAYERWISH, FTR_AUTOTEAM, FTR_SILENTFORCE, FTR_NUM
};
// forceteam-reasons
enum
{
	SP_OK = 0, SP_REFILLMATCH, SP_OK_NUM, SP_WRONGMAP, SP_SPECT, SP_NUM
};
...
enum
{
	BAN_NONE = 0, BAN_VOTE, BAN_AUTO, BAN_BLACKLIST, BAN_MASTER
};
...
enum
{
	GMODE_DEMO = -1, GMODE_TEAMDEATHMATCH = 0,           // 0
	GMODE_COOPEDIT,
	GMODE_DEATHMATCH,
	GMODE_SURVIVOR,
	GMODE_TEAMSURVIVOR,
...
char *filtertext ( char *dst,
                   const char *src,
                   int flags = 1,
                   int len = sizeof(string) - 1 );
void filterrichtext ( char *dst,
                      const char *src,
                      int len = sizeof(string) - 1 );
