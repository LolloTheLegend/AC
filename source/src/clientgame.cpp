...
static bool duplicatename ( playerent *d, char *name = NULL )
{
	...
	loopv(players)
		if ( players[i] && d != players[i] && !strcmp(name, players[i]->name) )
		    return true;
...
char *colorping ( int ping )
{
	static string cping;
	formatstring(cping)(
	        "\fs\f%d%d\fr", ping <= 500 ? 0 : ping <= 1000 ? 2 : 3, ping);
	return cping;
}
...
static void listignored ()
{
	...
	pl[0] = '\0';
	loopv(players)
		if ( players[i] && players[i]->ignored )
		    concatformatstring(pl, ", %s", colorname(players[i]));
	if ( *pl )
		conoutf(_("ignored players: %s"), pl + 2);
	else
		conoutf(_("no players were ignored."));
}

static void clearignored ( int *cn )
{
	loopv(players)
		if ( players[i] && ( *cn < 0 || *cn == i ) ) players[i]->ignored =
		        false;
}
...
static void listmuted ()
{
	...
	pl[0] = '\0';
	loopv(players)
		if ( players[i] && players[i]->muted )
		    concatformatstring(pl, ", %s", colorname(players[i]));
	if ( *pl )
		conoutf(_("muted players: %s"), pl + 2);
	else
		conoutf(_("no players were muted."));
}

static void clearmuted ( char *cn )
{
	loopv(players)
		if ( players[i] && ( *cn < 0 || *cn == i ) ) players[i]->muted = false;
}
...
static void newname ( const char *name )
{
	if ( name[0] ) {
		...
		if ( identexists("onNameChange") ) {
			defformatstring(onnamechange)("onNameChange %d \"%s\"",
			                              player1->clientnum, tmpname);
		...
		conoutf(_("your name is: %s"), player1->name);
...
static int teamatoi ( const char *name )
{
	...
	strtoupper(uc, name);
	loopi(TEAM_NUM)
		if ( !strcmp(teamnames[i], uc) ) return i;
	return -1;
}
...
void newteam ( char *name )
{
	if ( *name ) {
		...
		if ( !team_isvalid(nt) ) {
			conoutf(_(
			        "%c3\"%s\" is not a valid team name (try CLA, RVSF or SPECTATOR)"),
			        CC, name);
			return;
		}
		if ( player1->state == CS_EDITING )
			conoutf(_("you can't change team while editing"));
		else
			addmsg(SV_SWITCHTEAM, "ri", nt);
	}
	else
		conoutf(_("your team is: %s"), team_string(player1->team));
}
...
COMMANDF(
        curmap,
        "i",
        (int *cleaned) { result(*cleaned ? behindpath(getclientmap()) : getclientmap()); });
...
static void playerinfo ( int *cn, const char *attr )
{
	...
	if ( p == player1 || ( team_base(p->team) == team_base(player1->team)
	        && m_teammode )
...
botent *newbotent ()                 // create a new blank player
{
	...
	while ( lukin ) {
		...
		loopv(players)
			if ( !used && players[i] )
			    if ( players[i]->clientnum == nextcn ) used = true;
		if ( !used )
			lukin = false;
		else
			nextcn++;
...
{
	loopv(players)
		if ( players[i] && players[i]->type == ENT_PLAYER ) {
...
static bool showhudtimer ( int maxsecs,
                           int startmillis,
                           const char *msg,
                           bool flash )
{
	...
	if ( nextticks < maxticks )
		hudeditf(HUDMSG_TIMER | HUDMSG_OVERWRITE, flash ? str : str + 2);
	else
		hudeditf(HUDMSG_TIMER, msg);
	return true;
}
...
static void showrespawntimer ()
{
	...
	if ( m_arena ) {
		...
		showhudtimer(
		        ...
		        arenaintermission,
		        "FIGHT!",
		        lastspawnattempt >= arenaintermission && lastmillis
		                < lastspawnattempt + 100);
	}
...
void findplayerstart ( playerent *d, bool mapcenter, int arenaspawn )
{
	...
	if ( !mapcenter ) {
		...
		if ( m_arena && arenaspawn >= 0 ) {
			int x = -1;
			loopi(arenaspawn + 1)
				x = findentity(PLAYERSTART, x + 1, type);
			if ( x >= 0 ) e = &ents[x];
		}
		else if ( ( m_teammode || m_arena ) && !m_ktf )   // ktf uses ffa spawns
		{
			loopi(r)
				spawncycle = findentity(PLAYERSTART, spawncycle + 1, type);
			...
			loopi(r)
			{
				...
				spawncycle = /*m_ktf && */
				        numspawn[2] > 4 ?
				                ...
				                findentity(PLAYERSTART, spawncycle + 1);
				if ( spawncycle < 0 ) continue;
				float dist = nearestenemy(
				        vec(ents[spawncycle].x, ents[spawncycle].y,
				            ents[spawncycle].z),
				        d->team);
	...
	entinmap(d);
	if ( identexists(
	        "onSpawn")/* && (m_teammode && d->team == player1->team)*/) {
...
void respawnself ()
{
	if ( m_mp(gamemode) )
		...
		player1->weaponchanging -= player1->weapons[player1->gunselect]
		                               ->weaponchangetime
...
{
	return ( gamemode != GMODE_COOPEDIT
	        && ( Mh >= MAXMHEIGHT || MA >= MAXMAREA ) );
}
...
static inline const char * spawn_message ()
{
	if ( spawnpermission == SP_WRONGMAP ) {
		...
		if ( securemapcheck(getclientmap()) )
			...
			return "3You must be on the correct map to spawn. Type /getmap to download it.";
	}
	else if ( m_coop )
...
static void setkillmessage ( int gun, bool gib, const char *message )
{
	...
	copystring(killmessages[gib ? 1 : 0][gun], message,
	           sizeof ( killmessages[gib ? 1 : 0][gun] ));
}

COMMANDF(
        fragmessage,
        "is",
        (int *gun, const char *message) { setkillmessage(*gun, false, message); });
COMMANDF(
        gibmessage,
        "is",
        (int *gun, const char *message) { setkillmessage(*gun, true, message); });
...
static void burstshots ( int gun, int shots )
{
	...
	if ( gun >= 0 && gun < NUMGUNS && guns[gun].isauto ) {
		if ( shots >= 0 )
			burstshotssettings[gun] = min(shots, ( guns[gun].magsize - 1 ));
		else
			intret(burstshotssettings[gun]);
	}
	else
		conoutf(_("invalid gun specified"));
}
                ...
                bool local )
{
	...
	playerent *h =
	        player1->isspectating() && player1->followplayercn >= 0
	        && ( player1->spectatemode == SM_FOLLOW1ST || player1->spectatemode
	                == SM_FOLLOW3RD
	             || player1->spectatemode == SM_FOLLOW3RD_TRANSPARENT ) ?
	                getclient(player1->followplayercn) : NULL;
	if ( !h ) h = player1;
	if ( identexists("onHit") ) {
		defformatstring(o)("onHit %d %d %d %d %d", actor->clientnum,
		                   pl->clientnum, damage, gun, gib ? 1 : 0);
		execute(o);
	}
	if ( actor == h && pl != actor ) {
		if ( ( hitsound == 1 || ( hitsound && h != player1 ) ) && lasthit
		        != lastmillis ) audiomgr.playsound(S_HITSOUND, SP_HIGH);
		lasthit = lastmillis;
	}
...
void dokill ( playerent *pl, playerent *act, bool gib, int gun )
{
	...
	if ( identexists("onKill") ) {
		defformatstring(killevent)("onKill %d %d %d %d", act->clientnum,
		                           pl->clientnum, gun, gib ? 1 : 0);
		execute(killevent);
	}
	...
	if ( pl == act ) {
		...
		if ( pl == player1 )
			...
			outf("%s%s %s teammate %s", act == player1 ? "\f3" : "\f2", aname,
			     death, pname);
	}
	else {
		if ( pl == player1 )
...
static void pstat_weap ( int *cn )
{
	...
	if ( pl )
	    loopi(NUMGUNS)
		    concatformatstring(weapstring, "%s%d %d",
		                       strlen(weapstring) ? " " : "", pl->pstatshots[i],
		                       pl->pstatdamage[i]);
	result(weapstring);
}
...
void silenttimeupdate ( int milliscur, int millismax )
{
	...
	minutesremaining = ( gametimemaximum - gametimecurrent + 60000 - 1 )
	        / 60000;
}
...
void timeupdate ( int milliscur, int millismax )
{
	...
	if ( !minutesremaining ) {
		...
		int sec =
		        60 - ( ( gametimecurrent + ( lastmillis - lastgametimeupdate ) ) / 1000 ) % 60;
		if ( minutesremaining == 1 ) {
			...
			conoutf(_("time remaining: %d minutes"), minutesremaining);
		else
			clientlogf(_("time remaining: %d minutes"), minutesremaining);
	}
}
...
static entity flagdummies[2] =   // in case the map does not provide flags
        { entity(-1, -1, -1, CTF_FLAG, 0, 0, 0, 0), entity(-1, -1, -1, CTF_FLAG,
                                                           0, 1, 0, 0) };
...
{
	loopi(2)
		if ( flaginfos[i].state == CTFF_STOLEN && flaginfos[i].actor == p )
		    initflag(i);
}
	...
	if ( !cleanonly ) {
		loopv(ents)
		{
			...
			if ( e.type == CTF_FLAG ) {
				e.spawned = true;
				if ( e.attr2 >= 2 ) {
					conoutf(_("%c3invalid ctf-flag entity (%i)"), CC, i);
					e.attr2 = 0;
				}
...
static void showmapstats ()
{
	...
	if ( m_flags && F2F < 1000 )
	    conoutf("  Flag-to-flag distance is: %d", (int) fSqrt(F2F));
	if ( item_fail )
	    conoutf("  There are one or more items too close to each other in this map");
}
...
void startmap ( const char *name, bool reset, bool norespawn )   // called just after a map load
{
	...
	if ( good_map() == MAP_IS_BAD )
	    conoutf(_(
	            "You cannot play in this map due to quality requisites. Please, report this incident."));
	if ( mapstats_hud ) showmapstats();

	if ( !reset ) return;

	player1->frags = player1->flagscore = player1->deaths = player1
	    ->lifesequence = player1->points = player1->tks = 0;
	loopv(players)
		if ( players[i] )
		    players[i]->frags = players[i]->flagscore = players[i]->deaths =
		            players[i]->lifesequence = players[i]->points = players[i]
		                ->tks = 0;
	if ( editmode ) toggleedit(true);
	intermission = false;
	showscores(false);
...
void flagmsg ( int flag, int message, int actor, int flagtime )
{
	...
	const char *teamstr = m_ktf ? "the" : neutral ? ownerstr :
	                      own ? "your" : "the enemy";
	const char *flagteam =
	        ( m_ktf && !neutral ) ?
	                ( teammate ? "your teammate " : "your enemy " ) : "";
	...
	switch ( message )
	{
		case FM_PICKUP:
			...
			if ( firstperson ) {
				...
				audiomgr.musicsuggest(M_FLAGGRAB,
				                      m_ctf ? 90 * 1000 : 900 * 1000, true);
				musicplaying = flag;
			}
			else
				hudoutf("\f2%s%s has %s flag", flagteam, colorname(act),
				        teamstr);
			break;
		case FM_LOST:
		case FM_DROP: {
		...
		case FM_KTFSCORE: {
			...
			int m = flagtime / 60;
			if ( m )
				hudoutf("\f2%s%s%s kept the flag for %d minute%s %d seconds now",
				        tc, ta, tb, m, m == 1 ? "" : "s", flagtime % 60);
			else
				hudoutf("\f2%s%s%s kept the flag for %d seconds now", tc, ta,
				        tb, flagtime);
			break;
		}
...
char *votestring ( int type,
                   const char *arg1,
                   const char *arg2,
                   const char *arg3 )
{
	const char *msgs[] = { "kick player %s, reason: %s",
	                       "ban player %s, reason: %s", "remove all bans",
	                       "set mastermode to %s", "%s autoteam",
	                       "force player %s to team %s",
	                       "give admin to player %s",
	                       "load map %s in mode %s%s%s",
	                       "%s demo recording for the next match",
	                       "stop demo recording", "clear all demos",
	                       "set server description to '%s'", "shuffle teams" };
	...
	switch ( type )
	{
		...
		case SA_GIVEADMIN: {
			...
			if ( type == SA_KICK || type == SA_BAN ) {
				...
				if ( m_teammode )
					formatstring(reason)(
					        "%s (%d tks, ping %d)", arg2, p->tks, p->ping);
...
votedisplayinfo *newvotedisplayinfo ( playerent *owner,
                                      int type,
                                      const char *arg1,
                                      const char *arg2,
                                      const char *arg3 )
...
void callvote ( int type, const char *arg1, const char *arg2, const char *arg3 )
{
	...
	if ( v ) {
		...
		if ( identexists("onCallVote") ) {
			defformatstring(runas)("%s %d %d [%s] [%s]", "onCallVote", type,
			                       player1->clientnum, arg1, arg2);
			execute(runas);
		}
	}
	else
		conoutf(_("%c3invalid vote"), CC);
}
			...
			case SA_BAN: {
				if ( !arg1 || !isdigit(arg1[0]) || !arg2 || strlen(arg2) <= 3
					...
					if ( !multiplayer(false) )
						conoutf(_("%c3%s is not available in singleplayer."),
						        CC, t == SA_BAN ? "Ban" : "Kick");
...
int vote ( int v )
{
	...
	if ( curvote->localplayervoted ) {
		conoutf(_("%c3you voted already"), CC);
		return 0;
	}
	packetbuf p(MAXTRANS, ENET_PACKET_FLAG_RELIABLE);
	putint(p, SV_VOTE);
...
void displayvote ( votedisplayinfo *v )
{
	...
	DELETEP(curvote);
	curvote = v;
	conoutf(_("%s called a vote: %s"), v->owner ? colorname(v->owner) : "",
	        curvote->desc);
...
void voteresult ( int v )
{
	if ( curvote && v >= 0 && v < VOTE_NUM ) {
		...
		if ( multiplayer(false) )
		    audiomgr.playsound(v == VOTE_YES ? S_VOTEPASS : S_VOTEFAIL,
		                       SP_HIGH);
...
const char *modestrings[] = { "tdm", "coop", "dm", "lms", "ts", "ctf", "pf",
                              "btdm", "bdm", "lss", "osok", "tosok", "bosok",
                              "htf", "tktf", "ktf", "tpf", "tlss", "bpf",
                              "blss", "btsurv", "btosok" };
...
static void whois ( int *cn )
{
	loopv(players)
		if ( players[i] && players[i]->type == ENT_PLAYER
			...
			if ( m_teammode )
			    conoutf(_("%c0INFO: %c5%s has %d teamkills."), CC, CC, p->name,
			            p->tks);
			if ( ip[3] != 0 || player1->clientrole == CR_ADMIN )
				conoutf("WHOIS client %d:\n\f5name\t%s\n\f5IP\t%d.%d.%d.%d",
				        p->clientnum, colorname(p), ip[0], ip[1], ip[2], ip[3]);   // full IP
			else
				conoutf("WHOIS client %d:\n\f5name\t%s\n\f5IP\t%d.%d.%d.x",
				        p->clientnum, colorname(p), ip[0], ip[1], ip[2]);   // censored IP
		}
}
COMMAND(whois, "i");
...
{
	loopv(players)
		if ( players[i] && !strcmp(name, players[i]->name) ) {
...
void *kickmenu = NULL, *banmenu = NULL, *forceteammenu = NULL, *giveadminmenu =
        NULL;
	...
	loopv(players)
		if ( players[i] ) {
			...
			if ( getalias("_kickbanreason") != NULL )
			    formatstring(kbr)(" [ %s ]", getalias("_kickbanreason"));   // leading space!
			formatstring(m.cmd)(
			        "%s %d%s", menu==kickmenu ? "kick" : (menu==banmenu ? "ban" : (menu==forceteammenu ? "forceteam" : "giveadmin")), i, (menu==kickmenu||menu==banmenu)?(strlen(kbr)>8?kbr:" NONE"):"");   // 8==3 + "format-extra-chars"
...
playerent *updatefollowplayer ( int shiftdirection )
{
	playerent *f =
	        players.inrange(player1->followplayercn) ?
	                players[player1->followplayercn] : NULL;
	if ( !shiftdirection ) {
		if ( f && ( watchingdemo || !f->isspectating() || spectatepersistent
		            || ( !spectatepersistent && !m_arena ) ) ) return f;
	}
	...
	loopv(players)
		if ( players[i] ) {
			if ( player1->team != TEAM_SPECT&& !watchingdemo && m_teammode && team_base(players[i]->team) != team_base(player1->team) )
			    continue;
	...
	// rotate
	int oldidx = -1;
	if ( players.inrange(player1->followplayercn) )
	    oldidx = available.find(players[player1->followplayercn]);
...
static void setfollowplayer ( int cn )
{
	...
	if ( players.inrange(cn) && players[cn] && !m_botmode ) {
		if ( ! ( m_teammode && player1->team != TEAM_SPECT && !watchingdemo
			...
			if ( player1->spectatemode == SM_FLY ) player1->spectatemode =
			        SM_FOLLOW1ST;
		}
	}
}
...
void spectatemode ( int mode )
{
	if ( ( player1->state != CS_DEAD && player1->state != CS_SPECTATE
	     ...
	     || ( !m_teammode && !team_isspect(player1->team)
	          && servstate.mastermode == MM_MATCH ) ) return;   // during ffa matches only SPECTATORS can spectate
	if ( mode == player1->spectatemode || ( m_botmode && mode != SM_FLY ) )
	    return;
	showscores(false);
	switch ( mode )
	{
...
void togglespect ()   // cycle through all spectating modes
{
	if ( m_botmode )
		...
		if ( player1->spectatemode == SM_NONE )
			...
			mode = SM_FOLLOW1ST + ( ( player1->spectatemode - SM_FOLLOW1ST + 1 )
			        % ( SM_OVERVIEW - SM_FOLLOW1ST ) );   // replace SM_OVERVIEW by SM_NUM to enable overview mode
		spectatemode(mode);
	}
}
