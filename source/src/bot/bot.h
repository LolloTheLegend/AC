...
inline void debugbeam ( vec &s, vec &e )
{ /*if (!dedserv) particle_trail(1, 500, s, e);*/
}
#endif
...
{
	COMMAND_ADDBOT = 0, COMMAND_KICKBOT, COMMAND_BOTSKILL
};
...
struct unreachable_ent_s
{
	...
	unreachable_ent_s ( entity *e, int t ) :
			ent(e), time(t)
	{
	}
	;
...
public:
	...
	float Get2DDistance ( const vec &v )
	{
		return ::Get2DDistance(m_pMyEnt->o, v);
	}
	;
	vec GetViewAngles ( void )
	{
		return vec(m_pMyEnt->pitch, m_pMyEnt->yaw, m_pMyEnt->roll);
	}
	;
	void ResetMoveSpeed ( void )
	{
		m_pMyEnt->move = m_pMyEnt->strafe = 0;
	}
	;
	...
	void CheckFOV ( void );
	bool IsVisible ( const vec &o, bool CheckPlayers = false )
	{
		return ::IsVisible(m_pMyEnt->o, o, ( CheckPlayers ) ? m_pMyEnt : NULL);
	}
	;
	bool IsVisible ( playerent *d, bool CheckPlayers = false )
	{
		return ::IsVisible(m_pMyEnt->o, d->o,
		                   ( CheckPlayers ) ? m_pMyEnt : NULL);
	}
	;
	bool IsVisible ( entity *e, bool CheckPlayers = false );
	bool IsVisible ( vec o,
	                 int Dir,
	                 float flDist,
	                 bool CheckPlayers,
	                 float *pEndDist = NULL );
	...
	virtual entity *SearchForEnts ( bool bUseWPs,
	                                float flRange = 9999.0f,
	...
	waypoint_s *GetNearestWaypoint ( float flRange )
	{
		return GetNearestWaypoint(m_pMyEnt->o, flRange);
	}
	;
	waypoint_s *GetNearestTriggerWaypoint ( vec v_src, float flRange );
#ifdef WP_FLOOD
	waypoint_s *GetNearestFloodWP ( vec v_origin, float flRange );
	waypoint_s *GetNearestFloodWP ( float flRange )
	{
		return GetNearestFloodWP(m_pMyEnt->o, flRange);
	}
	;
...
public:
	...
	CStoredBot ( char *name, char *team, short skill ) :
			m_sSkillNr(skill)
	{
		strcpy(m_szName, name);
		strcpy(m_szTeam, team);
	}
	;
};
...
public:
	...
	void RespawnBots ( void )
	{
		loopv (bots)
		if ( bots[i] && bots[i]->pBot ) bots[i]->pBot->Spawn();
	}
	;
	void ClearStoredBots ( void )
	{
		while ( !m_StoredBots.Empty() )
			delete m_StoredBots.Pop();
	}
	void ReAddBot ( CStoredBot *bot )
	{
		CreateBot(bot->m_szTeam, SkillNrToSkillName(bot->m_sSkillNr),
		          bot->m_szName);
	}
	;
	void EndMap ( void );
	...
	;
	void SetBotsShoot ( bool bShoot )
	{
		m_bBotsShoot = bShoot;
	}
	;
	void SetIdleBots ( bool bIdle )
	{
		m_bIdleBots = bIdle;
	}
	;
	void EnableBotView ( botent *bot )
	{
		m_pBotToView = bot;
	}
	;
	...
	void MakeBotFileName ( const char *szFileName,
	                       const char *szDir1,
	                       const char *szDir2,
	                       char *szOutput );
};
