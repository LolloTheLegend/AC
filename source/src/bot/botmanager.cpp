...
const char *CBotManager::GetBotName ()
{
	...
	for ( int j = 0; j < m_sBotNameCount; j++ ) {
		...
		if ( ( player1->state != CS_DEDHOST ) && !strcasecmp(player1->name,
		                                                     m_szBotNames[j]) )
		    ChoiceVal -= 10;
	...
	if ( !BotNameChoices.GetSelection(szOutput) ) szOutput = "Bot";
...
const char *CBotManager::GetBotTeam ()
{
	...
	loopv(players)
		if ( players[i] && team_isactive(players[i]->team) ) {
	...
	// Couldn't find a selection?
	if ( !BotTeamChoices.GetSelection(szOutput) ) szOutput = "b0t";
...
void CBotManager::EndMap ()
{
	...
	loopv(bots)
	{
		...
		if ( bots[i]->pBot && bots[i]->name[0] && team_isactive(bots[i]->team) ) {
			CStoredBot *pStoredBot = new CStoredBot(
			        bots[i]->name, (char *) team_string(bots[i]->team),
...
void CBotManager::BeginMap ( const char *szMapName )
{
	...
	if ( !WaypointClass.LoadWaypoints() ) WaypointClass.StartFlood();
...
{
	loopv(bots)
		if ( bots[i] && bots[i]->pBot ) bots[i]->pBot->m_bSendC2SInit = false;
}
...
void CBotManager::LetBotsHear ( int n, vec *loc )
{
	...
	loopv(bots)
	{
		if ( !bots[i] || !bots[i]->pBot || ( bots[i]->state == CS_DEAD ) )
		    continue;
...
void CBotManager::AddWaypoint ( node_s *pNode )
{
	if ( bots.length() ) {
		...
		loopv(bots)
		{
			...
			if ( !bots[i]->pBot->GetWPFromNode(pNode) )
...
void CBotManager::MakeBotFileName ( const char *szFileName,
                                    const char *szDir1,
                                    const char *szDir2,
                                    char *szOutput )
{
...
void CBotManager::CreateSkillData ()
{
	...
	while ( pSkillFile ) {
		...
		if ( ( cmd_line[0] == '#' ) || ( cmd_line[0] == 0 ) ) continue;   // skip if comment or blank line
		...
		if ( strcasecmp(cmd, "[SKILL1]") == 0 )
			...
			SkillNr = 3;
		else if ( strcasecmp(cmd, "[SKILL5]") == 0 ) SkillNr = 4;

		if ( ( arg1 == NULL ) || ( *arg1 == 0 ) ) continue;
...
{
	static const char *SkillNames[5] = { "best", "good", "medium", "worse",
	                                     "bad" };
...
void CBotManager::CalculateMaxAStarCount ()
{
	...
	{
		m_sMaxAStarBots = 8
		        - short(ceil((float) WaypointClass.m_iWaypointCount / 1000.0f));
		if ( m_sMaxAStarBots < 1 ) m_sMaxAStarBots = 1;
	}
	else
		m_sMaxAStarBots = 1;
}
...
void CBotManager::PickNextTrigger ()
{
	...
	loopv(ents)
	{
		...
		if ( pWptNearEnt ) {
			if ( ( pWptNearEnt->sTriggerNr > 0 ) && ( ( pWptNearEnt->sTriggerNr
			        < lowest )
			                                          || ( lowest == -1 ) ) )
	...
	if ( lowest != -1 ) m_sCurrentTriggerNr = lowest;
}

botent *CBotManager::CreateBot ( const char *team,
                                 const char *skill,
                                 const char *name )
{
	if ( m_bInit ) {
...
#error "Unsupported mod!"
	...
	const char *tempteam =
	        team && *team && strcmp(team, "random") ? team :
	                                                  BotManager.GetBotTeam();
	loopi(TEAM_NUM)
		if ( !strcmp(teamnames[i], tempteam) ) {
			m->team = i;
			break;
		}
	...
	if ( skill && *skill && strcmp(skill, "random") ) {
		...
		// No skill specified, use default
		m->pBot->m_sSkillNr = BotManager.m_sBotSkill;
...
void addbot ( char *arg1, char *arg2, char *arg3 )
{
	...
	if ( b )
		conoutf("Bot connected: %s", b->name);
	else {
...
void drawbeamtobots ()
{
	...
	loopv(bots)
	{
		if ( bots[i] ) particle_trail(PART_SMOKE, 500, player1->o, bots[i]->o);
	}
}
...
void kickallbots ( void )
{
	...
	loopv(bots)
	{
		if ( bots[i] ) {
			if ( bots[i]->name[0] )
			    conoutf("bot %s disconnected", bots[i]->name);
			delete bots[i]->pBot;
			freebotent(bots[i]);
		}
	};
...
void togglebotview ( char *bot )
{
	 ...
	 */
	if ( m_arena ) {
		conoutf("togglebotview is not allowed in %s",
		        modestr(gamemode, modeacronyms > 0));
		return;
	}
...
void testvisible ( int iDir )
{
	...
	else if ( Dir & DOWN ) angles.x = WrapXAngle(angles.x - 45.0f);

	if ( ( Dir & FORWARD ) || ( Dir & BACKWARD ) ) {
		if ( Dir & BACKWARD ) angles.y = WrapYZAngle(angles.y + 180.0f);
	...
	else if ( Dir & DOWN ) angles.x = WrapXAngle(angles.x - 90.0f);
