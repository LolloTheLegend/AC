...
vec CBot::GetEnemyPos ( playerent *d )
{
	...
	vec o = m_pMyEnt->weaponsel->type == GUN_SNIPER && d->head.x >= 0 ?
	        d->head : d->o, offset;
...
{
	return ( IsInFOV(p)
	        || ( m_pBotSkill->flAlwaysDetectDistance > m_pMyEnt->o.dist(p->o) ) )
	       && IsVisible(p);
}
...
bool CBot::FindEnemy ( void )
{
	...
	if ( m_pMyEnt->enemy )   // Bot already has an enemy
	{
		...
		if ( found && !isteam(m_pMyEnt->team, m_pMyEnt->enemy->team) ) {
			if ( ( m_pMyEnt->enemy->state == CS_ALIVE ) && ( IsVisible(
			        m_pMyEnt->enemy) ) )
				return true;
			else
	...
	m_iEnemySearchDelay = lastmillis
	        + int(RandomFloat(MinDelay, MaxDelay) * 1000.0f);
	...
	loopv(players)
	{
		...
		if ( d == m_pMyEnt || !d || isteam(d->team, m_pMyEnt->team)
		     || ( d->state != CS_ALIVE ) ) continue;

		// Check if the enemy is visible
		if ( !DetectEnemy(d) ) continue;
	...
	if ( pNewEnemy ) {
		if ( !m_pMyEnt->enemy )   // Add shoot delay if new enemy is found
		{
			...
			m_iShootDelay =
			        lastmillis + int(
			                RandomFloat(flMinShootDelay, flMaxShootDelay) * 1000.0f);
		}
...
bool CBot::CheckHunt ( void )
{
	...
	if ( m_pHuntTarget )   // Bot already has an enemy to hunt
	{
		...
		if ( found && !isteam(m_pMyEnt->team, m_pHuntTarget->team) ) {
			if ( ( m_pHuntTarget->state == CS_ALIVE ) && IsReachable(
			        m_vHuntLocation) ) return true;
		}
		else
			m_pHuntTarget = NULL;
	}
	...
	if ( m_vHuntLocation != g_vecZero ) m_vPrevHuntLocation = m_vHuntLocation;
	...
	playerent *pNewEnemy = NULL, *d = NULL;
	float flDist, flNearestDist = 99999.9f, flNearestOldPosDistToEnemy =
	        99999.9f;
	...
	loopv(players)
	{
		...
		if ( player1 && !isteam(player1->team, m_pMyEnt->team)
		     && ( player1->state == CS_ALIVE )
		     && ( ( flDist = GetDistance(player1->o) ) <= 250.0f ) ) {
			d = player1;
			EnemyVal = 1;
			...
			if ( EnemyVal >= BestEnemyVal ) {
				...
				for ( int j = 0; j < d->history.size(); j++ ) {
					...
					if ( ( flDist < flNearestOldPosDistToEnemy ) && IsReachable(
					        v) ) {
						flNearestOldPosDistToEnemy = flDist;
						bestfromenemy = v;
					}
				...
				for ( int j = 0; j < m_pMyEnt->history.size(); j++ ) {
					...
					if ( ( flDist < flNearestOldPosDistToBot ) && ::IsVisible(
					        d->o, v)
					     && IsReachable(v) ) {
						flNearestOldPosDistToBot = flDist;
						bestfrombot = v;
	...
	if ( pNewEnemy ) {
		...
		if ( m_vHuntLocation != g_vecZero ) m_vPrevHuntLocation =
		        m_vHuntLocation;
...
bool CBot::HuntEnemy ( void )
{
	...
	if ( flDist <= 3.0f ) bDone = true;

	if ( ( m_fPrevHuntDist > 0.0 ) && ( flDist > m_fPrevHuntDist ) ) bDone =
	        true;
	...
	if ( ( m_iHuntPlayerUpdateTime < lastmillis ) || bDone ) {
		...
		for ( j = 0; j < m_pHuntTarget->history.size(); j++ ) {
			...
			if ( bDone && m_vHuntLocation == OldPos ) continue;

			if ( GetDistance(OldPos) > 250.0f ) continue;
		...
		for ( j = 0; j < m_pMyEnt->history.size(); j++ ) {
			...
			if ( GetDistance(OldPos) > 25.0f ) continue;

			flDist = GetDistance(OldPos);

			if ( ( flDist < NearestDistToBot ) && ::IsVisible(m_pHuntTarget->o,
			                                                  OldPos)
			     && ( IsReachable(OldPos) ) ) {
		...
		if ( BestPosIndexFromEnemy > -1 ) {
			...
			m_vHuntLocation = m_pHuntTarget->history.getpos(
			        BestPosIndexFromEnemy);
			bNew = true;
			m_fPrevHuntDist = 0.0f;
		}
		...
		if ( m_iLookAroundDelay < lastmillis ) {
			if ( m_iLookAroundTime > lastmillis ) {
				if ( m_iLookAroundUpdateTime < lastmillis ) {
					float flAddAngle;
					if ( m_bLookLeft )
						flAddAngle = RandomFloat(-110, -80);
					else
						flAddAngle = RandomFloat(80, 110);
					m_pMyEnt->targetyaw = WrapYZAngle(
					        m_pMyEnt->targetyaw + flAddAngle);
	...
	if ( aimtopos ) AimToVec(m_vHuntLocation);

	debugbeam(m_pMyEnt->o, m_vHuntLocation);

	if ( m_fYawToTurn <= 25.0f ) m_iHuntLastTurnLessTime = lastmillis;

	// Bot had to turn much for a while?
	if ( ( m_iHuntLastTurnLessTime > 0 ) && ( m_iHuntLastTurnLessTime
	        < ( lastmillis - 1000 ) ) ) {
			...
			if ( Get2DDistance(m_vHuntLocation) <= 2.0f ) {
				if ( flHeightDiff >= 1.5f ) {
					if ( flHeightDiff <= JUMP_HEIGHT ) {
						...
						char sz[64];
						sprintf(sz, "OldPos z diff: %f",
						        m_vHuntLocation.z - m_pMyEnt->o.z);
						condebug(sz);
#endif
...
void CBot::CheckWeaponSwitch ()
{
	...
	if ( timeprogress > weapon::weaponchangetime ) {
		...
		if ( m_pMyEnt->weaponsel != NULL )
		    addmsg(SV_WEAPCHANGE, "ri", m_pMyEnt->weaponsel->type);   // 2011jan17:ft: message possibly not needed in a local game!?!
	...
	//if ((lastmillis-m_pMyEnt->lastaction) >= m_pMyEnt->gunwait)
	{
		if ( m_pMyEnt->mag[m_pMyEnt->gunselect] ) {
			...
			if ( WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType == TYPE_SNIPER ) {
				float yawtoturn = fabs(
				        WrapYZAngle(m_pMyEnt->yaw - m_pMyEnt->targetyaw));
				float pitchtoturn = fabs(
				        WrapYZAngle(m_pMyEnt->pitch - m_pMyEnt->targetpitch));
			...
			if ( ( flDist < WeaponInfoTable[m_pMyEnt->gunselect]
			           .flMinFireDistance )
			     || ( flDist > WeaponInfoTable[m_pMyEnt->gunselect]
			              .flMaxFireDistance ) ) return;
...
int CBot::GetShootDelay ()
{
	...
	if ( ( WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType == TYPE_MELEE ) || ( WeaponInfoTable[m_pMyEnt
	                                                                                 ->gunselect]
	                                                                                 .eWeaponType
	                                                                             == TYPE_AUTO ) )
	    return m_pMyEnt->gunwait[m_pMyEnt->gunselect];
	...
	float flMinShootDelay = m_pBotSkill->flMinAttackDelay;
	float flMaxShootDelay = m_pBotSkill->flMaxAttackDelay;
	return max(m_pMyEnt->gunwait[m_pMyEnt->gunselect],
	           int(RandomFloat(flMinShootDelay, flMaxShootDelay) * 1000.0f));
}

void CBot::CheckReload ()   // reload gun if no enemies are around
{
	if ( m_pMyEnt->mag[m_pMyEnt->weaponsel->type] >= WeaponInfoTable[m_pMyEnt
	         ->weaponsel->type].sMinDesiredAmmo ) return;   // do not reload if mindesiredammo is satisfied
...
#define MINSCOPETIME 1000
	...
	else if ( m_pMyEnt->scoping
	        && lastmillis - sniper->scoped_since < MINSCOPETIME ) {
...
void CBot::MainAI ()
{
	...
	if ( CheckCrouch() )
		m_pMyEnt->trycrouch = true;
	else
		m_pMyEnt->trycrouch = false;

	if ( !BotManager.BotsShoot() && m_pMyEnt->enemy ) m_pMyEnt->enemy = NULL;   // Clear enemy when bots may not shoot
			...
			if ( CurrentWPIsValid() == false ) {
				if ( m_iLookForWaypointTime <= lastmillis ) {
					...
					if ( pWP && ( pWP != m_pCurrentWaypoint ) ) {
						...
						if ( bDoNormalNav ) ResetWaypointVars();
					}
					else
						ResetWaypointVars();
...
void CBot::DoCombatNav ()
{
	...
	if ( ( fabs(m_pMyEnt->enemy->o.x - m_pMyEnt->o.x) < r && fabs(
	        m_pMyEnt->enemy->o.y - m_pMyEnt->o.y)
	                                                         < r )
	     && ( ( m_pMyEnt->enemy->o.z + m_pMyEnt->enemy->aboveeye ) < ( m_pMyEnt
	                                                                       ->o.z
	                                                                   + m_pMyEnt
	                                                                       ->aboveeye ) ) ) {
		// Try to get off him!
		...
		if ( IsVisible(LEFT, 4.0f, false) ) AwayDirChoices.Insert(LEFT);
		if ( IsVisible(RIGHT, 4.0f, false) ) AwayDirChoices.Insert(RIGHT);
		if ( IsVisible(FORWARD, 4.0f, false) ) AwayDirChoices.Insert(FORWARD);
		if ( IsVisible(BACKWARD, 4.0f, false) ) AwayDirChoices.Insert(BACKWARD);
	...
	if ( ( ( m_iCheckEntsDelay < lastmillis ) || m_pTargetEnt ) && m_pBotSkill
	         ->bCanSearchItemsInCombat ) {
		m_iCheckEntsDelay = lastmillis + 125;
		bool bSearchItems = false;

		if ( m_pTargetEnt ) {
			// Bot has already found an entity, still valid?
			vec v(m_pTargetEnt->x,
			      m_pTargetEnt->y,
			      S(m_pTargetEnt->x, m_pTargetEnt->y)->floor + m_pMyEnt
			          ->eyeheight);
		...
		if ( !m_pTargetEnt && ( m_iCheckEntsDelay <= lastmillis ) ) {
			if ( WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType == TYPE_MELEE )
				...
				bSearchItems =
				        ( m_pMyEnt->ammo[m_pMyEnt->gunselect] <= WeaponInfoTable[m_pMyEnt
				              ->gunselect].sMinDesiredAmmo );
	...
	if ( ( m_sSkillNr <= 1 ) && ( m_fYawToTurn < 80.0f ) && ( flDist <= 20.0f )
		...
		if ( RandomLong(1, 100) <= sJumpPercent ) {
			...
			// Is the enemy close?
			if ( ( GetDistance(m_pMyEnt->enemy->o) <= 4.0f ) || ( WeaponInfoTable[m_pMyEnt
			                                                          ->gunselect]
			                                                          .eWeaponType
			                                                      == TYPE_MELEE ) ) {
			...
			else if ( WeaponInfoTable[m_pMyEnt->gunselect].eWeaponType
			        != TYPE_MELEE )   // else jump to a random direction
			        {
				/*
				...
				if ( IsVisible( ( BACKWARD | RIGHT ), 4.0f, false) )
				    JumpDirChoices.Insert( ( BACKWARD | RIGHT ),
				                          sBackwardScore);
	...
	if ( ( flDist <= WeaponInfoTable[m_pMyEnt->gunselect].flMinDesiredDistance ) || ( flDist
	        >= WeaponInfoTable[m_pMyEnt->gunselect].flMaxDesiredDistance ) ) {
		...
		for ( int j = -45; j <= 45; j += 45 ) {
			...
			if ( moveback ) src.y = WrapYZAngle(src.y + 180);
	...
	else if ( m_pBotSkill->bCircleStrafe )   // Circle strafe when in desired range...
	{
		...
		// Bot can't circle strafe(low skill), just stand still
		ResetMoveSpeed();
}
...
bool CBot::CheckStuck ()
{
	...
	if ( GetDistance(CurPos, PrevPos) <= 0.1f ) {
		if ( m_bStuck ) {
			if ( m_iStuckTime < lastmillis ) IsStuck = true;
		}
		else {
			m_bStuck = true;
	...
	if ( IsStuck ) {
		...
		if ( RandomLong(0, 2) == 0 )
			...
			m_pMyEnt->targetyaw = WrapYZAngle(
			        m_pMyEnt->yaw + RandomLong(60, 160));
		return true;
	}
...
//    if ((bHasGoal) && (flGoalDist < 2.0f))
	...
	if ( bHasGoal && ( flGoalDist < flTraceDist ) ) flTraceDist = flGoalDist;
	...
	if ( !IsVisible(start, FORWARD, flTraceDist, false) ) {
		...
		if ( IsVisible(start) && !IsVisible(start, FORWARD, flTraceDist,
		                                    false) ) {
			// Jump
			debugnav("High wall");
			m_pMyEnt->jumpnext = true;
...
bool CBot::CheckCrouch ()
{
	...
	if ( !IsVisible(vec(start).add(crouch), FORWARD, flTraceDist, false) && IsVisible(
	        vec(start).sub(crouch), FORWARD, flTraceDist, false) ) return true;
	return false;
}
...
bool CBot::CheckStrafe ()
{
	...
	if ( m_iStrafeCheckDelay >= lastmillis ) return false;
	...
	if ( tr.collided ) flLeftDist = GetDistance(from, tr.end);
	...
	to.add(dir);
	TraceLine(from, to, m_pMyEnt, false, &tr);
	if ( tr.collided ) flRightDist = GetDistance(from, tr.end);
	//debugbeam(m_pMyEnt->o, to);
	...
	if ( ( flLeftDist == -1.0f ) && ( flRightDist == -1.0f ) ) {
		...
		from = m_pMyEnt->o;
		from.sub(dir);
		if ( IsVisible(from, FORWARD, 3.0f, false, &flLeftDist) ) flLeftDist =
		        -1.0f;

		// Check right
		from = m_pMyEnt->o;
		from.add(dir);
		if ( IsVisible(from, FORWARD, 3.0f, false, &flRightDist) ) flRightDist =
		        -1.0f;
	}
...
void CBot::CheckFOV ()
{
		...
		for ( int x = cx - 1; x <= ( cx + 1 ); x++ ) {
			for ( int y = cy - 1; y <= ( cy + 1 ); y++ ) {
				...
				//if (!SOLID(s))
				{
					...
					if ( !tr.collided && RandomLong(0, 1) ) turn = true;   // Randomly avoid a fall

					if ( turn ) {
						m_pMyEnt->targetyaw = WrapYZAngle(m_pMyEnt->yaw + 180);
						m_iCheckEnvDelay = m_iStrafeCheckDelay = lastmillis
						        + RandomLong(750, 1500);
	...
	if ( flNearestHitDist <= 4.0f ) {
		...
		if ( WallLeft && tr.collided ) {
			...
			debugnav("Corner");
			m_pMyEnt->targetyaw = WrapYZAngle(
			        m_pMyEnt->yaw + RandomFloat(160.0f, 200.0f));
			m_iCheckEnvDelay = m_iStrafeCheckDelay = lastmillis
			        + RandomLong(750, 1500);
			return;
		}
...
bool CBot::WaterNav ()
{
	...
	if ( m_vWaterGoal == g_vecZero ) {
		...
		for ( int x = ( cx - iSearchRange ); x <= ( cx + iSearchRange ); x++ ) {
			for ( int y = ( cy - iSearchRange ); y <= ( cy + iSearchRange );
			        y++ ) {
				sqr *s = S(x, y);
	...
	if ( m_vWaterGoal != g_vecZero ) {
		...
		AimToVec(aim);
		if ( ( RandomLong(1, 100) <= 15 ) && ( Get2DDistance(m_vWaterGoal)
		        <= 7.0f ) ) m_pMyEnt->jumpnext = true;
		return true;
	}
...
bool CBot::CheckItems ()
{
	...
	if ( m_vGoal == g_vecZero ) m_pTargetEnt = NULL;
	...
	if ( m_pTargetEnt ) {
		if ( HeadToTargetEnt() ) return true;
	}
...
void CBot::HearSound ( int n, vec *o )
{
	...
	loopv(players)
	{
		...
		isteam(m_pMyEnt->team, d->team) ) continue;
